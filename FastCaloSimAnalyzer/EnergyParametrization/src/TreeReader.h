...
class TreeReader  //: public TTreeFormulaManager
 ...
 public:
  ...
  double GetVariable(const char* c, int entry = -2);  // return variable s for a
                                                      // given entry (<0 ->
                                                      // current entry)
  int GetEntry(int entry =
                   -1);  // Read a given entry in the buffer (-1 -> next entry);
  int GetEntries() { return m_entries; }
  TTree* GetTree() { return m_tree; }
  void Restart() { m_currentEntry = -1; }

  ClassDef(TreeReader,
           1)  // Integrate this class into ROOT (must be the last member)
};
