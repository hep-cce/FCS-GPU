...
class TFCSMakeFirstPCA {
 public:
  ...
  std::vector<TH1D*> get_G4_histos_from_tree(std::vector<int> layer_number,
                                             TreeReader* read_inputTree);
  std::vector<TH1D*> get_cumul_histos(std::vector<std::string> layer,
                                      std::vector<TH1D*>);
  double get_cumulant(double x, TH1D* h);
  // double get_cumulant_random(double x, TH1D* h);
  double get_cumulant_random(CLHEP::HepRandomEngine* randEngine, double x,
                             TH1D* h);
  double get_edepositcut() {
    return m_edepositcut;
  };
