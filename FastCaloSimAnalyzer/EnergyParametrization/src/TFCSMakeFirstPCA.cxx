...
void TFCSMakeFirstPCA::run(CLHEP::HepRandomEngine* randEngine) {
  ...
  cout << "  Number of bins in the cumulative histograms " << m_numberfinebins
       << endl;
  ...
  TH1I* h_layer_input =
      new TH1I("h_layer_input", "h_layer_input", 24, -0.5, 23.5);
  vector<string> layer;
  ...
  for (unsigned int l = 0; l < layer.size(); l++) {
    ...
    T_Gauss->Branch(Form("data_PCA_comp%i", l), &data_PCA[l],
                    Form("data_PCA_comp%i/D", l));
  }
  ...
  for (int event = 0; event < read_inputTree->GetEntries(); event++) {
    ...
    if (m_apply_etacut) {
      ...
      pass_eta =
          (fabs(tlv.Eta()) > m_cut_eta_low && fabs(tlv.Eta()) < m_cut_eta_high);
    }
    if (pass_eta) {
      ...
      for (unsigned int l = 0; l < layerNr.size(); l++)
        e_layer.push_back(
            read_inputTree->GetVariable(Form("cell_energy[%d]", layerNr[l])));
    ...
    if (event % 2000 == 0)
      cout << event << " from " << read_inputTree->GetEntries() << " done "
           << endl;
  ...
  for (int event = 0; event < read_T_Gauss0->GetEntries(); event++) {
    ...
    for (unsigned int l = 0; l < layer.size(); l++)
      data_Gauss[l] =
          read_T_Gauss0->GetVariable(Form("data_Gauss0_%s", layer[l].c_str()));
    principal->X2P(data_Gauss, data_PCA);

    // for(unsigned int l=0;l<layer.size();l++)    cout<<"l "<<l<<" layer
    // "<<layer[l]<<" Gauss "<<data_Gauss[l]<<" PCA "<<data_PCA[l]<<endl;
...
vector<TH1D*> TFCSMakeFirstPCA::get_cumul_histos(vector<string> layer_totE_name,
                                                 vector<TH1D*> histos) {
  ...
  for (unsigned int i = 0; i < histos.size(); i++) {
    TH1D* h_cumul =
        (TH1D*)histos[i]->Clone(Form("h_cumul_%s", layer_totE_name[i].c_str()));
...
vector<int> TFCSMakeFirstPCA::get_relevantlayers(TreeReader* read_inputTree,
                                                 double ecut) {
  ...
  for (int event = 0; event < read_inputTree->GetEntries(); event++) {
    ...
    if (pass_eta) {
      ...
      if (total_e > 0.0001) {
        for (int l = 0; l < NLAYER; l++) {
          double efraction =
              read_inputTree->GetVariable(Form("cell_energy[%d]", l)) / total_e;
        ...
        if (event_ok) {
          ...
          for (int l = 0; l < NLAYER; l++) {
            double eval =
                read_inputTree->GetVariable(Form("cell_energy[%i]", l));
  ...
  // if average energy fraction of a layer is above the threshold, it counts as
  // relevant
  ...
  for (int l = 0; l < NLAYER; l++) {
    if (sum_efraction[l] / (double)good_events >= ecut) {
      if ((m_use_absolute_layercut &&
           sum_e[l] / (double)good_events >= adc_to_energy_conversion[l]) ||
          !m_use_absolute_layercut) {
        layer_number.push_back(l);
        cout << "Layer " << l << " is relevant! <sum_efraction>= "
             << sum_efraction[l] / (double)good_events << " <sum_e> "
             << sum_e[l] / (double)good_events << endl;
      }
    }
  }
...
vector<TH1D*> TFCSMakeFirstPCA::get_G4_histos_from_tree(
    vector<int> layer_number, TreeReader* read_inputTree) {
  ...
  for (int event = 0; event < read_inputTree->GetEntries(); event++) {
    ...
    if (m_apply_etacut) {
      TLorentzVector tlv;
      tlv.SetPxPyPzE(read_inputTree->GetVariable("TruthPx"),
                     read_inputTree->GetVariable("TruthPy"),
                     read_inputTree->GetVariable("TruthPz"),
                     read_inputTree->GetVariable("TruthE"));
      pass_eta =
          (fabs(tlv.Eta()) > m_cut_eta_low && fabs(tlv.Eta()) < m_cut_eta_high);
    }
    if (pass_eta) {
      ...
      for (unsigned int l = 0; l < layer_number.size(); l++) {
        e_layer.push_back(read_inputTree->GetVariable(
            Form("cell_energy[%d]", layer_number[l])));
      }
      /*
      int all_bad=1;
      for(unsigned int l=0;l<layer_number.size();l++)
  ...
  for (unsigned int l = 0; l < layer_number.size(); l++) {
    TH1D* hist = new TH1D(Form("h_data_layer%i", l), Form("h_data_layer%i", l),
                          m_numberfinebins, 0, 1);
    hist->Sumw2();
    h_data.push_back(hist);
  }

  TH1D* h_data_total = new TH1D("h_data_totalE", "h_data_totalE",
                                m_numberfinebins, min_e, max_e);
...
double TFCSMakeFirstPCA::get_cumulant_random(CLHEP::HepRandomEngine* randEngine,
                                             double x, TH1D* h) {
