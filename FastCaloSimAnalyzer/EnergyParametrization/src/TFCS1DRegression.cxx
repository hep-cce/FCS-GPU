...
void TFCS1DRegression::storeRegression(
    string weightfilename, vector<vector<double> >& fWeightMatrix0to1,
...
TH1* TFCS1DRegression::transform(TH1* h_input, float& rangeval,
                                 float& startval) {
...
int TFCS1DRegression::testHisto(TH1* hist, std::string weightfilename,
                                float& rangeval, float& startval,
                                std::string outfilename, int neurons_start,
                                int neurons_end, double cut_maxdev, int ntoys) {
  // int debug=1;
  ...
  while (maxdev > cut_maxdev && neurons <= neurons_end) {
    ...
    int pass_training = 0;
    try {
      TFCS1DRegression::tmvaregression_training(neurons, tree, weightfilename,
                                                outfilename, pass_training);
    }
    ...
    if (pass_training) {
      ...
      std::cout << "Testing the regression with " << ntoys << " toys"
                << std::endl;
      ...
      for (int i = 0; i < ntoys; i++) {
        ...
        double value = TFCS1DRegression::tmvaregression_application(
            random, weightfilename);
        h_output->Fill(value);
      }
      ...
      std::cout << "---> Neurons=" << neurons << " MAXDEV=" << maxdev << "%"
                << std::endl;
    }
  ...
  /*
     TFile* out_iteration=new
     TFile(Form("output/iteration_%s.root",label.c_str()),"RECREATE");
...
double TFCS1DRegression::tmvaregression_application(double uniform,
  ...
  if (myMethodList != "") {
    ...
    for (UInt_t i = 0; i < mlist.size(); i++) {
      ...
      if (Use.find(regMethod) == Use.end()) {
        std::cout << "Method \"" << regMethod << "\" not known in TMVA under "
                                                 "this name. Choose among the "
                                                 "following:" << std::endl;
  ...
  for (std::map<std::string, int>::iterator it = Use.begin(); it != Use.end();
    ...
    if (it->second) {
      ...
      TString weightfilename =
          dir + prefix + "_" + TString(it->first) + ".weights.xml";
...
void TFCS1DRegression::tmvaregression_training(int neurons, TTree* regTree,
                                               std::string weightfile,
  ...
  std::cout << std::endl;
  std::cout << "==> Start TMVARegression with " << neurons << " Neurons "
            << std::endl;
  ...
  factory->BookMethod(
      ...
      Form(
          "!H:!V:VerbosityLevel=Info:NeuronType=sigmoid:NCycles=20000:"
          "HiddenLayers=%i:TestRate=6:TrainingMethod=BFGS:Sampling=0.3:"
...
void TFCS1DRegression::get_weights(string weightfile,
