...
TFCS1DFunction* TFCS1DFunctionFactory::Create(
    TH1* hist, int skip_regression, int neurons_start, int neurons_end,
    double maxdev_regression, double maxdev_smartrebin, int ntoys) {

  // This function is called by the user when he wants a histogram to be
  // transformed into a space efficient variant for the parametrization.
  // All code that decides whether a histogram should be transformed into a
  // TFCS1DFunctionRegression or TFCS1DFunctionHistogram
  // should go here.
  ...
  if (!skip_regression)
    status = TFCS1DRegression::testHisto(hist, xmlweightfilename, rangeval,
                                         startval, outfilename, neurons_start,
  ...
  if (status == 1) {
    ...
    TFCS1DRegression::storeRegression(xmlweightfilename, fWeightMatrix0to1,
                                      fWeightMatrix1to2);
    freg->set_weights(fWeightMatrix0to1, fWeightMatrix1to2);
    remove(outfilename.c_str());
    remove(Form("dl/%s/TMVARegression_MLP.weights.xml",
                xmlweightfilename.c_str()));
