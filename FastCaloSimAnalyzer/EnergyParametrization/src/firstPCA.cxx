...
void firstPCA::run() {
  ...
  cout << "  PCA binning: 1st compo " << m_nbins1 << ", 2nd compo " << m_nbins2
       << endl;
  ...
  vector<TH1D*> histos_data =
      get_relevantlayers_inputs(layerNr, read_inputTree);
  ...
  cout << "--- PCA application and binning of first principal component"
       << endl;
  ...
  TH1D* hPCA_first_component =
      new TH1D("hPCA_first_component", "hPCA_first_component", 10000, -20, 20);
  ...
  for (int event = 0; event < tree_Gauss->GetEntries(); event++) {
    ...
    for (unsigned int l = 0; l < layer.size(); l++)
      input_data[l] =
          tree_Gauss->GetVariable(Form("data_Gauss_%s", layer[l].c_str()));
  ...
  for (int m = 0; m < m_nbins1; m++) {
    ...
    cout << "Quantiles " << m + 1 << "  " << xq[m] << " " << yq[m]
         << " -> Events " << hPCA_first_component->Integral(a, b - 1) << endl;
  }
  ...
  for (int m = 0; m < m_nbins1; m++) {
    ...
    if (m_debuglevel) {
      cout << "1stPCA bin# " << m << " Events " << h_compo1[m]->Integral()
           << endl;
    }
    ...
    for (int u = 0; u < m_nbins2; u++) {
      ...
      cout << "Quantile # " << u << "  " << xq2[u] << " " << yq2[u]
           << " -> Events " << h_compo1[m]->Integral(a, b - 1) << endl;
    }
  ...
  for (unsigned int l = 0; l < layer.size(); l++)
    tree_1stPCA->Branch(Form("energy_%s", layer[l].c_str()), &data[l],
...
vector<TH1D*> firstPCA::get_cumul_histos(vector<string> layer,
                                         vector<TH1D*> histos) {
  ...
  for (unsigned int i = 0; i < histos.size(); i++) {
    TH1D* h_cumul =
        (TH1D*)histos[i]->Clone(Form("h_cumul_%s", layer[i].c_str()));
...
vector<TH1D*> firstPCA::get_relevantlayers_inputs(vector<int>& layerNr,
                                                  TreeReader* read_inputTree) {
  ...
  for (int l = 0; l < NLAYER; l++) {
    if (N_pass_eta > 0) {
      if (sum_efraction[l] / N_pass_eta >= m_edepositcut) {
        layerNr.push_back(l);
        cout << "Layer " << l
             << " is relevant! sum_efraction= " << sum_efraction[l]
  ...
  for (int l = 0; l < NLAYER; l++) {
    ...
    if (is_rel) {
      TH1D* h_data =
          new TH1D(Form("h_data_layer%i", l), Form("h_data_layer%i", l),
                   m_numberfinebins, MinInputs[l], MaxInputs[l]);
      h_data->Sumw2();
...
double firstPCA::get_cumulant(double x, TH1D* h) {
  ...
  y1 = h->GetBinContent(
      TMath::Min(bin, h->GetNbinsX() + 1));  // Y1 = F(x1);  Y1 <= 1
