...
void secondPCA::run(CLHEP::HepRandomEngine* randEngine) {
  ...
  /*
  //if a specific PCA bin was set,check if this is available, and set nbins to
  that
  if(m_PCAbin>0)
  {
         if(m_PCAbin<=nbins)
  ...
  */
  ...
  cout << "   Number of bins of the cumulative histos: " << m_numberfinebins
       << endl;
  ...
  cout << "   Neurons in the regression iteration:" << m_neurons_start << " - "
       << m_neurons_end << endl;
  cout << "   Maximal deviation of approximated histogram (regression):  "
       << m_maxdev_regression << "%" << endl;
  cout << "   Maximal deviation of approximated histogram (smart-rebin): "
       << m_maxdev_smartrebin << "%" << endl;
  cout << endl;
...
// void secondPCA::do_pca(vector<string> layer, int bin, TreeReader*
// read_inputTree, int* samplings)
void secondPCA::do_pca(CLHEP::HepRandomEngine* randEngine, vector<string> layer,
                       int bin, TreeReader* read_inputTree, int* samplings) {
  ...
  for (unsigned int l = 0; l < layer.size(); l++)
    bintree->Branch(Form("energy_%s", layer[l].c_str()), &data[l],
                    Form("energy_%s/D", layer[l].c_str()));
  for (int event = 0; event < read_inputTree->GetEntries(); event++) {
    read_inputTree->GetEntry(event);
    ...
    if (firstPCAbin == bin) {
      for (unsigned int l = 0; l < layer.size(); l++) {
        data[l] =
            read_inputTree->GetVariable(Form("energy_%s", layer[l].c_str()));
  ...
  TPrincipal* principal = new TPrincipal(
      layer.size(), "ND");  // ND means normalize cov matrix and store data
  ...
  for (unsigned int l = 0; l < layer.size(); l++) {
    ...
    T_Gauss->Branch(Form("energy_pca_comp%i", l), &data_PCA[l],
                    Form("energy_pca_%i/D", l));
  }
  ...
  for (unsigned int l = 0; l < layer.size(); l++)
    T_Gauss0->Branch(Form("energy_gauss0_%s", layer[l].c_str()),
                     &data_Gauss0[l],
  ...
  for (int event = 0; event < read_bintree->GetEntries(); event++) {
    ...
    for (unsigned int l = 0; l < layer.size(); l++) {
      double data =
          read_bintree->GetVariable(Form("energy_%s", layer[l].c_str()));
  ...
  cout << std::endl << "- Principal Component Analysis Results in bin " << bin
       << endl;
  principal->Print("MSE");
  ...
  for (int event = 0; event < reader_treeGauss0->GetEntries(); event++) {
    ...
    for (unsigned int l = 0; l < layer.size(); l++) {
      double data = reader_treeGauss0->GetVariable(
          Form("energy_gauss0_%s", layer[l].c_str()));
      data_Gauss[l] = data;
    }
  ...
  cout << "--- Application to get Mean and RMS of the PCA transformed data"
       << endl;
  ...
  for (int event = 0; event < reader_treeGauss->GetEntries(); event++) {
    ...
    for (unsigned int l = 0; l < layer.size(); l++)
      input_data[l] = reader_treeGauss->GetVariable(
          Form("energy_gauss_%s", layer[l].c_str()));
  ...
  for (unsigned int l = 0; l < layer.size(); l++) {
    h_data_PCA.push_back(new TH1D(Form("h_data_PCA_%s", layer[l].c_str()),
                                  Form("h_data_PCA_%s", layer[l].c_str()), 1000,
                                  data_PCA_min[l], data_PCA_max[l]));
  }
  ...
  */
  ...
  symCov->Use(CovarianceMatrix->GetNrows(),
              CovarianceMatrix->GetMatrixArray());  // symCov to be stored!
  ...
  for (unsigned int l = 0; l < layer.size(); l++) {
    cout << endl;
    cout << "====> Now create the fct object for " << layer[l]
         << " <====" << endl;
    cout << endl;
    stringstream ss;
    ss << bin;
    string binstring = ss.str();
    TFCS1DFunction* fct = TFCS1DFunctionFactory::Create(
        cumul_data[l], m_skip_regression, m_neurons_start, m_neurons_end,
...
vector<TH1D*> secondPCA::get_histos_data(vector<string> layer,
                                         TreeReader* read_bintree) {
  ...
  for (unsigned int l = 0; l < layer.size(); l++) {
    TH1D* h_data;
    h_data = new TH1D(Form("h_data_%s", layer[l].c_str()),
                      Form("h_data_%s", layer[l].c_str()), m_numberfinebins,
...
double secondPCA::get_cumulant_random(CLHEP::HepRandomEngine* randEngine,
                                      double x, TH1D* h) {
  ...
  // cout<<"before_content "<<before_content<<" content "<<content<<" cumulant
  // "<<cumulant<<endl;
