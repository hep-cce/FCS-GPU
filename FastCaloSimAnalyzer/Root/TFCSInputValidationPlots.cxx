...
void TFCSInputValidationPlots::PlotJOValidation(std::vector<std::string>* files, std::string var, std::string xlabel)
{
   ...
   for (unsigned int ilayer = 0; ilayer < v_layer.size(); ilayer++)
   {
      ...
      TCanvas* c = TFCSAnalyzerBase::PlotTH1Ratio(h_ppm, h_pm, "#pi, 65 GeV 0.20 < |#eta| < 0.25, layer " + std::to_string(layer), xlabel, name1, name2, "a.u.", name1 + "/" + name2);
...
void TFCSInputValidationPlots::PlotTH1Layer(std::string var, int nbins, double xmin, double xmax, std::string xlabel)
{
   ...
   for (unsigned int ilayer = 0; ilayer < v_layer.size(); ilayer++)
   {
      ...
      TString draw = "";
      if (ilayer == 0) draw = "hist EX2";
      else draw = "hist EX2 same";
...
void TFCSInputValidationPlots::PlotTH1PCA(std::string var, int layer, int nbins, double xmin, double xmax, std::string xlabel)
{
...
void TFCSInputValidationPlots::PlotTH2(std::string var, int layer, int pca, int nbinsx, double xmin, double xmax, int nbinsy, double ymin, double ymax, std::string xlabel, std::string ylabel)
{
   ...
   TString varexp(Form("%s>>hl%ipca%i(%i, %f, %f, %i, %f, %f)", var.c_str(), layer, pca, nbinsx, xmin, xmax, nbinsy, ymin, ymax));
   std::cout << " varexp = " << varexp << std::endl;
