...
bool CaloGeometryFromFile::LoadGeometryFromFile( std::string fileName, std::string treeName,
  ...
  while ( !hashStream->eof() ) {
    ...
    *hashStream >> id >> hash_id;
    ...
    if ( i % 10000 == 0 )
      std::cout << "Line: " << i << " id " << std::hex << id << " hash_id " << std::dec << hash_id << std::endl;
  }
  ...
  for ( Long64_t jentry = 0; jentry < nentries; jentry++ ) {
    ...
    if ( cellId_vs_cellHashId_map.find( cell.m_identify ) != cellId_vs_cellHashId_map.end() ) {
      cell.m_hash_id = cellId_vs_cellHashId_map[cell.m_identify];
      if ( cell.m_hash_id != jentry ) std::cout << jentry << " : ERROR hash=" << cell.m_hash_id << std::endl;
    } else
      std::cout << std::endl << "ERROR: Cell id not found in the cellId_vs_cellHashId_map!!!" << std::endl << std::endl;
    ...
    if ( jentry % 25000 == 0 ) {
      ...
      std::cout << "Checking loading cells from file" << std::endl
                << jentry << " : " << pcell->getSampling() << ", " << pcell->identify() << std::endl;
...
bool CaloGeometryFromFile::checkFCalGeometryConsistency() {
  ...
  for ( int imap = 1; imap <= 3; imap++ ) {
    ...
    if ( (int)m_cells_in_sampling[sampling].size() !=
      ...
      std::cout << "Error: Incompatibility between FCalChannel map and GEO file: "
                   "Different number of cells in m_cells_in_sampling and "
                   "FCal_ChannelMap"
                << std::endl;
      std::cout << "m_cells_in_sampling: " << m_cells_in_sampling[sampling].size() << std::endl;
      std::cout << "FCal_ChannelMap: "
                << 2 * std::distance( m_FCal_ChannelMap.begin( imap ), m_FCal_ChannelMap.end( imap ) ) << std::endl;
      return false;
    }
    ...
    for ( auto it = m_FCal_ChannelMap.begin( imap ); it != m_FCal_ChannelMap.end( imap ); it++ ) {
      ...
      if ( !TMath::AreEqualRel( x, DDE1->x(), 1.E-8 ) || !TMath::AreEqualRel( y, DDE1->y(), 1.E-8 ) ||
        ...
        std::cout << "Error: Incompatibility between FCalChannel map and GEO file \n"
                  << x << " " << DDE1->x() << " " << DDE2->x() << y << " " << DDE1->y() << " " << DDE2->y()
                  << std::endl;
        return false;
      }
    }
