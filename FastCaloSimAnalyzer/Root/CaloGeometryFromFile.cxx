...
bool CaloGeometryFromFile::LoadGeometryFromFile(std::string fileName,
  ...
  for (Long64_t jentry = 0; jentry < nentries; jentry++) {
    ...
    if (jentry % 25000 == 0) {
      ...
      std::cout << "Checking loading cells from file" << std::endl << jentry
                << " : " << pcell->getSampling() << ", " << pcell->identify()
                << std::endl;
    ...
    if ((int)m_cells_in_sampling[sampling].size() !=
      ...
      std::cout
          << "Error: Incompatibility between FCalChannel map and GEO file: "
             ...
             "FCal_ChannelMap" << std::endl;
      std::cout << "m_cells_in_sampling: " << m_cells_in_sampling[sampling]
                                                  .size() << std::endl;
      std::cout << "FCal_ChannelMap: "
                << 2 * std::distance(m_FCal_ChannelMap.begin(imap),
                                     m_FCal_ChannelMap.end(imap)) << std::endl;
      return false;
    }
