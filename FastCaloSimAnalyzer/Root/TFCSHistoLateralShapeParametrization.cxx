...
FCSReturnCode TFCSHistoLateralShapeParametrization::simulate_hit(
  ...
  if (TMath::IsNaN(center_r) or TMath::IsNaN(center_z) or
      ...
      TMath::IsNaN(center_phi)) {  // Check if extrapolation
                                   // fails
    return FCSFatal;
  }
  ...
  if (longprint) {
    if (is_phi_symmetric()) {
      ATH_MSG_INFO(optprint
                   ...
                   << "*" << m_hist.get_HistoBordersy().size() - 1
                   << " bins, #hits=" << m_nhits << " (phi symmetric)");
    } else {
      ATH_MSG_INFO(optprint
                   << "  Histo: " << m_hist.get_HistoBordersx().size() - 1
                   << "*" << m_hist.get_HistoBordersy().size() - 1
                   << " bins, #hits=" << m_nhits << " (not phi symmetric)");
    }
  }
}
...
void TFCSHistoLateralShapeParametrization::LoadHistFuncs() {
  ...
  m_LdFH->set_hf2d(&fh);
  //   std::cout<< "*m_LdFH.hf2d() " << (*(m_LdFH->hf2d())).h_bordersx <<
  // std::endl ;
  m_LdFH->LD2D();
}
#elif defined USE_SYCL
bool TFCSHistoLateralShapeParametrization::InitHisto(sycl::context* ctx) {
  ...
  if (m_histo->Init()) {
    if (m_histo->LoadH2DF(m_hist)) {
      return m_histo->LoadH2DFDevice();
    }
  }
  return false;
}
