...
TH1F* TFCSAnalyzerBase::InitTH1(std::string histname, std::string histtype, int nbins, float low, float high, std::string xtitle, std::string ytitle)
{
...
TH2F* TFCSAnalyzerBase::InitTH2(std::string histname, std::string histtype, int nbinsx, float lowx, float highx, int nbinsy, float lowy, float highy, std::string xtitle, std::string ytitle)
{
...
TProfile* TFCSAnalyzerBase::InitTProfile1D(std::string histname, std::string histtype, int nbinsx, float lowx, float highx, std::string xtitle, std::string ytitle, std::string profiletype)
{
...
TProfile2D* TFCSAnalyzerBase::InitTProfile2D(std::string histname, std::string histtype, int nbinsx, float lowx, float highx, int nbinsy, float lowy, float highy, std::string xtitle, std::string ytitle, std::string profiletype)
{

	string fullname = histname + "_" + histtype;

	TProfile2D* hist = new TProfile2D(fullname.c_str(), fullname.c_str(), nbinsx, lowx, highx, nbinsy, lowy, highy, profiletype.c_str());
	hist->GetXaxis()->SetTitle(xtitle.c_str());
	hist->GetYaxis()->SetTitle(ytitle.c_str());
...
void TFCSAnalyzerBase::GetTH1TTreeDraw(TH1F*& hist, TTree* tree, std::string var, std::string* cut, int nbins, double xmin, double xmax)
{
...
void TFCSAnalyzerBase::GetTH2TTreeDraw(TH2F*& hist, TTree* tree, std::string var, std::string* cut, int nbinsx, double xmin, double xmax, int nbinsy, double ymin, double ymax)
{
...
TCanvas* TFCSAnalyzerBase::PlotPolar(TH2F* h, std::string label, std::string xlabel, std::string ylabel, std::string zlabel, int zoom_level)
{
...
TCanvas* TFCSAnalyzerBase::PlotTH1Ratio(TH1F* h1, TH1F* h2, std::string label, std::string xlabel, std::string leg1, std::string leg2, std::string ylabel1, std::string ylabel2)
{
	...
	TLine *line = new TLine(hratio->GetXaxis()->GetBinLowEdge(1), 1.0, hratio->GetXaxis()->GetBinUpEdge(hratio->GetNbinsX()), 1.0);
...
std::tuple<float, float> TFCSAnalyzerBase::GetUnitsmm(float eta_hit, float d_eta, float d_phi,  float cell_r, float cell_z)
{
	float phi_dist2r = 1.0;
...
TString TFCSAnalyzerBase::GetLabel()
{
	...
	TString label = Form(" %i GeV, %s, %.2f < |#eta| < %.2f", (int)(m_energy / 1e3), particle.c_str(), m_etamin / 1e2, m_etamax / 1e2);
...
TString TFCSAnalyzerBase::GetLayerName(int layerid)
{
	...
	TString layer = "";
	if (layerid == 0) layer = "EM barrel presampler";
	else if (0 < layerid && layerid <= 3) layer = Form("EM barrel %i", layerid);
	else if ( layerid == 4) layer = "EM endcap presampler";
	else if (4 < layerid && layerid <= 7) layer = Form("EM endcap %i ", layerid - 4);
	else if (7 < layerid && layerid <= 11) layer = Form("Hadronic endcap %i ", layerid - 7);
	else if (11 < layerid && layerid <= 14) layer = Form("Tile barrel %i ", layerid - 11);
	else if (14 < layerid && layerid <= 17) layer = Form("Tile Gap %i ", layerid - 14);
	else if (17 < layerid && layerid <= 20) layer = Form("Tile extended barrel %i ", layerid - 17);
	else if (20 < layerid && layerid <= 23) layer = Form(" Forwad EM endcap %i ", layerid - 20);
	else layer = Form("layer %i", layerid);
...
void TFCSAnalyzerBase::CreateHTML(std::string filename, std::vector<std::string> plotNames)
{
	...
	if (myhtml.is_open())
	{
		...
		for (unsigned iplot = 0; iplot < plotNames.size(); iplot++)
		{
			...
			myhtml << "<td align=\"center\" class=\"style1\" ><a  href=\"" << outDir.c_str() << "/" << (plot + ".pdf").c_str() << "\"><img src=\"" << outDir.c_str() << "/" << (plot + ".png").c_str() << "\" width=\"700\" height=\"600\"></a><br>" << "" << "</td>\n";
