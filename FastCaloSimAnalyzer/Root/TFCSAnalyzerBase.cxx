...
TH1F* TFCSAnalyzerBase::InitTH1(std::string histname, std::string histtype,
  ...
  if (std::find(histNameVec.begin(), histNameVec.end(), histname) ==
      histNameVec.end()) {
...
TH2F* TFCSAnalyzerBase::InitTH2(std::string histname, std::string histtype,
                                int nbinsx, float lowx, float highx, int nbinsy,
                                float lowy, float highy, std::string xtitle,
                                std::string ytitle) {

  string fullname = histname + "_" + histtype;

  TH2F* hist = new TH2F(fullname.c_str(), fullname.c_str(), nbinsx, lowx, highx,
                        nbinsy, lowy, highy);
  hist->GetXaxis()->SetTitle(xtitle.c_str());
  hist->GetYaxis()->SetTitle(ytitle.c_str());
...
TProfile* TFCSAnalyzerBase::InitTProfile1D(std::string histname,
                                           std::string histtype, int nbinsx,
  ...
  TProfile* hist = new TProfile(fullname.c_str(), fullname.c_str(), nbinsx,
                                lowx, highx, profiletype.c_str());
  hist->GetXaxis()->SetTitle(xtitle.c_str());
  hist->GetYaxis()->SetTitle(ytitle.c_str());
...
TProfile2D* TFCSAnalyzerBase::InitTProfile2D(
    std::string histname, std::string histtype, int nbinsx, float lowx,
  ...
  TProfile2D* hist =
      new TProfile2D(fullname.c_str(), fullname.c_str(), nbinsx, lowx, highx,
                     nbinsy, lowy, highy, profiletype.c_str());
  hist->GetXaxis()->SetTitle(xtitle.c_str());
  hist->GetYaxis()->SetTitle(ytitle.c_str());
...
void TFCSAnalyzerBase::Fill(TProfile* h, float valuex, float valuey,
                            float weight) {
  TAxis* x = h->GetXaxis();
  int nbinsx = x->GetNbins();
  float xmax = x->GetBinUpEdge(nbinsx);
...
void TFCSAnalyzerBase::Fill(TProfile2D* h, float valuex, float valuey,
                            float valuez, float weight) {
  TAxis* x = h->GetXaxis();
  int nbinsx = x->GetNbins();
  float xmax = x->GetBinUpEdge(nbinsx);
  float xmin = x->GetBinLowEdge(1);
...
void TFCSAnalyzerBase::autozoom(TH1* h1, double& min, double& max, double& rmin,
                                double& rmax) {
...
TH1D* TFCSAnalyzerBase::refill(TH1* h_in, double min, double max, double rmin,
                               double rmax) {
  ...
  // if(debug) cout<<"AFTER rebin ->underflow "<<h_in->GetBinContent(0)<<"
  // startbin "<<start<<" minimum "<<min<<endl;

  TH1D* h_out = new TH1D(TString("rebin_") + h_in->GetName(), h_in->GetTitle(),
                         Nbins, rmin, rmax);
  ...
  // if(debug) cout<<"AFTER refill ->underflow "<<h_out->GetBinContent(0)<<"
  // startbin "<<start<<" minimum "<<min<<endl;

  return h_out;
}

void TFCSAnalyzerBase::GetTH1TTreeDraw(TH1F*& hist, TTree* tree,
                                       std::string var, std::string* cut,
...
void TFCSAnalyzerBase::GetTH2TTreeDraw(TH2F*& hist, TTree* tree,
                                       std::string var, std::string* cut,
                                       int nbinsx, double xmin, double xmax,
                                       int nbinsy, double ymin, double ymax) {

  TH2F* histo = new TH2F();
  TString varexp(Form("%s>>histo(%i, %f, %f, %i, %f, %f)", var.c_str(), nbinsx,
...
TCanvas* TFCSAnalyzerBase::PlotPolar(TH2F* h, std::string label,
  ...
  // TLatex* l2 = new TLatex(-0.6 * zoom, -1.20 * zoom, "Simulation
  // Preliminary");
...
TCanvas* TFCSAnalyzerBase::PlotTH1Ratio(TH1F* h1, TH1F* h2, std::string label,
                                        std::string xlabel, std::string leg1,
                                        std::string leg2, std::string ylabel1,
                                        std::string ylabel2) {
  ...
  TLine* line =
      new TLine(hratio->GetXaxis()->GetBinLowEdge(1), 1.0,
...
std::tuple<float, float> TFCSAnalyzerBase::GetUnitsmm(float eta_hit,
                                                      float d_eta, float d_phi,
                                                      CaloCell* cell) {
  ...
  float eta_jakobi =
      TMath::Abs(2.0 * TMath::Exp(-eta_hit) / (1.0 + TMath::Exp(-2 * eta_hit)));
...
double TFCSAnalyzerBase::GetParticleMass(int pdgid) {
  ...
  // * particle masses (MeV)
  std::map<int, double> pid_mass = {{11, 0.5},
                                    {22, 0},
                                    {211, 139.6},
                                    {2212, 938.2},
                                    {2112, 939.6},
...
void TFCSAnalyzerBase::InitInputTree(TChain* mychain, int layer) {
  ...
  setBranch(mychain, m_branches, "newTTC_IDCaloBoundary_eta",
            &m_TTC_IDCaloBoundary_eta);
  setBranch(mychain, m_branches, "newTTC_IDCaloBoundary_phi",
            &m_TTC_IDCaloBoundary_phi);
  setBranch(mychain, m_branches, "newTTC_IDCaloBoundary_r",
            &m_TTC_IDCaloBoundary_r);
  setBranch(mychain, m_branches, "newTTC_IDCaloBoundary_z",
            &m_TTC_IDCaloBoundary_z);
  ...
  // 		setBranch(mychain, m_branches, Form("energy_layer%d", i),
  // &m_total_layer_cell_energy[i]);
  // 	}
  // }
}
...
std::vector<float> TFCSAnalyzerBase::Getxbins(TH1F* histo, int nbins) {
  // * calculate variable bin width in alpha and dr making sure each bin has
  // almost equal amount of hits.
...
bool TFCSAnalyzerBase::findWord(const std::string sentence,
                                std::string search) {
...
std::string TFCSAnalyzerBase::replaceChar(std::string str, char find,
                                          char replace) {
...
TString TFCSAnalyzerBase::GetLabel() {
  ...
  TString label =
      Form(" %i GeV, %s, %.2f < |#eta| < %.2f", (int)(m_energy / 1e3),
           particle.c_str(), m_etamin / 1e2, m_etamax / 1e2);
...
void TFCSAnalyzerBase::CreateHTML(std::string filename,
  ...
  if (myhtml.is_open()) {
    myhtml << "<p align = \"center\" class=\"style3\"> Plots for : "
           << labeltitle << "</p>\n";
    myhtml << "<table align=\"center\" border=\"1\" cellspacing=\"0\" "
              "cellpadding=\"3\"><tr>\n";
    myhtml << "</tr><tr>\n";
    ...
    for (unsigned iplot = 0; iplot < plotNames.size(); iplot++) {
      ...
      myhtml << "<td align=\"center\" class=\"style2\" ><br> " << plot.c_str()
             << " </td>\n";
      myhtml << "<td align=\"center\" class=\"style1\" ><a  href=\""
             << outDir.c_str() << "/" << (plot + ".pdf").c_str()
             << "\"><img src=\"" << outDir.c_str() << "/"
             << (plot + ".png").c_str()
