...
FCSReturnCode TFCSLateralShapeParametrizationHitChain::simulate(
  ...
  unsigned int nhit = get_number_of_hits(simulstate, truth, extrapol);
  if (nhit <= 0) {
    ATH_MSG_ERROR(
        "TFCSLateralShapeParametrizationHitChain::simulate(): number of hits "
  ...
  /*
    std::string
    sA[5]={"TFCSCenterPositionCalculation","TFCSValidationHitSpy","TFCSHistoLateralShapeParametrization",
           "TFCSHitCellMappingWiggle", "TFCSValidationHitSpy" } ;
    std::string
    sB[3]={"TFCSCenterPositionCalculation","TFCSHistoLateralShapeParametrization",
  ...
  if (debug) {
    ...
    for (TFCSLateralShapeParametrizationHitBase* hitsim : m_chain)
      std::cout << "-----In TFCSLateralShapeParametizationHitChain:"
                << typeid(*hitsim).name() << " " << hitsim << std::endl;
    std::cout << std::endl;
  }
  int ichn = 0;
  ...
  /*
     for(TFCSLateralShapeParametrizationHitBase* hitsim : m_chain) {
        if (std::string(typeid( * hitsim ).name()).find(sA[ichn++]) ==
    std::string::npos) { our_chainA= false ; break ; }
    }

    ichn=0 ;
    for(TFCSLateralShapeParametrizationHitBase* hitsim : m_chain) {
        if (std::string(typeid( * hitsim ).name()).find(sB[ichn++]) ==
    std::string::npos) { our_chainB= false ; break ; }
    }
  */
  ...
  if (nhit > MIN_GPU_HITS && our_chainA) {
    ...
    for (auto hitsim : m_chain) {
      ...
      if (ichn == 1) {
        //	TFCS2DFunctionHistogram h=((TFCSHistoLateralShapeParametrization
        //*) hitsim)->histogram() ; 	std::cout << "size of hist:
        //"<<h.get_HistoBordersx().size() <<", "<<h.get_HistoBordersy().size()
        //		<<"Pointer: " << &h <<std::endl ;
        ...
        args.is_phi_symmetric =
            ((TFCSHistoLateralShapeParametrization*)hitsim)->is_phi_symmetric();
        args.fh2d =
            ((TFCSHistoLateralShapeParametrization*)hitsim)->LdFH()->d_hf2d();

        // std::cout<<"Hitsim_ptr="<<hitsim<<", Ld_FH_ptr="<<
        // ((TFCSHistoLateralShapeParametrization *) hitsim)->LdFH()
      ...
      if (ichn == 2) {
        if (0) {
          std::cout << "---NumberOfBins:"
                    << ((TFCSHitCellMappingWiggle*)hitsim)->get_number_of_bins()
                    << std::endl;
          std::vector<const TFCS1DFunction*> funcs =
              ((TFCSHitCellMappingWiggle*)hitsim)->get_functions();
          for (auto it = funcs.begin(); it != funcs.end(); ++it) {
            std::cout << "----+++type of funcs: " << typeid(*(*it)).name()
                      << ", pointer: " << *it << std::endl;
          }
        }
        auto tt1 = std::chrono::system_clock::now();
    ...
    //   std::cout <<  "Time before GPU simulate_hit :" << diff.count() <<" s"
    //   << std::endl ;

    CaloGpuGeneral::simulate_hits(Ehit, nhit, args);

    for (int ii = 0; ii < args.ct; ++ii) {
      // std::cout<<"celleleIndex="<< args.hitcells_h[ii]<<" " <<
      // args.hitcells_ct_h[ii]<<std::endl;
    ...
    //   diff = t2-t1;
    //  std::cout <<  "Time of GPU simulate_hit :" << diff.count() <<" s" <<"
    //  CT="<<args.ct<<  std::endl ;
  ...
  if (cs > 0 && cs < 8 && cs != 4) {
    doEM = true;
  } else if (cs == 0 || cs == 4 || (cs >= 8 && cs < 21))
    doHad = true;
  else
    doAll = true;
  if ((nhit > fastcalosycl::syclcommon::kMinHits) && doEM) {
    // std::cout << "nhit: " << nhit << std::endl;
    Geo* geo = (Geo*)simulstate.get_geo();

    // Set properties needed for the simulation
    // SimProps is sent to the device for processing/simulating hits
    syclcommon::SimProps simprops;
    simprops.pdgId = truth->pdgid();
    simprops.charge = HepPDT::ParticleID(simprops.pdgId).charge();
    simprops.calo_sampling = cs;
    simprops.extrap_eta_ent = extrapol->eta(cs, SUBPOS_ENT);
    simprops.extrap_phi_ent = extrapol->phi(cs, SUBPOS_ENT);
    simprops.extrap_r_ent = extrapol->r(cs, SUBPOS_ENT);
    simprops.extrap_z_ent = extrapol->z(cs, SUBPOS_ENT);
    simprops.extrap_eta_exit = extrapol->eta(cs, SUBPOS_EXT);
    simprops.extrap_phi_exit = extrapol->phi(cs, SUBPOS_EXT);
    simprops.extrap_z_exit = extrapol->z(cs, SUBPOS_EXT);
    simprops.extrap_r_exit = extrapol->r(cs, SUBPOS_EXT);
    simprops.num_hits = nhit;
    simprops.random_nums = nullptr;
    simprops.devgeo = Geo::DevGeo;
    simprops.rng = simulstate.get_rng();
    simprops.queue = &geo->GetGeoDeviceQueue();
    simprops.num_cells = geo->GetNumCells();
    // simprops.is_first_event = simulstate.get_es()->is_first;
    // simprops.is_last_event = simulstate.get_es()->is_last;

    // Get shared context from SimHitRng for Histo
    SimHitRng* smr = (SimHitRng*)simprops.rng;
    cl::sycl::context* ctx = smr->GetContext();
    unsigned int ichain = 0;
    for (auto simhit : m_chain) {
      if (ichain == 0) {
        ...
        if (!((TFCSHistoLateralShapeParametrization*)simhit)->InitHisto(ctx)) {
          ATH_MSG_ERROR(
              "TFCSLateralShapeParametrizationHitChain::simulate(): Could not "
              "initialize histograms for "
        ...
        simprops.is_phi_symmetric =
            ((TFCSHistoLateralShapeParametrization*)simhit)->is_phi_symmetric();
        ...
        if (!((TFCSHitCellMappingWiggle*)simhit)->InitHisto(ctx)) {
          ATH_MSG_ERROR(
              "TFCSLateralShapeParametrizationHitChain::simulate(): Could not "
              "initialize histograms for TFCSHitCellMappingWiggle!");
          return FCSFatal;
        }
        auto tt2 = std::chrono::system_clock::now();
        TFCSShapeValidation::time_o1 += (tt2 - tt1);
        ...
        simprops.h1df_dev =
            ((TFCSHitCellMappingWiggle*)simhit)->GetHisto()->h1df_dev();
      }
      ichain++;
    }
    ...
    for (unsigned int i = 0; i < nhit; ++i) {
      ...
      for (TFCSLateralShapeParametrizationHitBase* hitsim : m_chain) {
        ...
        for (int i = 0; i <= FCS_RETRY_COUNT; i++) {
          if (i > 0)
            ATH_MSG_WARNING(
                "TFCSLateralShapeParametrizationHitChain::simulate(): Retry "
                "simulate_hit call "
                << i << "/" << FCS_RETRY_COUNT);
