    ...
    for ( auto hitsim : m_chain ) {
      ...
      /*
              if(s.find("TFCSValidationHitSpy") != std::string::npos ) {
               ...
               std::endl ; std::cout<<"---m_hist_hitenergy_weight"<<
         ((TFCSValidationHitSpy*)hitsim)->hist_hitenergy_weight()
               << std::endl ; std::cout<<"---m_hist_hitenergy_mean_r"<<
               ((TFCSValidationHitSpy*)hitsim)->hist_hitenergy_mean_r() << std::endl ;
                            std::cout<<"---m_hist_hitenergy_mean_z"<<
         ((TFCSValidationHitSpy*)hitsim)->hist_hitenergy_mean_z()
               ...
               ((TFCSValidationHitSpy*)hitsim)->hist_matched_dphi_etaboundary() << std::endl ;
         std::cout<<"---m_hist_Rz"<<
               ...
               ((TFCSValidationHitSpy*)hitsim)->get_eta_boundary() << std::endl ;
                ...
                hs[hs_i].hist_hitgeo_dphi.nbin=  hspy_ptr ->hist_hitgeo_dphi()->GetNbinsX() ;
                      hs[hs_i].hist_hitgeo_matchprevious_dphi.nbin=  hspy_ptr
         ->hist_hitgeo_matchprevious_dphi()->GetNbinsX() ; hs[hs_i].hist_hitgeo_dphi.low=  hspy_ptr
         ->hist_hitgeo_dphi()->GetXaxis()->GetXmin() ; hs[hs_i].hist_hitgeo_matchprevious_dphi.low=  hspy_ptr
               ...
               "hs["<<hs_i<<"].hist_hitgeo_matchprevious_dphi.nbin" <<hs[hs_i].hist_hitgeo_matchprevious_dphi.nbin
         <<std::endl ; std::cout << "hs["<<hs_i<<"].hist_hitgeo_matchprevious_dphi.low"
               <<hs[hs_i].hist_hitgeo_matchprevious_dphi.low <<std::endl ; std::cout <<
               "hs["<<hs_i<<"].hist_hitgeo_matchprevious_dphi.up" <<hs[hs_i].hist_hitgeo_matchprevious_dphi.up
         <<std::endl ; std::cout << "hs["<<hs_i<<"].hist_hitgeo_dphi.low" <<hs[hs_i].hist_hitgeo_dphi.low <<std::endl ;
      ...
      */
      ...
      if ( ichn == 1 ) {
        ...
        // ((TFCSHistoLateralShapeParametrization *) hitsim)->LdFH()->hf2d_h()->nbinsx <<", " <<
        // ((TFCSHistoLateralShapeParametrization *) hitsim)->LdFH()->hf2d_h()->nbinsy <<std::endl ;
        auto t4 = std::chrono::system_clock::now();
        TFCSShapeValidation::time_o1 += ( t4 - t3 );

        htparams[n_simbins].f2d = ( (TFCSHistoLateralShapeParametrization*)hitsim )->LdFH()->hf2d_d();
#  ifdef USE_KOKKOS
        htparams[n_simbins].f2d_v = ( (TFCSHistoLateralShapeParametrization*)hitsim )->LdFH()->hf2d_v();
#  endif
      ...
      if ( ichn == 2 && our_chainA ) {
        ...
        auto t4 = std::chrono::system_clock::now();
        TFCSShapeValidation::time_o1 += ( t4 - t3 );
        htparams[n_simbins].f1d = ( (TFCSHitCellMappingWiggle*)hitsim )->LdFH()->hf_d();
#  ifdef USE_KOKKOS
...
#  endif
    ...
    /*
           ...
           ->hist_hitgeo_matchprevious_dphi()->SetEntries((double)args.hs2.hist_hitgeo_matchprevious_dphi.nentries) ;
       hsp2
