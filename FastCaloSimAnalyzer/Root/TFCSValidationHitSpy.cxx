...
FCSReturnCode TFCSValidationHitSpy::simulate_hit( Hit& hit, TFCSSimulationState& simulstate,
  ...
  if ( 1 == 1 ) {
    ...
    if ( cs < 21 ) {
      ...
      std::tie( m_deta_hit_minus_extrapol_mm, m_dphi_hit_minus_extrapol_mm ) = TFCSAnalyzerBase::GetUnitsmm(
          extrapol_eta, deta_hit_minus_extrapol, dphi_hit_minus_extrapol, extrapol_r, extrapol_z );
      ...
      if ( m_hist_hitenergy_alpha_radius or m_hist_hitenergy_alpha_absPhi_radius ) {
        ...
        float alpha_absPhi_mm =
            TMath::ATan2( TMath::Abs( m_dphi_hit_minus_extrapol_mm ), m_deta_hit_minus_extrapol_mm );
        float radius_mm = TMath::Sqrt( m_dphi_hit_minus_extrapol_mm * m_dphi_hit_minus_extrapol_mm +
    ...
    if ( m_hist_Rz ) { m_hist_Rz->Fill( hit.r(), hit.z(), hitenergy ); }
    if ( m_hist_hitenergy_weight ) {
      ...
      if ( ( cs != 3 ) && ( w <= -0.25 || w >= 1.25 ) )
        ATH_MSG_DEBUG( "Found weight outside [-0.25,1.25]: weight=" << w ); // Weights are expected out of range in EMB3
                                                                            // (cs==3)
    }
