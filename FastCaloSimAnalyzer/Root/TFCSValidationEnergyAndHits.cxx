...
TFCSValidationEnergyAndHits::TFCSValidationEnergyAndHits(const char* name, const char* title, TFCSAnalyzerBase* analysis):
...
int TFCSValidationEnergyAndHits::get_number_of_hits(TFCSSimulationState& /*simulstate*/, const TFCSTruthState* /*truth*/, const TFCSExtrapolationState* /*extrapol*/) const
{
  int nhits = 0;
...
FCSReturnCode TFCSValidationEnergyAndHits::simulate(TFCSSimulationState& simulstate, const TFCSTruthState* truth, const TFCSExtrapolationState* extrapol)
{
  if (!analysis()) return FCSFatal;
  simulstate.set_Ebin(analysis()->pca());

  //cout << "Ebin: TFCSValidationEnergyAndHits: " << Ekin_bin() << " TFCSSimulationState: " << simulstate.Ebin() << endl;

  if (Ekin_bin() >= 0 && Ekin_bin() != simulstate.Ebin()) return FCSFatal;

  ATH_MSG_DEBUG( "Ebin: TFCSValidationEnergyAndHits: " << Ekin_bin() << " TFCSSimulationState: " << simulstate.Ebin());
...
//m_hist_hit_dphi(0),m_hist_hitgeo_dphi(0),m_hist_hitgeo_wiggle_dphi(0)
  ...
  for (unsigned int icell = 0; icell < ncells; icell++) {
    ...
    std::vector< double > sum_hit_energy_in_time(m_hist_ratioErecoEhit_vs_Ehit.size(), 0);
    for (int i = 0; i < nhit; ++i) {
      ...
      if (h.sampling > 20 && fabs(hitvec.Eta()) < 3. ) std::cout << "Warning: Found hit in FCal layer which is not in FCal eta range!\n"
            << "Sampling: " << h.sampling << " Cell identifier: " << std::hex << h.identifier << std::dec << " energy: " << h.hit_energy << " eta: " << hitvec.Eta() << " phi: " << hitvec.Phi() << " z: " << h.hit_z << std::endl;
      
      if (h.sampling > 20) hit.setXYZE(h.hit_x, h.hit_y, h.hit_z , sf * h.hit_energy);
      else hit.setEtaPhiZE((float)hitvec.Eta(), (float)hitvec.Phi(), h.hit_z , sf * h.hit_energy);

      m_hitspy.set_saved_hit(hit);
      m_hitspy.set_saved_cellele(cellele);
      for (TFCSLateralShapeParametrizationHitBase* hitsim : chain()) {
        if (msgLvl(MSG::DEBUG)) {
          if (tot_nhit < 2) hitsim->setLevel(MSG::DEBUG);
          else hitsim->setLevel(MSG::INFO);
        }
        hitsim->simulate_hit(hit, simulstate, truth, extrapol);
      }
      sum_hit_energy += h.hit_energy;
      for (unsigned int itime = 0; itime < sum_hit_energy_in_time.size(); ++itime) {
        if (h.hit_time >= m_hist_ratioErecoEhit_vs_Ehit_starttime[itime] && h.hit_time <= m_hist_ratioErecoEhit_vs_Ehit_endtime[itime]) {
          sum_hit_energy_in_time[itime] += h.hit_energy;
        }
      }
      m_deltaEtaAveragedPerEvent += (((TFCSValidationHitSpy*)(chain()[0]))->get_deta_hit_minus_extrapol_mm()) * h.hit_energy;
    ...
    std::vector< double > sum_G4hit_energy_in_time(m_hist_ratioErecoEG4hit_vs_EG4hit.size(), 0);
    for (auto& g4hit : matchedcell.g4hit) {
      sum_G4hit_energy += g4hit.hit_energy;
      for (unsigned int itime = 0; itime < sum_G4hit_energy_in_time.size(); ++itime) {
        if (g4hit.hit_time >= m_hist_ratioErecoEhit_vs_Ehit_starttime[itime] && g4hit.hit_time <= m_hist_ratioErecoEhit_vs_Ehit_endtime[itime]) {
    ...
    if (1 == 0) {
      std::cout << "cell cs=" << cs << " eta=" << cellele->eta() << " phi=" << cellele->phi() << " E=" << matchedcell.cell.energy << " EFCShit=" << sum_hit_energy << " EG4hit=" << sum_G4hit_energy << std::endl;
    }
...
//    if(TMath::Abs(cellele->eta())>1.6 && TMath::Abs(cellele->eta())<2.0) {
    if (1 == 1) {
      ...
      if (TMath::Abs(sum_hit_energy - matchedcell.cell.energy) > diff) {
        ...
        if (m_hist_problematic_hit_eta_phi)
          m_hist_problematic_hit_eta_phi->Fill(cellele->eta(), cellele->phi(), 1);
      }
      for (unsigned int itime = 0; itime < sum_hit_energy_in_time.size(); ++itime) {
        m_hist_ratioErecoEhit_vs_Ehit[itime]->Fill(sum_hit_energy_in_time[itime], matchedcell.cell.energy / sum_hit_energy_in_time[itime]);
      }
      for (unsigned int itime = 0; itime < sum_G4hit_energy_in_time.size(); ++itime) {
        m_hist_ratioErecoEG4hit_vs_EG4hit[itime]->Fill(sum_G4hit_energy_in_time[itime], matchedcell.cell.energy / sum_G4hit_energy_in_time[itime]);
      }
    }
  }
  ...
  if (m_hist_deltaEtaAveragedPerEvent)m_hist_deltaEtaAveragedPerEvent->Fill(m_deltaEtaAveragedPerEvent);
