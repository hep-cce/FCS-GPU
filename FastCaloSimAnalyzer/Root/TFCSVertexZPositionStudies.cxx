...
void TFCSVertexZPositionStudies::loadFiles( string dirname, string filename_nominal,
  ...
  for ( int i = 0; i < m_nshifted; i++ ) {
    ...
    if ( m_files_shifted[i]->IsZombie() ) { cout << "Error: File " << full_filename << " is zombie!" << endl; }
...
void TFCSVertexZPositionStudies::setParticleInfo( string filename ) { // Function will attempt to determine particle
  ...
  while ( std::getline( keystream, substring, '_' ) ) { substrings.push_back( substring ); }
...
void TFCSVertexZPositionStudies::loadHistogramsAllPCAs( int layer, string histname ) {
  ...
  for ( int i = 0; i < m_nshifted; i++ ) { m_histos[i] = (TH1F*)m_files_shifted[i]->Get( m_name.c_str() ); }
...
void TFCSVertexZPositionStudies::loadMeanEnergyHistogram( string histname ) {
  ...
  for ( int ipca = 0; ipca < npcas; ipca++ ) {
    ...
    for ( int ilayer = 0; ilayer < nlayers; ilayer++ ) {
      ...
      m_hist_nominal->SetBinError( ipca + 1,
                                   sqrt( pow( m_hist_nominal->GetBinError( ipca ), 2 ) + h_temp->GetStdDev() ) );
      delete h_temp;
      for ( int ishifted = 0; ishifted < m_nshifted; ishifted++ ) {
        ...
        m_histos[ishifted]->SetBinError(
            ipca + 1, sqrt( pow( m_histos[ishifted]->GetBinError( ipca ), 2 ) + h_temp->GetStdDev() ) );
        delete h_temp;
      }
    }
  }
...
void TFCSVertexZPositionStudies::findBinning( bool useMMbinning, double factor, double quantile ) {
  ...
  for ( int ibin = 1; ibin <= nbins; ibin++ ) { binning_orig[ibin] = h1->GetXaxis()->GetBinUpEdge( ibin ); }
  ...
  for ( int i = 1; i <= Nbins_final; i++ ) { binning[i] = m_binning[i * ( m_binning.size() - 1 ) / Nbins_final]; }
  ...
  cout << Neff << " "
       ...
       << " " << Nbins_final << " " << m_binning.size() << " " << ( m_binning.back() - m_binning[0] ) / Nbins_final
       << endl;
