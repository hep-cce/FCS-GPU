...
TFCSShapeValidation::TFCSShapeValidation(long seed) {
  ...
  m_dev_geo = nullptr;
  m_simhitrng = new SimHitRng();
  // Intialize RNG
  auto tstart_shr_init = std::chrono::system_clock::now();
  m_simhitrng->Init(fastcalosycl::syclcommon::kMaxHits,
                    fastcalosycl::syclcommon::kMaxUniqueHits, 200000, seed);
  auto tstart_shr_gen = std::chrono::system_clock::now();
  // Generate random numbers
  m_simhitrng->Generate(fastcalosycl::syclcommon::kMaxHits);
  auto tend_shr_gen = std::chrono::system_clock::now();
#endif
}
...
TFCSShapeValidation::TFCSShapeValidation(TChain* chain, int layer, long seed) {
  ...
  m_simhitrng = new SimHitRng();
  // Intialize RNG
  tstart_shr_init = std::chrono::system_clock::now();
  m_simhitrng->Init(fastcalosycl::syclcommon::kMaxHits,
                    fastcalosycl::syclcommon::kMaxUniqueHits, 200000, seed);
  // Generate random numbers
  tstart_shr_gen = std::chrono::system_clock::now();
  m_simhitrng->Generate(fastcalosycl::syclcommon::kMaxHits);
  tend_shr_gen = std::chrono::system_clock::now();
#endif
}
...
void TFCSShapeValidation::LoadGeo() {
  ...
  // load geometry files
  m_geo->LoadGeometryFromFile(TFCSSampleDiscovery::geometryName(),
...
void TFCSShapeValidation::LoopEvents(int pcabin = -1) {
  ...
  std::chrono::duration<double> t_c[5] = {
      std::chrono::duration<double, std::ratio<1>>::zero()};
  std::chrono::duration<double> t_bc =
      std::chrono::duration<double, std::ratio<1>>::zero();
  std::chrono::duration<double> t_io =
      std::chrono::duration<double, std::ratio<1>>::zero();
  ...
  if (0) {
    ...
    for (int isample = 0; isample < 24; isample++) {
      ...
      for (int irg = 0; irg < rgs; irg++) {
        std::cout << " region: " << irg
                  << " cells: " << m_geo->get_region_size(isample, irg)
                  << std::endl;
        ...
        std::cout << "     Cell Grid neta,nphi :" << neta << "  " << nphi
                  << std::endl;
      }
      std::cout << "Total cells for sample " << isample << " is " << sample_tot
                << std::endl;
    }
    std::cout << "Total cells for all regions and samples: " << t_cells
              << std::endl;
  }
#elif defined USE_SYCL
  cl::sycl::context* ctx = m_simhitrng->GetContext();
  m_dev_geo = new Geo(ctx);
  auto tstart_geo_init = std::chrono::system_clock::now();
  if (!m_dev_geo->Init(m_geo)) {
    std::cout << "Could not initialize SYCL geometry!";
    return;
  }
  auto tstart_geo_load = std::chrono::system_clock::now();
  if (!m_dev_geo->LoadDeviceGeo()) {
    std::cout << "Could not load SYCL device geometry!";
    return;
  }
  auto tend_geo_load = std::chrono::system_clock::now();
#endif

  int nentries = m_nentries;
  int layer = m_layer;
  std::cout << "TFCSShapeValidation::LoopEvents(): Running on layer = " << layer
            << ", pcabin = " << pcabin << std::endl;
  ...
  for (int ievent = m_firstevent; ievent < nentries; ievent++) {
    ...
    // std::cout << std::endl << "Event: " << ievent <<"Number of Particles: "<<
    // particles << std::endl;
    for (size_t p = 0; p < particles; p++) {
      ...
      if (!m_isNewSample) {
        ...
        std::cout << std::endl
                  << " TTC size: "
                  << (*m_truthCollection)[0].TTC_entrance_eta.size()
                  << std::endl;
        ...
        for (int i = 0; i < CaloCell_ID_FCS::MaxSample; ++i) {
          ...
          extrapol.set_eta(i, TFCSExtrapolationState::SUBPOS_ENT,
                           (*m_truthCollection)[0].TTC_entrance_eta[i]);
          extrapol.set_phi(i, TFCSExtrapolationState::SUBPOS_ENT,
                           (*m_truthCollection)[0].TTC_entrance_phi[i]);
          extrapol.set_r(i, TFCSExtrapolationState::SUBPOS_ENT,
                         (*m_truthCollection)[0].TTC_entrance_r[i]);
          extrapol.set_z(i, TFCSExtrapolationState::SUBPOS_ENT,
                         (*m_truthCollection)[0].TTC_entrance_z[i]);

          extrapol.set_OK(i, TFCSExtrapolationState::SUBPOS_EXT, true);
          extrapol.set_eta(i, TFCSExtrapolationState::SUBPOS_EXT,
                           (*m_truthCollection)[0].TTC_back_eta[i]);
          extrapol.set_phi(i, TFCSExtrapolationState::SUBPOS_EXT,
                           (*m_truthCollection)[0].TTC_back_phi[i]);
          extrapol.set_r(i, TFCSExtrapolationState::SUBPOS_EXT,
                         (*m_truthCollection)[0].TTC_back_r[i]);
          extrapol.set_z(i, TFCSExtrapolationState::SUBPOS_EXT,
                         (*m_truthCollection)[0].TTC_back_z[i]);

          // extrapol.set_OK(i,TFCSExtrapolationState::SUBPOS_MID, true);
          // extrapol.set_eta(i,TFCSExtrapolationState::SUBPOS_MID,
          // m_TTC_mid_eta->at(p).at(i));
          // extrapol.set_phi(i,TFCSExtrapolationState::SUBPOS_MID,
          // m_TTC_mid_phi->at(p).at(i));
          // extrapol.set_r(i,TFCSExtrapolationState::SUBPOS_MID,
          // m_TTC_mid_r->at(p).at(i));
          // extrapol.set_z(i,TFCSExtrapolationState::SUBPOS_MID,
          // m_TTC_mid_z->at(p).at(i));
          extrapol.set_OK(i, TFCSExtrapolationState::SUBPOS_MID, true);
          extrapol.set_eta(i, TFCSExtrapolationState::SUBPOS_MID,
                           0.5 * ((*m_truthCollection)[0].TTC_entrance_eta[i] +
                                  (*m_truthCollection)[0].TTC_back_eta[i]));
          extrapol.set_phi(i, TFCSExtrapolationState::SUBPOS_MID,
                           0.5 * ((*m_truthCollection)[0].TTC_entrance_phi[i] +
                                  (*m_truthCollection)[0].TTC_back_phi[i]));
          extrapol.set_r(i, TFCSExtrapolationState::SUBPOS_MID,
                         0.5 * ((*m_truthCollection)[0].TTC_entrance_r[i] +
                                (*m_truthCollection)[0].TTC_back_r[i]));
          extrapol.set_z(i, TFCSExtrapolationState::SUBPOS_MID,
                         0.5 * ((*m_truthCollection)[0].TTC_entrance_z[i] +
                                (*m_truthCollection)[0].TTC_back_z[i]));
        }
      } else {
        if (m_TTC_IDCaloBoundary_eta->size() > 0) {
          ...
          if (std::isnan(m_TTC_IDCaloBoundary_eta->at(p)) ||
              std::abs(m_TTC_IDCaloBoundary_eta->at(p)) > 5) {
            continue;
          }
        }

        TTC_eta = ((*m_TTC_entrance_eta).at(p).at(layer) +
                   (*m_TTC_back_eta).at(p).at(layer)) /
                  2;

        TTC_phi = ((*m_TTC_entrance_phi).at(p).at(layer) +
                   (*m_TTC_back_phi).at(p).at(layer)) /
                  2;
        TTC_r = ((*m_TTC_entrance_r).at(p).at(layer) +
                 (*m_TTC_back_r).at(p).at(layer)) /
                2;
        TTC_z = ((*m_TTC_entrance_z).at(p).at(layer) +
                 (*m_TTC_back_z).at(p).at(layer)) /
                2;

        for (int i = 0; i < CaloCell_ID_FCS::MaxSample; ++i) {
          //          if(m_total_layer_cell_energy[i]==0) continue;
          // extrapol.set_OK(i,TFCSExtrapolationState::SUBPOS_ENT, true);
          extrapol.set_OK(i, TFCSExtrapolationState::SUBPOS_ENT,
                          m_TTC_entrance_OK->at(p).at(i));
          extrapol.set_eta(i, TFCSExtrapolationState::SUBPOS_ENT,
                           m_TTC_entrance_eta->at(p).at(i));
          extrapol.set_phi(i, TFCSExtrapolationState::SUBPOS_ENT,
                           m_TTC_entrance_phi->at(p).at(i));
          extrapol.set_r(i, TFCSExtrapolationState::SUBPOS_ENT,
                         m_TTC_entrance_r->at(p).at(i));
          extrapol.set_z(i, TFCSExtrapolationState::SUBPOS_ENT,
                         m_TTC_entrance_z->at(p).at(i));

          // extrapol.set_OK(i,TFCSExtrapolationState::SUBPOS_EXT, true);
          extrapol.set_OK(i, TFCSExtrapolationState::SUBPOS_EXT,
                          m_TTC_back_OK->at(p).at(i));
          extrapol.set_eta(i, TFCSExtrapolationState::SUBPOS_EXT,
                           m_TTC_back_eta->at(p).at(i));
          extrapol.set_phi(i, TFCSExtrapolationState::SUBPOS_EXT,
                           m_TTC_back_phi->at(p).at(i));
          extrapol.set_r(i, TFCSExtrapolationState::SUBPOS_EXT,
                         m_TTC_back_r->at(p).at(i));
          extrapol.set_z(i, TFCSExtrapolationState::SUBPOS_EXT,
                         m_TTC_back_z->at(p).at(i));

          /*
          //extrapol.set_OK(i,TFCSExtrapolationState::SUBPOS_MID, true);
          extrapol.set_OK(i,TFCSExtrapolationState::SUBPOS_MID,
          m_TTC_mid_OK->at(p).at(i));
          extrapol.set_eta(i,TFCSExtrapolationState::SUBPOS_MID,
          m_TTC_mid_eta->at(p).at(i));
          extrapol.set_phi(i,TFCSExtrapolationState::SUBPOS_MID,
          m_TTC_mid_phi->at(p).at(i));
          ...
          */
          ...
          extrapol.set_eta(i, TFCSExtrapolationState::SUBPOS_MID,
                           0.5 * (m_TTC_entrance_eta->at(p).at(i) +
                                  m_TTC_back_eta->at(p).at(i)));
          extrapol.set_phi(i, TFCSExtrapolationState::SUBPOS_MID,
                           0.5 * (m_TTC_entrance_phi->at(p).at(i) +
                                  m_TTC_back_phi->at(p).at(i)));
          extrapol.set_r(i, TFCSExtrapolationState::SUBPOS_MID,
                         0.5 * (m_TTC_entrance_r->at(p).at(i) +
                                m_TTC_back_r->at(p).at(i)));
          extrapol.set_z(i, TFCSExtrapolationState::SUBPOS_MID,
                         0.5 * (m_TTC_entrance_z->at(p).at(i) +
                                m_TTC_back_z->at(p).at(i)));
        }
      }
      ...
      if (m_debug == 2)
        std::cout << "TTC eta, phi, r, z = " << TTC_eta << " , " << TTC_phi
                  << " , " << TTC_r << " , " << TTC_z << std::endl;
      ...
      for (auto& validation : m_validations) {
        ...
        if (m_debug >= 1) {
          std::cout << "Simulate : " << validation.basesim()->GetTitle()
                    << " event=" << ievent << " E=" << total_energy()
                    << " Ebin=" << pca() << std::endl;
          std::cout << "Simulate : " << typeid(*(validation.basesim())).name()
                    << " Title: " << validation.basesim()->GetTitle()
                    << " event=" << ievent << " E=" << total_energy()
                    << " Ebin=" << pca()
...
#elif defined USE_SYCL
        chain_simul.set_rng(m_simhitrng);
        chain_simul.set_geo(m_dev_geo);
#endif
        //        std::cout<<"Start simulation of " <<
        //        typeid(*validation.basesim()).name() <<std::endl ;

        validation.basesim()->simulate(chain_simul, &truthTLV, &extrapol);
        if (m_debug >= 1) {
          chain_simul.Print();
          std::cout << "End simulate : " << validation.basesim()->GetTitle()
  ...
  std::cout << "Time of  eventloop GPU load FH  :" << time_o1.count() << " s"
            << std::endl;
#elif defined USE_SYCL
  diff = tstart_shr_gen - tstart_shr_init;
  std::cout << "Time for SyclRng::SimHitRng::Init(): " << diff.count() << " s"
            << std::endl;
  diff = tend_shr_gen - tstart_shr_init;
  std::cout << "Time for SyclRng::SimHitRng::Generate(): " << diff.count()
            << " s" << std::endl;
  diff = tstart_geo_load - tstart_geo_init;
  std::cout << "Time for SyclGeo::Geo::Init(): " << diff.count() << " s"
            << std::endl;
  diff = tend_geo_load - tstart_geo_load;
  std::cout << "Time for SyclGeo::Geo::LoadDeviceGeo(): " << diff.count()
            << " s" << std::endl;
#endif
  std::cout << "Time of  eventloop  LateralShapeParamHitChain  :"
            << time_o2.count() << " s" << std::endl;
  std::cout << "Time of  eventloop  :" << diff1.count() << " s" << std::endl;
  std::cout << "Time of  eventloop  GPU ChainA:" << time_g1.count() << " s"
            << std::endl;
  std::cout << "Time of  eventloop  GPU ChainB:" << time_g2.count() << " s"
            << std::endl;
  std::cout << "Time of  eventloop  host Chain0:" << time_h.count() << " s"
            << std::endl;
  std::cout << "Time of  eventloop  before chain simul:" << t_bc.count() << " s"
            << std::endl;
  std::cout << "Time of  eventloop  I/O read from tree:" << t_io.count() << " s"
            << std::endl;

  for (int ii = 0; ii < 5; ii++)
    std::cout << "Time for Chain " << ii << " is " << t_c[ii].count() << " s"
              << std::endl;
  ...
  /*
    ...
    c=new
    TCanvas(hist_cellEvsdxdy_org->GetName(),hist_cellEvsdxdy_org->GetTitle());
    ...
    c=new
    TCanvas(hist_cellEvsdxdy_sim->GetName(),hist_cellEvsdxdy_sim->GetTitle());
    ...
    c=new
    TCanvas(hist_cellEvsdxdy_ratio->GetName(),hist_cellEvsdxdy_ratio->GetTitle());
...
void TFCSShapeValidation::GeoLg() {
  ...
  Rg_Sample_Index* si = (Rg_Sample_Index*)malloc(CaloGeometry::MAX_SAMPLING *
                                                 sizeof(Rg_Sample_Index));
  ...
  for (int is = 0; is < CaloGeometry::MAX_SAMPLING; ++is) {
    ...
    //    std::cout<<"Sample " << is << "regions: "<< nr << ", Region Index " <<
    //    i << std::endl ;
  }
}

void TFCSShapeValidation::region_data_cpy(CaloGeometryLookup* glkup,
                                          GeoRegion* gr) {
  // Copy all parameters
  ...
  // std::cout << " copy region " << glkup->index() << "neta= " << neta<< ",
  // nphi= "<<nphi<< std::endl ;
  ...
  if (neta != (*(glkup->cell_grid())).size())
    std::cout << "neta " << neta << ", vector eta size "
              << (*(glkup->cell_grid())).size() << std::endl;
  for (int ie = 0; ie < neta; ++ie) {
    //    	if(nphi != (*(glkup->cell_grid()))[ie].size() )
    //		 std::cout<<"neta " << neta << "nphi "<<nphi <<", vector phi
    // size
    //"<<
    //(*(glkup->cell_grid()))[ie].size() << std::endl;

    for (int ip = 0; ip < nphi; ++ip) {
      //	if(glkup->index()==0 ) std::cout<<"in loop.."<< ie << " " <<ip
      //<< std::endl;
      ...
      if (c) {
        ...
        cells[ie * nphi + ip] = -1;
        //	        std::cout<<"NUll cell in loop.."<< ie << " " <<ip <<
        // std::endl;
      }
    }
  }
