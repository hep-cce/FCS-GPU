         ...
         for (int ix = 1; ix <= h_radius_alpha->GetNbinsX(); ++ix) {
            for (int iy = 1; iy <= h_radius_alpha->GetNbinsY(); ++iy) {
               if (h_radius_alpha->GetBinContent(ix, iy) < 0) {
                  std::cout << "WARNING: Histo: " << h_radius_alpha->GetName() << " : " << h_radius_alpha->GetTitle() << " : bin(" << ix << "," << iy << ")=" << h_radius_alpha->GetBinContent(ix, ix) << " is negative. Fixing to 0!" << std::endl;
...
TH2F* TFCS2DParametrization::GetParametrization(TH1F* h_radius, int layer, int pca, std::string opt)
{
   ...
   TH2F* h_r_alpha = new TH2F(Form("h_r_alpha_layer%i_pca%i", layer, pca), Form("h_r_alpha_layer%i_pca%i", layer, pca), nbinsx, xmin, xmax, nbinsy - 1, ybins);
...
TH1F* TFCS2DParametrization::GetHisto(std::string var, int layer, int pca, double cutoff, std::string opt)
{
   ...
   //    std::cout << " bin = " << i << " up edge = " << h_radius->GetXaxis()->GetBinUpEdge(i) << " width = " << h_radius->GetBinWidth(i) << std::endl;
   // }
