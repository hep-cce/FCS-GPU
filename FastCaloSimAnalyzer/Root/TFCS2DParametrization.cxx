...
TFCS2DParametrization::TFCS2DParametrization(TTree* tree,
...
void TFCS2DParametrization::CreateShapeHistograms(double cutoff,
                                                  std::string opt) {
  ...
  auto fout =
      std::unique_ptr<TFile>(TFile::Open(outfileName.c_str(), "recreate"));
  if (!fout) {
      ...
      for (int ix = 1; ix <= h_radius_alpha->GetNbinsX(); ++ix) {
        for (int iy = 1; iy <= h_radius_alpha->GetNbinsY(); ++iy) {
          if (h_radius_alpha->GetBinContent(ix, iy) < 0) {
            std::cout << "WARNING: Histo: " << h_radius_alpha->GetName()
                      << " : " << h_radius_alpha->GetTitle() << " : bin(" << ix
                      << "," << iy
                      << ")=" << h_radius_alpha->GetBinContent(ix, ix)
...
void TFCS2DParametrization::PlotShapePolar() {
  ...
  if (!fshape) {
    std::cerr << "Error: Could not open file '" << shapefileName << "'"
              << std::endl;
    return;
  }
  ...
  for (unsigned int ilayer = 0; ilayer < v_layer.size(); ilayer++) {
    ...
    for (int ipca = 1; ipca < npca; ipca++) {
      ...
      TCanvas* c = TFCSAnalyzerBase::PlotPolar(h, label.Data(), xlabel, ylabel,
                                               zlabel, 4);
      std::string outfile = outDir + "/" + "h_r_alpha_layer" +
                            std::to_string(layer) + "_pca" +
                            std::to_string(pca);
...
TH2F* TFCS2DParametrization::GetParametrization(TH1F* h_radius, int layer,
                                                int pca, std::string opt) {
  ...
  TH2F* h_r_alpha = new TH2F(Form("h_r_alpha_layer%i_pca%i", layer, pca),
                             Form("h_r_alpha_layer%i_pca%i", layer, pca),
                             nbinsx, xmin, xmax, nbinsy - 1, ybins);
...
std::vector<double> TFCS2DParametrization::CreateEqualEnergyBinning(TH1F* h,
                                                                    int nbins) {
...
TH1F* TFCS2DParametrization::GetHisto(std::string var, int layer, int pca,
                                      double cutoff, std::string opt) {
  ...
  TString varexp_mm(Form("%s>>hlmm%ipca%i(%i, %f, %f)", var.c_str(), layer, pca,
                         nbins, xmin, xmax_mm));
  TString cut(
      Form("hit_energy*scale_factor*(layer==%i && pca==%i)", layer, pca));
  ...
  TString exp(Form("%s>>h_r_l%ipca%i(%i, %f, %f)", "radius_mm", layer, pca,
                   nbinsr, xmin, rmax_mm));
  TString cut2 = "";
  ...
  //    std::cout << " bin = " << i << " up edge = " <<
  // h_radius->GetXaxis()->GetBinUpEdge(i) << " width = " <<
