...
void TFCSHitCellMappingWiggle::initialize(
    const std::vector<const TH1*> histograms, std::vector<float> bin_low_edges,
    float xscale) {
...
bool TFCSHitCellMappingWiggle::InitHisto(cl::sycl::context* ctx) {
  ...
  m_histo = new syclcommon::Histo(ctx);
  if (m_histo->Init()) {
    if (m_histo->LoadH1DF<TFCS1DFunction, TFCS1DFunctionInt32Histogram>(
            m_functions, m_bin_low_edge)) {
      return m_histo->LoadH1DFDevice();
    }
  }
  return false;
}
