...
void init_hit_to_cell_mapping(LateralShapeParametrizationArray &mapping,
                              bool isNewWiggle) {
  if (isNewWiggle) {
    ...
    init_hit_to_cell_mapping_with_wiggle(
        mapping, 1, {"eta_020_025", "eta_100_105"}, {0, 0.6, 100});
    init_hit_to_cell_mapping_with_wiggle(
        mapping, 2, {"eta_020_025", "eta_100_105"}, {0, 0.6, 100});
    init_hit_to_cell_mapping_with_wiggle(
        mapping, 3, {"eta_020_025", "eta_100_105"}, {0, 0.6, 100});

    init_hit_to_cell_mapping_with_wiggle(mapping, 5, {"eta_200_205"}, {0, 100});
    init_hit_to_cell_mapping_with_wiggle(
        mapping, 6, {"eta_200_205", "eta_280_285"}, {0, 2.5, 100});
    init_hit_to_cell_mapping_with_wiggle(
        mapping, 7, {"eta_200_205", "eta_280_285"}, {0, 2.5, 100});
  }
  ...
  for (int ilayer = 0; ilayer < 24; ++ilayer) {
    if (mapping[ilayer] == 0) {
      ...
      if (ilayer < 21) {
        mapping[ilayer] = new TFCSHitCellMapping(
            Form("hit_to_cell_mapping_%d", ilayer),
            Form("hit to cell mapping sampling %d", ilayer));
      } else {
        mapping[ilayer] = new TFCSHitCellMappingFCal(
            Form("hit_to_cell_mapping_%d", ilayer),
            Form("hit to cell mapping sampling %d", ilayer));
      }
...
void init_hit_to_cell_mapping_with_wiggle(
    LateralShapeParametrizationArray &mapping, int sampling,
    const std::vector<std::string> &etaRange,
    const std::vector<float> &etaLowEdge,
    const std::vector<float> &cellDphiHalve) {
  TFCSHitCellMappingWiggle *wigglefunc = new TFCSHitCellMappingWiggle(
      Form("hit_to_cell_mapping_wiggle_%d", sampling),
  ...
  for (unsigned int ifnc = 0; ifnc < etaRange.size(); ++ifnc) {
    std::string filename =
        TFCSSampleDiscovery::getWiggleName(etaRange[ifnc], sampling);
    ...
    if (wigglefile) {
      ...
      if (wigglehist) {
        std::cout << "Get wiggle histogram " << histname
                  << " from file:" << filename << std::endl;
        TFCS1DFunctionInt32Histogram *func =
            new TFCS1DFunctionInt32Histogram(wigglehist);
        float xscale = 1;
        if (cellDphiHalve.size() == etaRange.size())
          xscale = cellDphiHalve[ifnc];
        for (auto &ele : func->get_HistoBordersx()) ele *= xscale;
        functions[ifnc] = func;
      } else {
        std::cout << "Wiggle hist " << histname << " not found, do ls"
                  << std::endl;
#ifdef FCS_DEBUG
        wigglefile->ls();
#endif
...
void init_hit_to_cell_mapping_with_wiggle(
    LateralShapeParametrizationArray &mapping, int sampling, double rangeMin,
  ...
  for (int int_etaMin = int_rangemin; int_etaMin < int_rangemax;
       int_etaMin += 5) {

    int int_etaMax = int_etaMin + 5;
    double etaMin = int_etaMin / 100.0;
    std::string str_etarange =
        "eta_" + std::to_string(int_etaMin) + "_" + std::to_string(int_etaMax);
    etarange.push_back(str_etarange);
    vec_etamin.push_back(etaMin);

    if ((sampling == 5 or sampling == 6 or sampling == 7)and int_etaMin ==
        int_rangemin)
  ...
  for (unsigned int ifnc = 0; ifnc < etarange.size(); ++ifnc) {
    std::string filename =
        TFCSSampleDiscovery::getWiggleName(etarange[ifnc], sampling, true);
    functions[ifnc] = nullptr;
    auto wigglefile = std::unique_ptr<TFile>(TFile::Open(filename.c_str()));
    if (!wigglefile) {
      std::cerr << "Error: Could not open file '" << filename << "'"
    ...
    if (wigglefile) {
      ...
      if ((sampling == 6 or sampling == 7)and vec_etamin[ifnc] > 2.5) {
        std::cout << "In different graunalrity. sampling: " << sampling
                  << " eta: " << vec_etamin[ifnc] << std::endl;
        histname = Form("cs%d_above_etaboundary_efficiency_derivative_positive",
                        sampling);
      } else {
...
void init_numbers_of_hits(LateralShapeParametrizationArray &mapping,
                          float scale) {
  ...
  for (int ilayer = 0; ilayer < 8; ++ilayer) {
    if (mapping[ilayer] == 0) {
      mapping[ilayer] = new TFCSLateralShapeParametrizationHitNumberFromE(
          "numbers_of_hits_EM", "Calc numbers of hits EM", 0.101 * scale,
          0.002);
      mapping[ilayer]->set_calosample(ilayer);
    }
  }
  ...
  for (int ilayer = 8; ilayer < 12; ++ilayer) {
    if (mapping[ilayer] == 0) {
      ...
      mapping[ilayer] = new TFCSLateralShapeParametrizationHitNumberFromE(
          "numbers_of_hits_HEC", "Calc numbers of hits HEC", 0.762, 0.0);
      mapping[ilayer]->set_calosample(ilayer);
    }
  }
...
TFCSLateralShapeParametrizationHitBase *NewCenterPositionCalculation(
    std::string fileName, int pdgId, int intMom, double etaMin, double etaMax,
    int Ebin, int cs) {
  int int_etamin = TMath::Nint(100 * etaMin);
  int int_etamax = TMath::Nint(100 * etaMax);
  TString centerPosParam_name =
      Form("CenterPosParam_id%d_Mom%d_eta_%d_%d_Ebin%d_cs%d", pdgId, intMom,
           int_etamin, int_etamax, Ebin, cs);
  TString centerPosParam_title =
      Form("Center pos param for id=%d Mom=%d %4.2f<eta<%4.2f Ebin=%d cs=%d",
           pdgId, intMom, etaMin, etaMax, Ebin, cs);
  ...
  TMatrixT<float> *tm_mean_weight =
      (TMatrixT<float> *)extrapfile->Get("tm_mean_weight");

  TFCSCenterPositionCalculation *centerPosCalc =
      new TFCSCenterPositionCalculation(centerPosParam_name,
                                        centerPosParam_title);
...
TFCSLateralShapeParametrizationHitBase *NewHistoShapeParametrization(
    std::string fileName, int pdgId, int intMom, double etaMin, double etaMax,
    int Ebin, int cs) {
  ...
  int int_etaMin = TMath::Nint(100 * etaMin);
  int int_etaMax = TMath::Nint(100 * etaMax);
  TString shapepara_name = Form("Shape_id%d_Mom%d_eta_%d_%d_Ebin%d_cs%d", pdgId,
                                intMom, int_etaMin, int_etaMax, Ebin, cs);
  TString shapepara_title =
      Form("Shape param for id=%d Mom=%d %4.2f<eta<%4.2f Ebin=%d cs=%d", pdgId,
  ...
  if (!inhist) {
    TString shapehist2(
        Form("cs%d_pca%d_hist_hitenergy_alpha_radius_2D", cs, Ebin));
    inhist = (TH2 *)shapefile->Get(shapehist2);
    if (!inhist) {
      std::cout << "ERROR: neither histograms '" << shapehist << "' nor '"
                << shapehist2 << "' found in file " << fileName << std::endl;
  ...
  if (cs < 21) {
    shapeparam = new TFCSHistoLateralShapeParametrization(shapepara_name,
                                                          shapepara_title);
  } else {
    shapeparam = new TFCSHistoLateralShapeParametrizationFCal(shapepara_name,
                                                              shapepara_title);
  }
...
TFCSParametrizationEbinChain *NewShapeEbinCaloSampleChain(
    TFCSParametrizationBase *epara,
    const LateralShapeParametrizationArray &mapping,
    const LateralShapeParametrizationArray &numbersOfHits,
    std::string shapeFileName, int pdgId, int intMom, double etaMin,
    double etaMax) {
  int int_etaMin = TMath::Nint(100 * etaMin);
  int int_etaMax = TMath::Nint(100 * etaMax);

  TString para_name =
      Form("Param_id%d_Mom%d_eta_%d_%d", pdgId, intMom, int_etaMin, int_etaMax);
  TString para_title = Form("Param for id=%d Mom=%d %4.2f<eta<%4.2f", pdgId,
                            intMom, etaMin, etaMax);
  TFCSParametrizationEbinChain *para =
      new TFCSParametrizationEbinChain(para_name, para_title);

  std::string extrapolFileName = shapeFileName;
  size_t index = extrapolFileName.rfind(".shapepara.");
  if (index != std::string::npos)
    extrapolFileName.replace(index, 11, ".extrapol.");
  ...
  for (int iEbin = 0; iEbin <= max_nbin; ++iEbin) {
    ...
    for (int ilayer = 0; ilayer < 24; ++ilayer) {
      ...
      TFCSLateralShapeParametrizationHitBase *centerPosCalc =
          NewCenterPositionCalculation(extrapolFileName, pdgId, intMom, etaMin,
                                       etaMax, iEbin, ilayer);

      TFCSLateralShapeParametrizationHitBase *shape =
          NewHistoShapeParametrization(shapeFileName, pdgId, intMom, etaMin,
                                       etaMax, iEbin, ilayer);
      if (!shape) return nullptr;

      TString chain_name =
          Form("ShapeChain_id%d_Mom%d_eta_%d_%d_Ebin%d_cs%d", pdgId, intMom,
               int_etaMin, int_etaMax, iEbin, ilayer);
      TString chain_title =
          Form("Shape chain for id=%d Mom=%d %4.2f<eta<%4.2f Ebin=%d cs=%d",
               pdgId, intMom, etaMin, etaMax, iEbin, ilayer);
      ...
      if (centerPosCalc) {
        hitchain->push_back(centerPosCalc);
      }
      hitchain->push_back(shape);

      if (!numbersOfHits.empty()) {
...
TFCSParametrizationBase *NewEnergyChain(
  ...
  TString Ekinpara_name =
      Form("SelEkin_id%d_Mom%d_%d_eta_%d_%d", pdgid, int_Mom_min, int_Mom_max,
           int_etamin, int_etamax);
  TString Ekinpara_title =
      Form("Select Ekin for id=%d %d<=Mom<=%d %4.2f<=|eta|<%4.2f", pdgid,
  ...
  if (!Einterfile) {
    std::cerr << "================ interpolation ERROR ==============="
              << std::endl << "ERROR: pdgid =" << pdgid << " " << etamin
              << "<=eta<" << etamax << std::endl
              << "ERROR: Interpolation file=" << EinterfileName << std::endl
              << "ERROR: graph=" << graph << std::endl
              << "===================================================="
              << std::endl;
    std::cerr << "Error: Could not open file '" << EinterfileName << "'"
              << std::endl;
    gSystem->Exit(1);
  }

  std::cout << "================ interpolation ===============" << std::endl
            << "pdgid =" << pdgid << " " << etamin << "<=eta<" << etamax
            << std::endl << "Interpolation file=" << EinterfileName << std::endl
  ...
  for (int int_Mom = int_Mom_min; int_Mom <= int_Mom_max; int_Mom *= 2) {
    ...
    TFCSParametrization *para = NewParametrization(
        randEngine, mapping, numbersOfHits, Eparafilename, shapefilename, pdgid,
        int_Mom, etamin, etamax, addinit);
    ...
    if (para) {
      ...
      std::cout << "========== EkinSelectChain ==========" << std::endl
                << "pdgid=" << pdgid << " Mom=" << int_Mom << " " << etamin
                << "<=eta<" << etamax << std::endl << "Efile=" << Eparafilename
                << std::endl << "shapefile=" << shapefilename << std::endl
                << "=====================================" << std::endl;
    } else {
      std::cout << "============= ERROR =================" << std::endl
                << "ERROR: pdgid=" << pdgid << " Mom=" << int_Mom << " "
  ...
  if (gr) {
    TString Einterpara_name =
        Form("SplineInterpolEkin_id%d_Mom%d_%d_eta_%d_%d", pdgid, int_Mom_min,
             int_Mom_max, int_etamin, int_etamax);
    TString Einterpara_title = Form(
        "Spline interpolation Ekin for id=%d %d<=Mom<=%d %4.2f<=|eta|<%4.2f",
        pdgid, int_Mom_min, int_Mom_max, etamin, etamax);
    TFCSEnergyInterpolationSpline *Einterpol =
        new TFCSEnergyInterpolationSpline(Einterpara_name, Einterpara_title);
    Einterpol->set_pdgid_Ekin_eta(*EkinSelectChain);
    Einterpol->InitFromArrayInEkin(gr->GetN(), gr->GetX(), gr->GetY(), "b2e2",
                                   0, 0);
...
TFCSEnergyParametrization *NewPCAEnergyParametrization(
    CLHEP::HepRandomEngine *randEngine, std::string filename, int pdgid,
  ...
  int int_etamin = TMath::Nint(100 * etamin);
  int int_etamax = TMath::Nint(100 * etamax);
  TString epara_name = Form("Epara_id%d_Mom%d_eta_%d_%d", pdgid, int_Mom,
                            int_etamin, int_etamax);
  TString epara_title = Form("Energy param for id=%d Mom=%d %4.2f<eta<%4.2f",
                             pdgid, int_Mom, etamin, etamax);

  TFCSPCAEnergyParametrization *epara =
      new TFCSPCAEnergyParametrization(epara_name, epara_title);
  ...
  for (int itoy = 0; itoy < 100000; ++itoy) {
    ...
    if (itoy > 100 && meanEresponse.mean_error() / meanEresponse.mean() < 0.001)
      break;
  }

  std::cout << "Emean=" << meanEresponse.mean() << " +- "
            << meanEresponse.mean_error() << " (" << ntoy << " toys)"
            << std::endl;
...
TFCSParametrization *NewParametrization(
  ...
  TFCSEnergyParametrization *epara = NewPCAEnergyParametrization(
      randEngine, Eparafilename, pdgid, int_Mom, etamin, etamax);
  if (!epara) return nullptr;

  TFCSParametrizationEbinChain *para = FCS::NewShapeEbinCaloSampleChain(
      epara, mapping, numbersOfHits, shapefilename, pdgid, int_Mom, etamin,
      etamax);
  ...
  if (!eparafile) {
    std::cerr << "================ PCA probability ERROR ==============="
              << std::endl << "ERROR: pdgid =" << pdgid << " " << etamin
              << "<=eta<" << etamax << std::endl
              << "ERROR: pca probability file=" << Eparafilename << std::endl
              << "===================================================="
              << std::endl;
    std::cerr << "Error: Could not open file '" << Eparafilename << "'"
              << std::endl;
    gSystem->Exit(1);
  }
  ...
  std::cout << "================ PCA probability ===============" << std::endl
            << "pdgid =" << pdgid << " " << etamin << "<=eta<" << etamax
            << std::endl << "pca prob  file =" << Eparafilename << std::endl
            << "===============================================" << std::endl;

  TString ebinpara_name = Form("ParamEbin_id%d_Mom%d_eta_%d_%d", pdgid, int_Mom,
                               int_etamin, int_etamax);
  TString ebinpara_title = Form("Param Ebin for id=%d Mom=%d %4.2f<eta<%4.2f",
                                pdgid, int_Mom, etamin, etamax);
  TFCSEnergyBinParametrization *ebinpara =
      new TFCSEnergyBinParametrization(ebinpara_name, ebinpara_title);
  ebinpara->set_number_of_Ekin_bins(epara->n_bins());
  ebinpara->set_pdgid_Ekin_eta(*epara);
  ebinpara->set_Ekin_nominal(Ekin);
  ebinpara->load_pdgid_Ekin_bin_probability_from_file(pdgid, eparafile.get(),
                                                      "PCAbinprob");
  if (pdgid == 11) {
    ebinpara->load_pdgid_Ekin_bin_probability_from_file(-11, eparafile.get(),
                                                        "PCAbinprob");
  }
  if (pdgid == 211) {
    ebinpara->load_pdgid_Ekin_bin_probability_from_file(-211, eparafile.get(),
                                                        "PCAbinprob");
  ...
  if (addinit) {
    ...
    TString Einitpara_name = Form("LinearInterpolEkin_id%d_Mom%d_eta_%d_%d",
                                  pdgid, int_Mom, int_etamin, int_etamax);
    TString Einitpara_title =
        Form("Linear interpolation Ekin for id=%d Mom=%d %4.2f<eta<%4.2f",
             pdgid, int_Mom, etamin, etamax);
    TFCSEnergyInterpolationLinear *Einit =
        new TFCSEnergyInterpolationLinear(Einitpara_name, Einitpara_title);
...
TFCSParametrizationBase *NewParametrizationSimple(
    CLHEP::HepRandomEngine *randEngine,
    const FCS::LateralShapeParametrizationArray &mapping,
    const FCS::LateralShapeParametrizationArray &numbersOfHits,
    std::string Eparafilename, std::string shapefilename, int pdgid,
  ...
  TString para_name = Form("Param_id%d_Mom%d_eta_%d_%d", pdgid, int_Mom,
                           int_etamin, int_etamax);
  TString para_title = Form("Param for id=%d Mom=%d %4.2f<eta<%4.2f", pdgid,
                            int_Mom, etamin, etamax);
  TFCSParametrizationChain *para =
      new TFCSParametrizationChain(para_name, para_title);

  TFCSEnergyParametrization *epara = FCS::NewPCAEnergyParametrization(
      randEngine, Eparafilename, pdgid, int_Mom, etamin, etamax);
  ...
  for (int iEbin = 0; iEbin <= epara->n_bins(); ++iEbin) {
    ...
    for (int ilayer = 0; ilayer < 24; ++ilayer) {
      if (!epara->is_match_calosample(ilayer)) continue;
      TFCSLateralShapeParametrizationHitBase *shape =
          NewHistoShapeParametrization(shapefilename, pdgid, int_Mom, etamin,
                                       etamax, iEbin, ilayer);
      TString chain_name =
          Form("ShapeChain_id%d_Mom%d_eta_%d_%d_Ebin%d_cs%d", pdgid, int_Mom,
               int_etamin, int_etamax, iEbin, ilayer);
      TString chain_title =
          Form("Shape chain for id=%d Mom=%d %4.2f<eta<%4.2f Ebin=%d cs=%d",
               pdgid, int_Mom, etamin, etamax, iEbin, ilayer);
      TFCSLateralShapeParametrizationHitChain *hitchain =
          new TFCSLateralShapeParametrizationHitChain(shape);
