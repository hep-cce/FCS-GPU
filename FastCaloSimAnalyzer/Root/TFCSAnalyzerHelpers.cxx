    ...
    double rangeMax, const std::vector<float> &cellDphiHalve)
{
  ...
  for (int int_etaMin = int_rangemin; int_etaMin < int_rangemax;
    ...
    std::string str_etarange = "eta_" + std::to_string(int_etaMin) + "_"
                               + std::to_string(int_etaMax);
    etarange.push_back(str_etarange);
    vec_etamin.push_back(etaMin);

    if ((sampling == 5 or sampling == 6 or sampling == 7)
        and int_etaMin == int_rangemin)
...
NewCenterPositionCalculation(std::string fileName, int pdgId, int intMom,
                             double etaMin, double etaMax, int Ebin, int cs)
{
...
TFCSLateralShapeParametrizationHitBase *
NewHistoShapeParametrization(std::string fileName, int pdgId, int intMom,
                             double etaMin, double etaMax, int Ebin, int cs)
{
  ...
  TString shapepara_title
      = Form("Shape param for id=%d Mom=%d %4.2f<eta<%4.2f Ebin=%d cs=%d",
             pdgId, intMom, etaMin, etaMax, Ebin, cs);
  TString shapehist(Form("h_r_alpha_layer%d_pca%d", cs, Ebin));
...
// #endif
  ...
  if (pdgId == 11 || pdgId == 211)
    shapeparam->add_pdgid(-pdgId);
...
NewShapeEbinCaloSampleChain(TFCSParametrizationBase *epara,
                            ...
                            double etaMin, double etaMax)
{
  ...
  TString para_name = Form("Param_id%d_Mom%d_eta_%d_%d", pdgId, intMom,
  ...
  for (int iEbin = 0; iEbin <= max_nbin; ++iEbin) {
    if (!epara->is_match_Ekin_bin(iEbin))
      continue;
    for (int ilayer = 0; ilayer < 24; ++ilayer) {
      if (!epara->is_match_calosample(ilayer))
        continue;
               ...
               double etamin, double etamax)
{
  ...
  TString Ekinpara_name = Form("SelEkin_id%d_Mom%d_%d_eta_%d_%d", pdgid,
                               int_Mom_min, int_Mom_max, int_etamin, int_etamax);
  ...
  for (int int_Mom = int_Mom_min; int_Mom <= int_Mom_max; int_Mom *= 2) {
    ...
    if (para) {
      if (int_Mom == int_Mom_min)
        para->set_Ekin_min(0);
      if (int_Mom == int_Mom_max)
        para->set_Ekin_max(14000000);
      ...
      std::cout << "============= ERROR =================" << std::endl
                << "ERROR: pdgid=" << pdgid << " Mom=" << int_Mom << " " << etamin
                << "<=eta<" << etamax << std::endl
  ...
  if (gr) {
    ...
    Einterpol->InitFromArrayInEkin(gr->GetN(), gr->GetX(), gr->GetY(), "b2e2", 0,
                                   0);
...
NewPCAEnergyParametrization(CLHEP::HepRandomEngine *randEngine,
                            std::string filename,
                            int pdgid, int int_Mom,
                            double etamin, double etamax)
{
  ...
  if (!epara->loadInputs(eparafile.get(), ""))
    return nullptr;
  eparafile->Close();

  // if(epara->n_bins()==1) return nullptr;
  if (epara->get_layers().size() == 0)
    return nullptr;
  ...
  std::cout << "Emean=" << meanEresponse.mean()
            << " +- " << meanEresponse.mean_error() << " (" << ntoy << " toys)" << std::endl;
...
TFCSParametrization *
NewParametrization(CLHEP::HepRandomEngine *randEngine,
                   const FCS::LateralShapeParametrizationArray &mapping,
                   const FCS::LateralShapeParametrizationArray &numbersOfHits,
                   std::string Eparafilename, std::string shapefilename,
                   int pdgid, int int_Mom,
                   double etamin, double etamax,
                   bool addinit)
{
  ...
  for (int iEbin = 1; iEbin <= epara->n_bins(); ++iEbin)
    prob[iEbin] = 1;
...
TFCSParametrizationBase *
NewParametrizationSimple(CLHEP::HepRandomEngine *randEngine,
                         const FCS::LateralShapeParametrizationArray &mapping,
                         const FCS::LateralShapeParametrizationArray &numbersOfHits,
                         std::string Eparafilename, std::string shapefilename,
                         int pdgid, int int_Mom,
  ...
  for (int iEbin = 0; iEbin <= epara->n_bins(); ++iEbin) {
    ...
    for (int ilayer = 0; ilayer < 24; ++ilayer) {
      if (!epara->is_match_calosample(ilayer))
        continue;
      TFCSLateralShapeParametrizationHitBase *shape
          = NewHistoShapeParametrization(shapefilename, pdgid, int_Mom,
