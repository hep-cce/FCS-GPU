         ...
         for (unsigned int icell = 0; icell < ncells; icell++)
         {
            ...
            for (unsigned int ihit = 0; ihit < nhits;  ihit++)
            {
               ...
               if (m_debug == 3 && icell < 10  && ihit < 10)
               {
                  ...
                  std::cout << "eta_mm_v2 - eta_mm, phi_mm_v2 - phi_mm " << eta_mm_v2 - eta_mm << ", " << phi_mm_v2 - phi_mm << std::endl;
                  std::cout << "(eta_mm_v2 - eta_mm)/eta_mm, (phi_mm_v2 - phi_mm)/phi_mm " << (eta_mm_v2 - eta_mm) / eta_mm << ", " << (phi_mm_v2 - phi_mm) / phi_mm << std::endl;
               }
               ...
               // TBranches
               b_m_ievent = ievent;
               if (icell == 0 and ihit == 0 ) b_m_new_event = true;
               else b_m_new_event = false;
               if (ihit == 0 ) b_m_new_cell = true;
               else b_m_new_cell = false;
...
void TFCSFlatNtupleMaker::StudyHitMerging()
{
   ...
   for ( unsigned int ilayer = 0; ilayer < v_relLayers.size(); ilayer++)
   {
      ...
      TH2D *h2min = new TH2D(Form("hit_time_diff_min_layer%i", layer), Form("hit_time_diff_min_layer%i", layer),  3000, 0, 3000, 1000, 0, 10);
            ...
            if (nhits > 1)
            {
               for (unsigned int ihit = 0; ihit < nhits;  ihit++)
               {
                  ...
                  for ( unsigned int jhit = nhits - 1; jhit > ihit; jhit--)
                  {
                     ...
                     float diff2 = (vhit_i.X() - vhit_j.X()) * (vhit_i.X() - vhit_j.X()) + (vhit_i.Y() - vhit_j.Y()) * (vhit_i.Y() - vhit_j.Y()) + (vhit_i.Z() - vhit_j.Z()) * (vhit_i.Z() - vhit_j.Z());
