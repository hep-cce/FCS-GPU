...
TFCSFlatNtupleMaker::TFCSFlatNtupleMaker(TChain *chain, TString outputfilename,
...
void TFCSFlatNtupleMaker::LoopEvents() {
  ...
  auto f_out =
      std::unique_ptr<TFile>(TFile::Open(m_output.c_str(), "recreate"));
  if (!f_out) {
    std::cerr << "Error: Could not create file '" << m_output << "'"
              << std::endl;
    return;
  }
  ...
  for (unsigned int ilayer = 0; ilayer < v_relLayers.size(); ilayer++) {
    ...
    // bool isBarrel = (layer >= 0 && layer <= 3) || (layer >= 12 && layer <=
    // 20);
    ...
    for (int ievent = 0; ievent < nentries; ievent++) {
      ...
      if (!m_isNewSample) {
        ...
        TTC_eta = ((*m_TTC_entrance_eta).at(0).at(layer) +
                   (*m_TTC_back_eta).at(0).at(layer)) /
                  2;

        TTC_phi = ((*m_TTC_entrance_phi).at(0).at(layer) +
                   (*m_TTC_back_phi).at(0).at(layer)) /
                  2;
        TTC_r = ((*m_TTC_entrance_r).at(0).at(layer) +
                 (*m_TTC_back_r).at(0).at(layer)) /
                2;
        TTC_z = ((*m_TTC_entrance_z).at(0).at(layer) +
                 (*m_TTC_back_z).at(0).at(layer)) /
                2;
      }

      if (m_debug == 2 && ievent < 100)
        std::cout << " TTC eta, phi, z = " << TTC_eta << " , " << TTC_phi
      ...
      for (unsigned int icell = 0; icell < ncells; icell++) {
        ...
        for (unsigned int ihit = 0; ihit < nhits; ihit++) {
          ...
          float hit_time =
              m_cellVector->m_vector.at(icell).hit.at(ihit).hit_time;

          Long64_t cell_identifier =
              m_cellVector->m_vector.at(icell).hit.at(ihit).cell_identifier;
          Long64_t identifier =
              m_cellVector->m_vector.at(icell).hit.at(ihit).identifier;
          ...
          // std::tie(eta_mm, phi_mm) = GetUnitsmm(hit_eta, eta, phi, TTC_r,
          // TTC_z);
          std::tie(eta_mm, phi_mm) =
              GetUnitsmm(TTC_eta, eta, phi, TTC_r, TTC_z);

          if (m_debug == 3 && icell < 10 && ihit < 10) {
            float eta_mm_v2, phi_mm_v2;
            std::tie(eta_mm_v2, phi_mm_v2) =
                GetUnitsmm(hit_eta, eta, phi, TTC_r, TTC_z);
            std::cout << "sampling: " << sampling << std::endl;
            std::cout << "eta, phi " << eta << ", " << phi << std::endl;
            std::cout << "eta_mm, phi_mm " << eta_mm << ", " << phi_mm
                      << std::endl;
            std::cout << "eta_mm_v2, phi_mm_v2 " << eta_mm_v2 << ", "
                      << phi_mm_v2 << std::endl;
            std::cout << "eta_mm_v2 - eta_mm, phi_mm_v2 - phi_mm "
                      << eta_mm_v2 - eta_mm << ", " << phi_mm_v2 - phi_mm
                      << std::endl;
            std::cout
                << "(eta_mm_v2 - eta_mm)/eta_mm, (phi_mm_v2 - phi_mm)/phi_mm "
                << (eta_mm_v2 - eta_mm) / eta_mm << ", "
          ...
          if (m_debug == 2 && icell < 10 && ihit < 10) {
            ...
            std::cout << " r_mm, alpha_mm = " << r_mm << ", " << alpha_mm
                      << std::endl;
            std::cout << " d_phi/d_eta = " << phi / eta
                      << " d_phi_mm/d_eta_mm = " << phi_mm / eta_mm
                      << std::endl;
...
void TFCSFlatNtupleMaker::StudyHitMerging() {
  ...
  for (unsigned int ilayer = 0; ilayer < v_relLayers.size(); ilayer++) {
    ...
    TH1D *hmin = new TH1D(Form("hit_diff_min_layer%i", layer),
                          Form("hit_diff_min_layer%i", layer), 500, 0, 5);
    TH2D *h2min = new TH2D(Form("hit_time_diff_min_layer%i", layer),
                           Form("hit_time_diff_min_layer%i", layer), 3000, 0,
                           3000, 1000, 0, 10);
    ...
    TCanvas *canvas = new TCanvas(Form("c_layer%i", layer),
                                  Form("c_layer%i", layer), 800, 600);
      ...
      for (unsigned int icell = 0; icell < ncells; icell++) {
        ...
        if (nhits > 1) {
          for (unsigned int ihit = 0; ihit < nhits; ihit++) {
            ...
            float hit_time_i =
                m_cellVector->m_vector.at(icell).hit.at(ihit).hit_time * energy;

            Long64_t identifier_i =
                m_cellVector->m_vector.at(icell).hit.at(ihit).identifier;
            ...
            for (unsigned int jhit = nhits - 1; jhit > ihit; jhit--) {
              Long64_t identifier_j =
                  m_cellVector->m_vector.at(icell).hit.at(jhit).identifier;

              if (identifier_i != identifier_j) {
                // std::cout << "id_i, id_j = " << identifier_i << " , " <<
                // identifier_j << std::endl ;
                continue;
              }
              ...
              float energy =
                  m_cellVector->m_vector.at(icell).hit.at(jhit).hit_energy;

              energy = 1.;

              float hit_time_j =
                  m_cellVector->m_vector.at(icell).hit.at(jhit).hit_time *
                  energy;
              ...
              float diff2 =
                  (vhit_i.X() - vhit_j.X()) * (vhit_i.X() - vhit_j.X()) +
    ...
    canvas->SetLogx();
    canvas->SaveAs((dir + "/min_hit_merge_layer" + std::to_string(layer) +
                    ".png").c_str());
    fout->cd();
    hmin->Write();
    h2min->Write();
