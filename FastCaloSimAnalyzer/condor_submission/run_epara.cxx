...
void epara_validation_plots_fromtree(TString firstPCAfilename,
                                     TString firstPCA_Appfilename,
                                     TString valifilename,
                                     TString valiplotsname, int dsid,
                                     bool do_PCAclosure) {
  double GeV = 1000.0;
  ...
  for (unsigned int l = 0; l < layerNr.size(); l++) {
    TH1D* hist =
        new TH1D(Form("h_efrac_layer%i_g4", layerNr[l]),
                 Form("h_efrac_layer%i_g4", layerNr[l]), 200, -0.1, 1.1);
    hist->GetXaxis()->SetTitle(Form("E fraction in layer %i (%s)", layerNr[l],
                                    lname[layerNr[l]].c_str()));
    h_efrac_g4.push_back(hist);
    TH1D* hist2 =
        new TH1D(Form("h_efrac_layer%i_sim", layerNr[l]),
                 Form("h_efrac_layer%i_sim", layerNr[l]), 200, -0.1, 1.1);
    h_efrac_sim.push_back(hist2);
  }
  double emin = tree_g4->GetMinimum("energy_totalE");
  emin *= 1.2;
  double emax = tree_g4->GetMaximum("energy_totalE");
  emax *= 1.05;
  TH1D* h_totalE_g4 =
      new TH1D("h_totalE_g4", "h_totalE_g4", 200, emin / GeV, emax / GeV);
  h_totalE_g4->GetXaxis()->SetTitle("total energy [GeV]");
  TH1D* h_totalE_sim =
      new TH1D("h_totalE_sim", "h_totalE_sim", 200, emin / GeV, emax / GeV);
  ...
  if (pcmax < 5) pcmax = 5;
  for (unsigned int l = 0; l <= layerNr.size(); l++) {
    TH1D* hist = new TH1D(Form("h_PC%i_g4", l), Form("h_PC%i_g4", l), 200,
                          -pcmax, pcmax);
    hist->GetXaxis()->SetTitle(Form("PC %i", l));
    h_PC_g4.push_back(hist);
    TH1D* hist2 = new TH1D(Form("h_PC%i_sim", l), Form("h_PC%i_sim", l), 200,
                           -pcmax, pcmax);
  ...
  for (int event = 0; event < read_tree_g4->GetEntries(); event++) {
    ...
    for (unsigned int l = 0; l < layerNr.size(); l++)
      h_efrac_g4[l]
          ->Fill(read_tree_g4->GetVariable(Form("energy_layer%i", layerNr[l])));
  }
  ...
  for (int event = 0; event < read_tree_sim->GetEntries(); event++) {
    ...
    for (unsigned int l = 0; l < layerNr.size(); l++)
      h_efrac_sim[l]->Fill(read_tree_sim->GetVariable(
          Form("simulated_efrac_layer%i", layerNr[l])));
    for (unsigned int l = 0; l <= layerNr.size(); l++)
      h_PC_sim[l]
          ->Fill(read_tree_sim->GetVariable(Form("simulated_PCA_comp%i", l)));
    if (read_tree_sim->GetVariable("randombin") ==
        read_tree_sim->GetVariable("simulated_PCAbin"))
      count_match++;
  ...
  h_PC_g4[0]->GetXaxis()->SetTitle(
      Form("Matched bins: %.1f%%   %s",
           (double)count_match / (double)read_tree_sim->GetEntries() * 100.0,
           title.c_str()));
  ...
  for (unsigned int i = 0; i < h_sim.size(); i++) {
    ...
    if (h_sim[i]->GetBinContent(h_sim[i]->GetMaximumBin()) > ymax)
      ymax = h_sim[i]->GetBinContent(h_sim[i]->GetMaximumBin());
    ymax *= 1.2;
...
void epara_validation_plots(TString firstPCAfilename, TString valifilename,
                            TString valiplotsname, int dsid,
                            bool do_PCAclosure) {
  ...
  for (unsigned int l = 0; l < layer.size(); l++) {
    ...
    if (use_autozoom) {
      h_output_lin =
          (TH1D*)file->Get(Form("h_output_zoom_%s", layer[l].c_str()));
      h_output_lin->SetName("h_output_lin");
      h_input_lin = (TH1D*)file->Get(Form("h_input_zoom_%s", layer[l].c_str()));
      h_input_lin->SetName("h_input_lin");
    } else {
      h_output_lin = (TH1D*)file->Get(Form("h_output_%s", layer[l].c_str()));
      h_output_lin->SetName("h_output_lin");
      h_input_lin = (TH1D*)file->Get(Form("h_input_%s", layer[l].c_str()));
      h_input_lin->SetName("h_input_lin");
    }
    ...
    // cumulative:
    TH1D* h_output_cumul = (TH1D*)get_cumul(h_output_lin);
    h_output_cumul->SetName("h_output_cumul");
    TH1D* h_input_cumul = (TH1D*)get_cumul(h_input_lin);
    h_input_cumul->SetName("h_input_cumul");
  ...
  if (do_PCAclosure) {
    ...
    if (h_comp0_closure->GetBinContent(h_comp0_closure->GetMaximumBin()) > max)
      max = h_comp0_closure->GetBinContent(h_comp0_closure->GetMaximumBin());
...
void run_epara(int dsid = 431004,
               std::string sampleData = "../python/inputSampleList.txt",
               std::string topDir = "./output_epara/", int npca1 = 5,
               int npca2 = 1, bool run_validation = true,
               std::string version = "ver01",
               std::string topPlotDir = "output_plot_epara/", long seed = 42) {

  // bool run_validation = true; // this will directly create the standard
  // validation plots
  bool do_PCAclosure =
      true;  // this will rerun the simulated events through the 1st PCA
  ...
  TString pca1Filename(Form("%s%s.firstPCA.%s.root", topDir.c_str(),
                            baselabel.c_str(), version.c_str()));
  TString pca1AppFilename(Form("%s%s.firstPCA_App.%s.root", topDir.c_str(),
                               baselabel.c_str(), version.c_str()));
  TString pca2Filename(Form("%s%s.secondPCA.%s.root", topDir.c_str(),
                            baselabel.c_str(), version.c_str()));
  TString valiFilename(Form("%s%s.eparavalidation.%s.root", topDir.c_str(),
                            baselabel.c_str(), version.c_str()));
  ...
  TFCSMakeFirstPCA* myfirstPCA =
      new TFCSMakeFirstPCA(mychain, pca1Filename.Data());
  ...
  TFCSApplyFirstPCA* myfirstPCA_App =
      new TFCSApplyFirstPCA(pca1Filename.Data());
  ...
  secondPCA* mysecondPCA =
      new secondPCA(pca1AppFilename.Data(), pca2Filename.Data());
  ...
  mysecondPCA->set_skip_regression(
      1);  // if set to 1 the regression is switched off
  ...
  if (run_validation) {
    ...
    for (unsigned int l = 0; l < layerNr.size(); l++) {
      h_output_fine[l] =
          new TH1D(Form("h_output_%s", layer[l].c_str()),
                   Form("h_output_%s", layer[l].c_str()), 5000, 0., 1.);
    }
    ...
    h_output_fine[layerNr.size()] =
        new TH1D("h_output_totalE", "h_output_totalE", 5000, minE, maxE);
    ...
    cout << "number of pca bins after load " << etest.n_pcabins()
         << " , number from firstPCA file: " << pcabins << endl;
    ...
    float ptot, p;
    ptot = p = 0.0;
    for (int i = 0; i <= pcabins; i++) ptot += (float)prob[i];
    ...
    for (int i = 0; i <= pcabins; i++) {
      cout << "bin " << i << " probability: " << prob[i]
           << " ordered: " << prob_ordered[i] << endl;
    }
    ...
    for (unsigned int l = 0; l < layerNr.size(); l++)
      tree_sim->Branch(Form("simulated_efrac_layer%i", layerNr[l]),
                       &simulated_efrac[l],
                       Form("simulated_efrac_layer%i/D", layerNr[l]));
    for (unsigned int l = 0; l < layer.size(); l++)
      tree_sim->Branch(Form("simulated_PCA_comp%i", l), &simulated_PCA[l],
                       Form("simulated_PCA_comp%i/D", l));
    ...
    for (int i = 0; i < ntoys; i++) {
      ...
      randombin =
          TMath::BinarySearch(pcabins + 1, prob_ordered, searchRand) + 1;
    ...
    if (do_PCAclosure) {
      ...
      for (unsigned int l = 0; l < layer.size(); l++) {
        TH1D* h_cumul = (TH1D*)h_output_fine[l]
                            ->Clone(Form("h_cumul_%s", layer[l].c_str()));
        for (int b = 1; b <= h_cumul->GetNbinsX(); b++) {
      ...
      TFCSEnergyParametrizationPCABinCalculator PCABinCalculator(
          closurefirstPCA_App, "test", "test");
      ...
      for (unsigned int i = 0; i < simulstates.size(); i++) {
        ...
        vector<double> PCAdata =
            closurefirstPCA_App.get_PCAdata_from_simstate(simulstates[i]);
        for (unsigned int l = 0; l < layer.size(); l++)
    ...
    epara_validation_plots_fromtree(pca1Filename, pca1AppFilename, valiFilename,
                                    valiPlotsFilename, dsid, do_PCAclosure);

    for (unsigned int l = 0; l <= layerNr.size(); l++) delete h_output_fine[l];
    file1->Close();
