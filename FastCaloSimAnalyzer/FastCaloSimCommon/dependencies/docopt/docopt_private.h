...
using UniquePatternSet = std::unordered_set<
    std::shared_ptr<Pattern>, PatternHasher, PatternPointerEquality>;
...
inline std::pair<size_t, std::shared_ptr<LeafPattern>> Argument::single_match(
    PatternList const& left) const {
...
inline std::pair<size_t, std::shared_ptr<LeafPattern>> Command::single_match(
    PatternList const& left) const {
  std::pair<size_t, std::shared_ptr<LeafPattern>> ret{};
...
inline Option Option::parse(std::string const& option_description) {
  ...
  for (std::sregex_iterator
           i{option_description.begin(), options_end,
             pattern,                    std::regex_constants::match_not_null},
       e{};
       i != e; ++i) {
    ...
    if (match[3].length() == 0) {  // [3] always matches.
      ...
      // empty
      // at the end, and then we'll spin in an infinite loop. So, if we hit an
      // empty
  ...
  return {std::move(shortOption), std::move(longOption),
          argcount,               std::move(val)};
}

inline std::pair<size_t, std::shared_ptr<LeafPattern>> Option::single_match(
    PatternList const& left) const {
  auto thematch = find_if(left.begin(), left.end(),
                          [this](std::shared_ptr<Pattern> const& a) {
...
inline bool Either::match(PatternList& left,
                          std::vector<std::shared_ptr<LeafPattern>>& collected)
    const {
  using Outcome =
