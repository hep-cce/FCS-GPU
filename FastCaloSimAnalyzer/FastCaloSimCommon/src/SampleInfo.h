...
namespace FCS {
  ...
  struct DSIDInfo {
    DSIDInfo( int dsid_ = -1, int pdgId_ = -1, int energy_ = -1, float eta_ = -1, int zVertex_ = -1 )
        : dsid( dsid_ ), pdgId( pdgId_ ), energy( energy_ ), eta( eta_ ), zVertex( zVertex_ ) {}
  ...
  struct SampleInfo {
    SampleInfo( int dsid_ = -1, std::string location_ = "", std::string label_ = "", int pdgId_ = -1, int energy_ = -1,
