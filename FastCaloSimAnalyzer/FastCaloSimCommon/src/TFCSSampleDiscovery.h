...
public:
  TFCSSampleDiscovery(std::string fileName = "DSID_DB.txt",
                      bool debug = false);
  ~TFCSSampleDiscovery() = default;

  const FCS::DSIDInfo &findDSID(int pdgId, int energy,
                                float eta, int zVertex) const;

  int getEnergy( int dsid ) const;
  int getPdgId( int dsid ) const;

  FCS::SampleInfo findSample(int inDSID,
                             const std::string &fileName = "InputSamplesList.txt") const;

  std::string        getBaseName( int dsid ) const;
  std::string getFirstPCAAppName(int dsid,
                                 const std::string &basedir = FCS::BASEDIR_FIRSTPCA,
  ...
  std::string getEinterpolMeanName(int pdgId,
                                   const std::string &basedir = FCS::BASEDIR_INTERPOLATION,
  ...
  static std::string getWiggleName(const std::string &etaRange,
                                   int sampling,
