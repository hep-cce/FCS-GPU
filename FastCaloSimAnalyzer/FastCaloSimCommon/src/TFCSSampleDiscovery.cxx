...
TFCSSampleDiscovery::TFCSSampleDiscovery( std::string fileName, bool debug ) : m_invalid( FCS::DSIDInfo( -1 ) ) {
  ...
  if ( debug ) {
    for ( const FCS::DSIDInfo& info : m_dbDSID ) { std::cout << info << std::endl; }
  }

  std::cout << "DB ready" << std::endl;
}

const FCS::DSIDInfo& TFCSSampleDiscovery::findDSID( int pdgId, int energy, float eta, int zVertex ) const {
  for ( const FCS::DSIDInfo& info : m_dbDSID ) {
    if ( pdgId == info.pdgId && energy == info.energy && eta == info.eta && zVertex == info.zVertex ) { return info; }
  }
...
int TFCSSampleDiscovery::getEnergy( int dsid ) const {
  for ( const FCS::DSIDInfo& info : m_dbDSID ) {
    if ( info.dsid == dsid ) { return info.energy; }
  }
...
int TFCSSampleDiscovery::getPdgId( int dsid ) const {
  for ( const FCS::DSIDInfo& info : m_dbDSID ) {
    if ( info.dsid == dsid ) { return info.pdgId; }
  }
...
FCS::SampleInfo TFCSSampleDiscovery::findSample( int inDSID, const std::string& fileName ) const {
  ...
  while ( !file.eof() ) {
    file >> dsid >> location >> label >> pdgId >> energy >> etaMin >> etaMax >> zVertex;

    if ( dsid == inDSID ) {
      return FCS::SampleInfo( dsid, prefixLocation( FCS::BASEDIR_INPUTS + location ), label, pdgId, energy, etaMin,
                              etaMax, zVertex );
    }
  }
...
std::string TFCSSampleDiscovery::getBaseName( int dsid ) const {
  for ( const FCS::DSIDInfo& info : m_dbDSID ) {
    if ( info.dsid == dsid ) {
      std::stringstream s;
      s << "mc16_13TeV." << dsid << ".ParticleGun_pid" << info.pdgId << "_E" << info.energy << "_disj_eta_m"
        << ( info.eta + 5 ) << "_m" << info.eta << "_" << info.eta << "_" << ( info.eta + 5 ) << "_zv_" << info.zVertex;
      return s.str();
    }
  }
...
std::string TFCSSampleDiscovery::getFirstPCAAppName( int dsid, const std::string& basedir,
                                                     const std::string& version ) const {
  return getName( dsid, "firstPCA_App", basedir, version );
}

std::string TFCSSampleDiscovery::getSecondPCAName( int dsid, const std::string& basedir,
                                                   const std::string& version ) const {
  return getName( dsid, "secondPCA", basedir, version );
}

std::string TFCSSampleDiscovery::getShapeName( int dsid, const std::string& basedir,
                                               const std::string& version ) const {
  return getName( dsid, "shapepara", basedir, version );
}

std::string TFCSSampleDiscovery::getAvgSimShapeName( int dsid, const std::string& basedir,
                                                     const std::string& version ) const {
  return getName( dsid, "AvgSimShape", basedir, version );
}

std::string TFCSSampleDiscovery::getEinterpolMeanName( int pdgId, const std::string& basedir,
                                                       const std::string& version ) const {
  return prefixLocation( basedir ) + "mc16_13TeV.pid" + std::to_string( pdgId ) + ".EinterpolMean." + version + ".root";
}

std::string TFCSSampleDiscovery::getParametrizationName( const std::string& basedir, const std::string& version ) {
  return prefixLocation( basedir ) + "TFCSparam_" + version + ".root";
}

std::string TFCSSampleDiscovery::getWiggleName( const std::string& etaRange, int sampling, bool isNewWiggle,
                                                const std::string& basedir, const std::string& version ) {
  if ( isNewWiggle ) {
    return prefixLocation( basedir ) + "Wiggle/" + etaRange + "." + version + ".root";
  } else {
    return prefixLocation( basedir ) + "Wiggle_old/" + etaRange + "/wiggle_input_deriv_Sampling_" +
           std::to_string( sampling ) + ".ver03.root";
  }
}

std::string TFCSSampleDiscovery::geometryTree() { return "ATLAS-R2-2016-01-00-01"; }
...
std::string TFCSSampleDiscovery::getName( int dsid, const std::string& label, const std::string& basedir,
                                          const std::string& version ) const {
