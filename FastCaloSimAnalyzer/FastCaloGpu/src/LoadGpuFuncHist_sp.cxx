...
void LoadGpuFuncHist::LD2D() {
  ...
  std::memcpy(hf.h_bordersx, (*m_hf2d).h_bordersx,
              (hf.nbinsx + 1) * sizeof(float));
  std::memcpy(hf.h_bordersy, (*m_hf2d).h_bordersy,
              (hf.nbinsy + 1) * sizeof(float));
  std::memcpy(hf.h_contents, (*m_hf2d).h_contents,
...
void LoadGpuFuncHist::LD() {
  ...
  std::memcpy(hf_ptr->low_edge, (*m_hf).low_edge,
              (hf_ptr->nhist + 1) * sizeof(float));

  hf_ptr->h_szs = new unsigned int[hf_ptr->nhist];
  std::memcpy(hf_ptr->h_szs, (*m_hf).h_szs,
              hf_ptr->nhist * sizeof(unsigned int));
  ...
  uint32_t **contents_ptr =
      (uint32_t **)malloc(hf_ptr->nhist * sizeof(uint32_t *));
  ...
  for (unsigned int i = 0; i < hf_ptr->nhist; ++i) {
    ...
    std::memcpy(contents_ptr[i], (*m_hf).h_contents[i],
                h_szs[i] * sizeof(uint32_t));
    std::memcpy(borders_ptr[i], (*m_hf).h_borders[i],
                (h_szs[i] + 1) * sizeof(float));
  }

  std::memcpy(hf_ptr->h_contents, contents_ptr,
              hf_ptr->nhist * sizeof(uint32_t *));
  ...
  // hf_ptr->d_contents1D = (uint32_t*) malloc(hf_ptr->nhist * hf_ptr->mxsz *
  // sizeof(uint32_t));
  // hf_ptr->d_borders1D  = (float*)    malloc(hf_ptr->nhist * hf_ptr->mxsz *
  // sizeof(float));
