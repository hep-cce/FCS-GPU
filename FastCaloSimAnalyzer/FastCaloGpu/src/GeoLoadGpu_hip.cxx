...
bool GeoLoadGpu::TestGeo() {
  ...
  hipLaunchKernelGGL(testGeo_g, 1, 1, 0, 0,  m_geo_d, 14, 0, 32 );
  gpuQ( hipDeviceSynchronize() );
  err = hipGetLastError();
  if ( err != hipSuccess ) {
    std::cout << hipGetErrorString( err ) << std::endl;
...
bool GeoLoadGpu::LoadGpu_cu() {
  ...
  if ( hipSuccess != hipMalloc( (void**)&m_cells_d, sizeof( CaloDetDescrElement ) * m_ncells ) ) return false;
  ...
  if ( hipSuccess !=
       hipMemcpy( &m_cells_d[0], cells_Host, sizeof( CaloDetDescrElement ) * m_ncells, hipMemcpyHostToDevice ) )
    return false;
