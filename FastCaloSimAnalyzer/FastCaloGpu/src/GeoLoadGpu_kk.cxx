...
bool GeoLoadGpu::LoadGpu_kk() {
  ...
  Kokkos::View<CaloDetDescrElement *>::HostMirror cells_hv =
      Kokkos::create_mirror_view(m_cells_vd);
  ...
  m_sample_index_vd =
      Kokkos::View<Rg_Sample_Index *>("sample index", m_max_sample);
  Kokkos::View<Rg_Sample_Index *, Kokkos::HostSpace> hostSampleIndex(
      m_sample_index_h, m_max_sample);
  ...
  for (unsigned int ir = 0; ir < m_nregions; ++ir) {
    Kokkos::View<long long *> *ptr_gv = new Kokkos::View<long long *>(
        "region grid",
    ...
    Kokkos::View<long long *, Kokkos::HostSpace> ptr_h(
        m_regions[ir].cell_grid(),
        m_regions[ir].cell_grid_eta() * m_regions[ir].cell_grid_phi());
  ...
  Kokkos::View<GeoGpu>::HostMirror geo_gpu_hv =
      Kokkos::create_mirror_view(m_gptr_v);
