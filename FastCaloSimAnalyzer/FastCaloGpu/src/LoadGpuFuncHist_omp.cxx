...
LoadGpuFuncHist::LoadGpuFuncHist() {}
  ...
  LoadGpuFuncHist::~LoadGpuFuncHist() {
  free( m_hf );
  omp_target_free ( ( *m_hf_h ).low_edge,     m_default_device ); 
  omp_target_free ( ( *m_hf_h ).h_szs,        m_default_device ); 
  omp_target_free ( ( *m_hf_h ).h_contents,   m_default_device ); 
  omp_target_free ( ( *m_hf_h ).h_borders,    m_default_device ); 
  omp_target_free ( ( *m_hf_h ).d_contents1D, m_default_device ); 
  omp_target_free ( ( *m_hf_h ).d_borders1D,  m_default_device ); 
  free( m_hf_h );
  omp_target_free ( m_hf_d,  m_default_device ); 

  free( m_hf2d );
  omp_target_free ( ( *m_hf2d_h ).h_bordersx,  m_default_device ); 
  omp_target_free ( ( *m_hf2d_h ).h_bordersy,  m_default_device ); 
  omp_target_free ( ( *m_hf2d_h ).h_contents,  m_default_device ); 
  free( m_hf2d_h );
  omp_target_free ( m_hf2d_d,  m_default_device ); 
}
...
void LoadGpuFuncHist::LD2D() {
  ...
  // std::cout << "(*m_hf2d).h_bordersy, pointer " <<  (*m_hf2d).h_bordersy   << std::endl ;
    ...
    hf.h_bordersx = (float *) omp_target_alloc( ( hf.nbinsx + 1 ) * sizeof( float ), m_default_device );
    hf.h_bordersy = (float *) omp_target_alloc( ( hf.nbinsy + 1 ) * sizeof( float ), m_default_device );
    hf.h_contents = (float *) omp_target_alloc( ( hf.nbinsy * hf.nbinsx ) * sizeof( float ), m_default_device );
...
//  //  std::cout << "hf.h_bordersy, pointer " <<  hf.h_bordersy  <<  std::endl ;
    ...
    *( hf_ptr ) = hf;
    ...
    m_hf2d_d = (FH2D *) omp_target_alloc( sizeof( FH2D ), m_default_device );
    if ( m_hf2d_d == NULL ) {
      std::cout << " ERROR: No space left on device." << std::endl;;
    }
    if ( omp_target_memcpy( m_hf2d_d, m_hf2d_h, sizeof( FH2D ), m_offset, m_offset, m_default_device, m_initial_device ) ) {
...
void LoadGpuFuncHist::LD() {
  ...
  unsigned int* h_szs = ( *m_hf ).h_szs; // already allocateded on host ;
    ...
    hf.low_edge = (float *) omp_target_alloc( ( hf.nhist + 1 ) * sizeof( float ), m_default_device );
    if ( hf.low_edge == NULL ) {
      std::cout << " ERROR: No space left on device." << std::endl;;
      //return false;
    }
    ...
    hf.h_szs = (unsigned int *) omp_target_alloc(  hf.nhist * sizeof( unsigned int ), m_default_device );
    if ( hf.h_szs == NULL ) {
      std::cout << " ERROR: No space left on device." << std::endl;;
      //return false;
    }
    ...
    hf.d_contents1D = (uint32_t *) omp_target_alloc( hf.nhist * hf.mxsz * sizeof( uint32_t ), m_default_device );
    hf.d_borders1D  =    (float *) omp_target_alloc( hf.nhist * hf.mxsz * sizeof( float ), m_default_device );
  ...
  *( hf_ptr ) = hf;
  m_hf_h      = hf_ptr;
    ...
    m_hf_d = (FHs *) omp_target_alloc( sizeof( FHs ), m_default_device );
    if ( m_hf_d == NULL ) {
      std::cout << " ERROR: No space left on device." << std::endl;;
      //return false;
    }
    if ( omp_target_memcpy( m_hf_d, m_hf_h, sizeof( FHs ), m_offset, m_offset, m_default_device, m_initial_device ) ) {
      std::cout << "ERROR: copy hf.h_szs. " << std::endl;
      //std::exit(1);
    }
}
