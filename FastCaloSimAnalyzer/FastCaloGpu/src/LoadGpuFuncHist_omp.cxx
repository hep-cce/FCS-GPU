...
void LoadGpuFuncHist::LD2D() {
  ...
  // std::cout << ".....Loading  2DFnctionHists, Size of hist" <<  hf.nbinsx  <<
  // "x" << hf.nbinsy << std::endl ; std::cout << "(*m_hf2d).h_bordersy, pointer
  // " <<  (*m_hf2d).h_bordersy   << std::endl ;

  hf.h_bordersx = (float *)omp_target_alloc((hf.nbinsx + 1) * sizeof(float),
                                            m_select_device);
  hf.h_bordersy = (float *)omp_target_alloc((hf.nbinsy + 1) * sizeof(float),
                                            m_select_device);
  hf.h_contents = (float *)omp_target_alloc(
      (hf.nbinsy * hf.nbinsx) * sizeof(float), m_select_device);
  ...
  //  ;
  if (omp_target_memcpy(hf.h_bordersx, (*m_hf2d).h_bordersx,
                        (hf.nbinsx + 1) * sizeof(float), m_offset, m_offset,
                        m_select_device, m_initial_device)) {
    std::cout << "ERROR: copy hf.nbinsx. " << std::endl;
    // std::exit(1);
  }
  if (omp_target_memcpy(hf.h_bordersy, (*m_hf2d).h_bordersy,
                        (hf.nbinsy + 1) * sizeof(float), m_offset, m_offset,
                        m_select_device, m_initial_device)) {
    std::cout << "ERROR: copy hf.nbinsy. " << std::endl;
    // std::exit(1);
  }
  if (omp_target_memcpy(hf.h_contents, (*m_hf2d).h_contents,
                        (hf.nbinsx * hf.nbinsy) * sizeof(float), m_offset,
  ...
  *(hf_ptr) = hf;
  ...
  if (omp_target_memcpy(m_hf2d_d, m_hf2d_h, sizeof(FH2D), m_offset, m_offset,
                        m_select_device, m_initial_device)) {
...
void LoadGpuFuncHist::LD() {
  ...
  hf.low_edge = (float *)omp_target_alloc((hf.nhist + 1) * sizeof(float),
                                          m_select_device);
  if (hf.low_edge == NULL) {
    std::cout << " ERROR: No space left on device." << std::endl;
    ;
  ...
  if (omp_target_memcpy(hf.low_edge, (*m_hf).low_edge,
                        (hf.nhist + 1) * sizeof(float), m_offset, m_offset,
                        m_select_device, m_initial_device)) {
    std::cout << "ERROR: copy hf.low_edge. " << std::endl;
    // std::exit(1);
  }

  hf.h_szs = (unsigned int *)omp_target_alloc(hf.nhist * sizeof(unsigned int),
                                              m_select_device);
  if (hf.h_szs == NULL) {
    std::cout << " ERROR: No space left on device." << std::endl;
    ;
  ...
  if (omp_target_memcpy(hf.h_szs, (*m_hf).h_szs,
                        hf.nhist * sizeof(unsigned int), m_offset, m_offset,
                        m_select_device, m_initial_device)) {
    std::cout << "ERROR: copy hf.h_szs. " << std::endl;
    // std::exit(1);
  }

  hf.d_contents1D = (uint32_t *)omp_target_alloc(
      hf.nhist * hf.mxsz * sizeof(uint32_t), m_select_device);
  hf.d_borders1D = (float *)omp_target_alloc(hf.nhist * hf.mxsz * sizeof(float),
                                             m_select_device);
  ...
  for (size_t i = 0; i < hf.nhist; ++i) {
    if (omp_target_memcpy(&(hf.d_contents1D[i * hf.mxsz]),
                          (*m_hf).h_contents[i], h_szs[i] * sizeof(uint32_t),
                          m_offset, m_offset, m_select_device,
                          m_initial_device)) {
      std::cout << "ERROR: copy hf.d_contents1D. " << std::endl;
      // std::exit(1);
    }
    if (omp_target_memcpy(&(hf.d_borders1D[i * hf.mxsz]), (*m_hf).h_borders[i],
                          h_szs[i] * sizeof(float), m_offset, m_offset,
                          m_select_device, m_initial_device)) {
  ...
  *(hf_ptr) = hf;
  ...
  if (omp_target_memcpy(m_hf_d, m_hf_h, sizeof(FHs), m_offset, m_offset,
                        m_select_device, m_initial_device)) {
    std::cout << "ERROR: copy hf.h_szs. " << std::endl;
    // std::exit(1);
  }
