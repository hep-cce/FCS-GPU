...
void test_atomicAdd_int() {
  ...
  std::cout << "   after loop: " << *ii << " (should be " << N << ")"
            << std::endl;
...
void simulate_clean(Sim_Args args) {
  nvtxRangeId_t r;
  if (!first)
    r = nvtxRangeStartA("sim_clean");

  // std::cout << "args.nsims: " << args.nsims << " args.ncells: " <<
  // args.ncells
  //           << "  MAX_SIM: "<< MAX_SIM
  //           << " ct: " << (void*)args.ct
  ...
  std::for_each_n(std::execution::par_unseq, counting_iterator(0),
                  args.ncells * args.nsims, [=](unsigned int tid) {
    args.cells_energy[tid] = 0;
    if (tid < args.nsims)
      args.ct[tid] = 0; // faster than separate kernel
  });
  // std::for_each_n(std::execution::par_unseq, counting_iterator(0),
  // args.nsims,
  //                 [=](unsigned int tid) {
...
void simulate_hits_de(const Sim_Args args) {
  ...
  std::for_each_n(std::execution::par_unseq, counting_iterator(0), args.nhits,
    ...
    if (hp.cmw)
      HitCellMappingWiggle_g_d(hp, hit, i, args);
    HitCellMapping_g_d(hp, hit, i, args);
  });
  // int j = *ii;

  // if (j != args.nhits) {
  //   std::cout << "ERROR: loop not executed fully in simulate_hits_de.
  // expected "
