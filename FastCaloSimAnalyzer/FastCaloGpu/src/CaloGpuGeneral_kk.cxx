...
KOKKOS_INLINE_FUNCTION int find_index_f(Kokkos::View<float *> array, int size,
                                        float value) {
...
KOKKOS_INLINE_FUNCTION void rnd_to_fct2d(float &valuex, float &valuey,
  ...
  float basecont = 0;
  if (ibin > 0)
    basecont = fh2d_v.contents(ibin - 1);

  float dcont = fh2d_v.contents(ibin) - basecont;
  if (dcont > 0) {
    valuex = fh2d_v.bordersx(binx) +
             (fh2d_v.bordersx(binx + 1) - fh2d_v.bordersx(binx)) *
                 (rnd0 - basecont) / dcont;
  } else {
    valuex = fh2d_v.bordersx(binx) +
             (fh2d_v.bordersx(binx + 1) - fh2d_v.bordersx(binx)) / 2;
  }
  valuey = fh2d_v.bordersy(biny) +
           (fh2d_v.bordersy(biny + 1) - fh2d_v.bordersy(biny)) * rnd1;
}

/* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * */

KOKKOS_INLINE_FUNCTION void HistoLateralShapeParametrization_g_d(
    const HitParams hp, Hit &hit, unsigned long tid, const Sim_Args &args,
    const FH2D_v &fh2d_v, Kokkos::View<float *> rand_v) {
  ...
  //    printf ("%f %f %f %f %f %f\n", center_eta, delta_eta, center_phi,
  // delta_phi, center_z, hit.E());
}

/* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * */
KOKKOS_INLINE_FUNCTION int find_index_uint32(Kokkos::View<uint32_t *> array,
...
KOKKOS_INLINE_FUNCTION float rnd_to_fct1d(float rnd,
                                          Kokkos::View<uint32_t *> contents,
                                          Kokkos::View<float *> borders,
...
KOKKOS_INLINE_FUNCTION void HitCellMappingWiggle_g_d(HitParams hp, Hit &hit,
                                                     unsigned long t,
                                                     const Sim_Args &args,
                                                     const FHs_v &fhs_v) {
...
void simulate_hits_de(const Sim_Args args) {
  ...
  Kokkos::parallel_for(args.nhits, KOKKOS_LAMBDA(long tid) {
    ...
    if (hp.cmw)
      HitCellMappingWiggle_g_d(hp, hit, tid, args, f1d(bin));
...
void simulate_hits_ct(const Sim_Args args) {
  ...
  Kokkos::View<float *> cellE_v(args.cells_energy, args.ncells * args.nsims);
  Kokkos::View<int *> hitcells_ct_v(args.ct, args.nsims);
  Kokkos::View<Cell_E *> hitcells_E_v(args.hitcells_E,
                                      args.maxhitct * args.nsims);

  Kokkos::parallel_for(ncells * nsims, KOKKOS_LAMBDA(unsigned long tid) {
    if (tid < ncells * nsims) {
      if (cellE_v(tid) > 0) {
        int sim = tid / ncells;
...
void simulate_hits_gr(Sim_Args &args) {
  ...
  Kokkos::View<Cell_E *, Kokkos::HostSpace> cevh(args.hitcells_E_h,
                                                 MAXHITCT * args.nsims);
