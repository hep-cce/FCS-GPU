...
bool GeoLoadGpu::LoadGpu() {
   ...
   num_cells=m_ncells ;
    ...
    m_cells_g = (CaloDetDescrElement *) omp_target_alloc( sizeof(CaloDetDescrElement)*m_ncells, m_default_device);
    if ( m_cells_g == NULL ) {
  ...
  //  return false;
  if ( omp_target_memcpy( &m_cells_g[0], cells_Host, sizeof( CaloDetDescrElement ) * m_ncells, 
...
// GPU allocate Regions data  and load them to GPU as array of regions
    ...
    m_regions_g = (GeoRegion *) omp_target_alloc( sizeof(GeoRegion)*m_nregions, m_default_device);
  ...
  //  return false;
  if ( omp_target_memcpy( m_regions_g, m_regions, sizeof( GeoRegion ) * m_nregions, 
                               m_offset, m_offset, m_default_device, m_initial_device  ) ) {
    std::cout << " ERROR: Unable to copy ptr_g to device." << std::endl;
    return false;
  }
