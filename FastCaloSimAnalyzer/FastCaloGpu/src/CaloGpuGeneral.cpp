...
__host__ void *CaloGpuGeneral::Rand4Hits_init(long long maxhits, int maxbin,
...
__host__ void CaloGpuGeneral::load_hitsim_params(void *rd4h, HitParams *hp,
...
__device__ void HitCellMappingWiggle_g_d(HitParams hp, Hit &hit, long t,
                                         Sim_Args args) {
...
__host__ void CaloGpuGeneral::simulate_hits_gr(Sim_Args &args) {
  ...
  //           << "   sims: " << args.nsims << "  threads: " <<
  //           args.ncells*args.nsims
  //           << std::endl;
  ...
  auto t0 = std::chrono::system_clock::now();
  hipLaunchKernelGGL(simulate_clean, dim3(nblocks), dim3(blocksize), 0, 0,
                     args);
  err = hipGetLastError();
  if ( err != hipSuccess ) {
  ...
  auto t1 = std::chrono::system_clock::now();
  hipLaunchKernelGGL(simulate_hits_de, dim3(nblocks), dim3(blocksize), 0, 0,
                     args);
  err = hipGetLastError();
  if ( err != hipSuccess ) {
  ...
  auto t2 = std::chrono::system_clock::now();
  hipLaunchKernelGGL(simulate_hits_ct, dim3(nblocks), dim3(blocksize), 0, 0,
                     args);
  err = hipGetLastError();
  if ( err != hipSuccess ) {
  ...
  //   for( int isim=0 ; isim<args.nsims ; isim++ )
  //     gpuQ(hipMemcpy(&args.hitcells_E_h[isim*MAXHITCT],
  //     &args.hitcells_E[isim*MAXHITCT], args.ct_h[isim]*sizeof(Cell_E),
  //     hipMemcpyDeviceToHost));
}
