...
//  long t = threadIdx.x + blockIdx.x * blockDim.x;
  long t = hipThreadIdx_x + hipBlockIdx_x * hipBlockDim_x;
  if ( t < nhits ) {
    Hit hit;
    hit.E() = E;
...
__host__ void CaloGpuGeneral::simulate_hits( float E, int nhits, Chain0_Args& args ) {
  ...
  hipDeviceSynchronize() ;
  // if (err != hipSuccess) {
  //       std::cout<< "simulate_clean "<<hipGetErrorString(err)<< std::endl;
  //}
  ...
  auto t1 = std::chrono::system_clock::now();
  hipLaunchKernelGGL(simulate_A, dim3(nblocks), dim3(blocksize), 0, 0,  E, nhits, args );
  hipDeviceSynchronize() ;
  
  //  err = hipGetLastError();
  // if (err != hipSuccess) {
  //        std::cout<< "simulate_A "<<hipGetErrorString(err)<< std::endl;
  //}

  nblocks = ( ncells + blocksize - 1 ) / blocksize;
  auto t2 = std::chrono::system_clock::now();
  hipLaunchKernelGGL(simulate_ct, dim3(nblocks), dim3(blocksize), 0, 0,  args );
  hipDeviceSynchronize() ;
  
  // err = hipGetLastError();
  // if (err != hipSuccess) {
  //        std::cout<< "simulate_chain0_B1 "<<hipGetErrorString(err)<< std::endl;
  //}
