...
__global__ void testGeo(CaloDetDescrElement *cells, GeoRegion *regions,
  ...
  printf(" From GPU.., region %d, cell_grid[%d][%d]: index %llu index, "
         "hashid=%lld,eta=%f, phi=%f, sample=%d , ID=%lld cell_ptr=%p \n",
         r, ir, ip, index, hashid, eta, phi, sample, id,
         (void *)regions[r].all_cells());
...
bool GeoLoadGpu::LoadGpu() {
  ...
  if (hipSuccess !=
      hipMalloc((void **)&m_cells_g, sizeof(CaloDetDescrElement) * m_ncells))
    return false;
  ...
  if (hipSuccess != hipMemcpy(&m_cells_g[0], cells_Host,
  ...
  for (unsigned int ir = 0; ir < m_nregions; ++ir) {
    //	std::cout << "debug m_regions_g[ir].cell_grid()[0] " <<
    ...
    m_regions[ir].set_all_cells(
        m_cells_g); // set this so all region instance know where the GPU cells
                    // are, before copy to GPU
                    //	std::cout<<"Gpu cell Pintor in region: " <<m_cells_g << "
    //m_regions[ir].all_cells() " << m_regions[ir].all_cells() << std::endl ;
  }

  // GPU allocate Regions data  and load them to GPU as array of regions

  if (hipSuccess !=
      hipMalloc((void **)&m_regions_g, sizeof(GeoRegion) * m_nregions))
    return false;
  //      std::cout<< "cuMalloc "<< m_nregions << " regions" << std::endl;
  if (hipSuccess != hipMemcpy(m_regions_g, m_regions,
  ...
  //        Pointer GPU print from host" <<  m_regions_g  << std::endl;
  ...
  if (0) {
    hipLaunchKernelGGL(testGeo, dim3(1), dim3(1), 0, 0, m_cells_g, m_regions_g,
                       m_ncells, m_nregions, 14, 0, 32);
    gpuQ(hipDeviceSynchronize());
    hipError_t err = hipGetLastError();
    if (err != hipSuccess) {
      std::cout << hipGetErrorString(err) << std::endl;
