...
__global__ void testGeo(CaloDetDescrElement *cells, GeoRegion *regions,
  ...
  printf(" From GPU.., region %d, cell_grid[%d][%d]: index %lu index, "
         "hashid=%ld,eta=%f, phi=%f, sample=%d , ID=%ld "
         "cell_ptr=%#015lx \n",
         r, ir, ip, index, hashid, eta, phi, sample, id,
         regions[r].all_cells());

  CaloDetDescrElement cc = (regions[r].all_cells())[index];
  printf(" GPU test region have cells: cell index %lu, eta=%f phi=%f size of "
         "cell*GPU=%lu\n",
...
bool GeoLoadGpu::TestGeo() {
  ...
  testGeo_g << <1, 1>>> (get_geoPtr(), 14, 0, 32);
  gpuQ(cudaDeviceSynchronize());
...
bool GeoLoadGpu::LoadGpu_cu() {
  ...
  if (cudaSuccess !=
      cudaMalloc((void **)&m_cells_d, sizeof(CaloDetDescrElement) * m_ncells))
    return false;
  ...
  if (cudaSuccess != cudaMemcpy(&m_cells_d[0], cells_Host,
  ...
  if (cudaSuccess != cudaMemcpy(SampleIndex_g, m_sample_index_h,
                                sizeof(Rg_Sample_Index) * m_max_sample,
                                cudaMemcpyHostToDevice)) {
  ...
  if (cudaSuccess !=
      cudaMalloc((void **)&m_regions_d, sizeof(GeoRegion) * m_nregions))
    return false;
  //      std::cout<< "cuMalloc "<< m_nregions << " regions" << std::endl;
  if (cudaSuccess != cudaMemcpy(m_regions_d, m_regions,
