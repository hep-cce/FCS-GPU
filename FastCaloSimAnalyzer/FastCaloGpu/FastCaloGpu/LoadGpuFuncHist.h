...
public:
  ...
  FHs*  hf_h()   const { return m_hf_h; };
  FHs*  hf_d()   const { return m_hf_d; };
  FH2D* hf2d()   const { return m_hf2d; };
  FH2D* hf2d_h() const { return m_hf2d_h; };
...
private:
  ...
  struct FHs*  m_hf_h{nullptr}; // host pointer to struct hold device param that is copied to device
  
  struct FH2D* m_hf2d{nullptr};
  struct FH2D* m_hf2d_d{nullptr}; // device pointer
  struct FH2D* m_hf2d_h{nullptr}; // host poniter struct hold device param to be copied to device
