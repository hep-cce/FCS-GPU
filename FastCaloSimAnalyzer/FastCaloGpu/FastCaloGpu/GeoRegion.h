...
public:
  ...
  __HOSTDEV__ CaloDetDescrElement *all_cells() const {
    return m_all_cells;
  };
  ...
  __HOSTDEV__ float calculate_distance_eta_phi(const long long DDE, float eta,
                                               float phi, float &dist_eta0,
                                               float &dist_phi0) const;

  __HOSTDEV__ long long getDDE(float eta, float phi, float *distance = 0,
                               int *steps = 0);
