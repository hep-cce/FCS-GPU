...
typedef struct FHs_v {
  ...
  FHs_v(FHs *f1_h, FHs *f1_d) {
    ...
    low_edge = Kokkos::View<float *>(f1_d->low_edge, f1_h->nhist + 1);
    h_szs = Kokkos::View<unsigned int *>(f1_d->h_szs, f1_h->nhist);
    ...
    d_contents1D =
        Kokkos::View<uint32_t *>(f1_d->d_contents1D, max_sz * f1_h->nhist);
    //                                         tot_sz );
    d_borders1D =
        Kokkos::View<float *>(f1_d->d_borders1D, max_sz * f1_h->nhist);
    //                                       tot_sz );
  }
...
typedef struct FH2D_v {
  ...
  FH2D_v(FH2D *f2_h, FH2D *f2_d) {
    ...
    bordersx = Kokkos::View<float *>(f2_d->h_bordersx, f2_h->nbinsx + 1);
    bordersy = Kokkos::View<float *>(f2_d->h_bordersy, f2_h->nbinsy + 1);
    contents =
        Kokkos::View<float *>(f2_d->h_contents, f2_h->nbinsx * f2_h->nbinsy);
  }
