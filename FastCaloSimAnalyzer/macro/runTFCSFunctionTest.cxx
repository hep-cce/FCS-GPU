...
void runTFCSFunctionTest() {
  ...
  inputhist[1] = TFCS1DFunction::generate_histogram_random_gauss(4 * binscale,
                                                                 1000000, 1, 4);
  inputhist[2] = TFCS1DFunction::generate_histogram_random_gauss(16 * binscale,
                                                                 4000000, 1, 4);
  ...
  for (int itest = 0; itest < ntest; ++itest) {
    ...
    if (itest > 0) {
      ...
      hist = new TH1D(TString(histfine->GetName()) + "_coarse",
                      TString(histfine->GetTitle()) + " coarse", ibin,
                      xbins.data());
      for (int i = 1; i <= histfine->GetNbinsX(); ++i) {
        hist->Fill(histfine->GetBinCenter(i),
                   histfine->GetBinContent(i) / binscale);
      }
    }
    hist->SetLineWidth(3);
    ...
    histfine->SetName(TString(hist->GetName()) + "_fine");
    TFCS1DFunction* funcInterInt8Int8 =
        new TFCS1DFunctionInt8Int8InterpolationHistogram(hist);
    ...
    histfine->SetName(TString(hist->GetName()) + "_fine");
    TFCS1DFunction* funcInterInt8Int16 =
        new TFCS1DFunctionInt8Int16InterpolationHistogram(hist);
  ...
  TFCSHitCellMappingWiggle* wiggle_test =
      new TFCSHitCellMappingWiggle("WiggleTest", "WiggleTest");
  ...
  if (ftest) {
    ftest->ls();
    TFCSHitCellMappingWiggle* wiggle_test =
        (TFCSHitCellMappingWiggle*)ftest->Get("WiggleTest");
    if (wiggle_test) {
      ...
      for (int i = 0; i < wiggle_test->get_number_of_bins(); ++i) {
        ...
        cout << wiggle_test->get_bin_low_edge(i) << " <= eta < "
             << wiggle_test->get_bin_up_edge(i) << " func=" << func << endl;
        TH1* histfine =
            new TH1D(Form("Test%d", i), Form("Test %d", i), 128, 0, 1);
  ...
  */
  ...
  TFCS1DFunction* funcInterInt8Int8Gauss=new
  TFCS1DFunctionInt8Int8InterpolationHistogram(hist);
  ...
  */
  ...
  TFCS1DFunction* funcInterInt8Int16Gauss=new
  TFCS1DFunctionInt8Int16InterpolationHistogram(hist);
  ...
  */
  ...
  TFCS1DFunction* funcInt8Int8Gaussfine =
      new TFCS1DFunctionInt8Int8Histogram(histfine);
  TFCS1DFunction::unit_test(histfine, funcInt8Int8Gaussfine, 10000000);

  histfine->SetName("Int8Int16Gaussfine");
  TFCS1DFunction* funcInt8Int16Gaussfine =
      new TFCS1DFunctionInt8Int16Histogram(histfine);
  TFCS1DFunction::unit_test(histfine, funcInt8Int16Gaussfine, 10000000);

  /*
    histfine->SetName("Int8Int32Gaussfine");
    TFCS1DFunction* funcInt8Int32Gaussfine=new
    TFCS1DFunctionInt8Int32Histogram(histfine);
