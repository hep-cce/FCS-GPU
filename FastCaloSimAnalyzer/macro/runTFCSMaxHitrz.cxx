...
void CreatePolarPlot( TH2F* h, std::string outDir ) {
  ...
  TCanvas* c = TFCSAnalyzerBase::PlotPolar( h, title.c_str(), xlabel, ylabel, zlabel, 4 );
...
void runTFCSMaxHitrz( int dsid = 431004, std::string sampleData = "../python/inputSampleList.txt",
  ...
  TFCSApplyFirstPCA* myfirstPCA_App = new TFCSApplyFirstPCA( /*inputChain, pcaSample.Data(),*/ pcaAppSample.Data() );
  ...
  for ( int ilayer = 0; ilayer < v_layer.size(); ilayer++ ) {
    for ( int ipca = 1; ipca <= npca; ipca++ ) {
      ...
      TFCSValidationHitSpy hitspy_orig( "hitspy_2D_E_alpha_radius", "shape parametrization" );
      ...
      h_alpha_r = (TH2F*)Create2DHistogram( h_orig_hitEnergy_alpha_r, energy_cutoff );
      // save polar plots
