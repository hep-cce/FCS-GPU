...
// TFCSEnergyParametrization* NewPCAEnergyParametrization(CLHEP::HepRandomEngine
// *randEngine, std::string filename, int pdgid, int E, double etamin, double
// etamax)
// {
//   double Emin = (E / 2) * TMath::Sqrt(2);
//   double Emax = E * TMath::Sqrt(2);
//   return NewPCAEnergyParametrization(randEngine, filename, pdgid, E, Emin,
// Emax, etamin, etamax);
// }

TFCSParametrizationBase* NewEtaChain(
  ...
  TString etapara_name = Form("SelEta_id%d_Mom%d_%d_eta_%d_%d", pdgid,
                              int_Mom_min, int_Mom_max, int_etamin, int_etamax);
  TString etapara_title =
      Form("Select eta for id=%d %d<=Mom<%d %4.2f<=|eta|<%4.2f", pdgid,
  ...
  for (; int_etamin < int_etamax; int_etamin += 5) {
    ...
    TFCSParametrizationBase* para =
        FCS::NewEnergyChain(randEngine, mapping, numbersOfHits, pdgid,
                            int_Mom_min, int_Mom_max, etamin, etamax);
    if (para) {
      EtaSelectChain->push_back_in_bin(para);
      cout << "=========== EtaSelectChain ==========" << endl;
      cout << "pdgid=" << pdgid << " " << int_Mom_min << "<=Mom<" << int_Mom_max
           << " " << etamin << "<=eta<" << etamax << endl;
      // para->Print("short");
      cout << "=====================================" << endl;
    } else {
      cout << "============= ERROR =================" << endl;
      cout << "ERROR: pdgid=" << pdgid << " " << int_Mom_min << "<=Mom<"
           << int_Mom_max << " " << etamin << "<=eta<" << etamax << endl;
      cout << "=====================================" << endl;
    }
  }
...
int runTFCSCreateParametrization(int pid = 22, int Emin = 64,
                                 int Emax = 4194304, float etamin = 1.0,
                                 float etamax = 1.05,
                                 std::string topDir = "TFCSParam",
                                 long seed = 42) {
  ...
  FCS::init_numbers_of_hits(numbers_of_hits,
                            1);  // 1 is nominal stochastic term for EM
  ...
  TFCSParametrizationBase* param =
      NewEtaChain(randEngine, hit_to_cell_mapping, numbers_of_hits, pid, Emin,
                  Emax, etamin, etamax);
  ...
  auto fullchainfile =
      std::unique_ptr<TFile>(TFile::Open(paramFileName, "recreate"));
  if (!fullchainfile) {
    std::cerr << "Error: Could not create file '" << paramFileName << "'"
              << std::endl;
    return 1;
  }
  ...
  if (!fullchainfile) {
    std::cerr << "Error: Could not open file '" << paramFileName << "'"
              << std::endl;
    return 1;
  }
  ...
  TFCSParametrizationBase* fullpara =
...
int main(int argc, char** argv) {
  ...
  return runTFCSCreateParametrization(pdgId, Emin, Emax, etamin, etamax, output,
                                      seed);
}
