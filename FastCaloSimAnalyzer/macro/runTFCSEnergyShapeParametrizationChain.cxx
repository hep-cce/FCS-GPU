...
void runTFCSEnergyShapeParametrizationChain(int pdgid = 22, int Emin = 64,
                                            int Emax = 4194304,
                                            double etamin = 0,
                                            double etamax = 5.) {
  ...
  std::string topDir =
      "/eos/atlas/user/a/ahasib/public/Simul-FastCalo/"
      "ParametrizationProductionVer04/";
  std::string topPlotDir =
      "/eos/user/a/ahasib/www/Simul-FastCalo/ParametrizationProductionVer04/";
  ...
  for (int ieta = int_etamin; ieta < int_etamax; ieta += 5) {
    for (int ienergy = 0; ienergy < v_Energy.size(); ienergy++) {
      ...
      std::string dsid =
          FCS_dsid::find_dsid(std::to_string(pdgid), std::to_string(int_Emin),
                              std::to_string(ieta), "0");
      int int_dsid = std::stoi(dsid);

      run_epara(int_dsid, sampleData, topDir, npca1, npca2, run_validation,
                version, topPlotDir);

      runTFCS2DParametrizationHistogram(int_dsid, dsid_zv0, sampleData, topDir,
                                        version, energy_cutoff, topPlotDir,
                                        do2DParam, isPhisymmetry, doMeanRz,
                                        useMeanRz, doZVertexStudies);
    }
  }
}
