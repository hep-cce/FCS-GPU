...
TH1F* zoomHisto(TH1* h_in) {
  ...
  TH1F* h_out = new TH1F(h_in->GetName() + TString("_zoom"), h_in->GetTitle(),
                         Nbins, rmin, rmax);
...
TH2* Create2DHistogram(TH2* h, float energy_cutoff) {
  ...
  TH2* h2 =
      new TH2F(h->GetName() + TString("2D"), h->GetTitle() + TString("2D"),
...
void CreatePolarPlot(TH2F* h, std::string outDir) {
  ...
  TCanvas* c =
      TFCSAnalyzerBase::PlotPolar(h, title.c_str(), xlabel, ylabel, zlabel, 1);
...
void runTFCS2DParametrizationHistogram(
    int dsid, int dsid_zv0, std::string sampleData, std::string topDir,
  ...
  TString shapeSample(Form("%s%s.shapepara.%s.root", topDir.c_str(),
                           baselabel.c_str(), version.c_str()));
  TString extrapolSample(Form("%s%s.extrapol.%s.root", topDir.c_str(),
                              baselabel.c_str(), version.c_str()));
  TString zvertexSample(Form("%s%s.zvertex.%s.root", topDir.c_str(),
                             baselabel.c_str(), version.c_str()));
  ...
  // pcaSample =
  // Form("../../EnergyParametrization/scripts/output/ds%i.FirstPCA.ver01.root",dsid_zv0);
  ...
  if (useMeanRz) {
    TString name = Form("%s%s.extrapol.%s.root", topDir.c_str(),
                        sample_zv0.label.c_str(), version.c_str());
  ...
  for (size_t ilayer = 0; ilayer < v_layer.size(); ilayer++) {
    ...
    std::vector<TFCSCenterPositionCalculation*> centerPosCalc(
        npca);  // Will decorate hits with center extrap positions
    ...
    for (int ipca = 1; ipca <= npca; ipca++) {
      ...
      std::string prefixall =
          Form("cs%d_pca%d_", analyze_layer, analyze_pcabin);
      ...
      RunInputHits[i] = new TFCSParametrizationChain(
          "input_EnergyAndHits", "original energy and hits from input file");

      input_EnergyAndHits[i] = new TFCSValidationEnergyAndHits(
          "input_EnergyAndHits", "original energy and hits from input file",
          &analyze);
      ...
      centerPosCalc[i] = new TFCSCenterPositionCalculation(
          "CenterPosCalculation", "Center position calculation");
      centerPosCalc[i]->set_calosample(analyze_layer);

      if (useMeanRz)
        centerPosCalc[i]
            ->setExtrapWeight((*tm_mean_weight)[analyze_layer][ipca]);
      else
        centerPosCalc[i]->setExtrapWeight(0.5);

      hitspy_orig[i] = new TFCSValidationHitSpy("hitspy_2D_E_alpha_radius",
                                                "shape parametrization");

      hitspy_orig[i]->set_calosample(analyze_layer);

      if (doMeanRz)
        RunInputHits[i]->push_back(
            hitspy_orig[i]);  // to call the simulate() method in HitSpy
      ...
      if (do2DParam) {
        if (isPhiSymmetry) {
          h_orig_hitEnergy_alpha_r[i] =
              analyze.InitTH2(prefixall + "hist_hitenergy_alpha_radius", "", 8,
                              0, TMath::Pi(), nbinsy, ymin, ymax);
          hitspy_orig[i]->hist_hitenergy_alpha_absPhi_radius() =
              h_orig_hitEnergy_alpha_r[i];
          ...
          hitspy_orig[i]->hist_hitenergy_alpha_radius() =
              h_orig_hitEnergy_alpha_r[i];
        }
      }
      ...
      if (doMeanRz) {
        ...
        h_hitenergy_r[i] = analyze.InitTH1(prefixall + "hist_hitenergy_R", "1D",
                                           nbinsr, rmin, rmax);
        hitspy_orig[i]->hist_hitenergy_r() = h_hitenergy_r[i];
        h_orig_mean_hitenergy_r[i] = analyze.InitTH1(
            prefixall + "hist_mean_hitenergy_R", "1D", nbinsr, rmin, rmax);
        hitspy_orig[i]->hist_hitenergy_mean_r() = h_orig_mean_hitenergy_r[i];

        h_hitenergy_weight[i] = analyze.InitTH1(
            prefixall + "hist_hitenergy_weight", "1D", 2000, -2., 3.);
        hitspy_orig[i]->hist_hitenergy_weight() = h_hitenergy_weight[i];
        h_orig_mean_hitenergy_weight[i] = analyze.InitTH1(
            prefixall + "hist_mean_hitenergy_weight", "1D", 2000, 0., 1.);
        hitspy_orig[i]->hist_hitenergy_mean_weight() =
            h_orig_mean_hitenergy_weight[i];

        h_hitenergy_z[i] = analyze.InitTH1(prefixall + "hist_hitenergy_z", "1D",
                                           nbinsz, zmin, zmax);
        hitspy_orig[i]->hist_hitenergy_z() = h_hitenergy_z[i];
        h_orig_mean_hitenergy_z[i] = analyze.InitTH1(
            prefixall + "hist_mean_hitenergy_z", "1D", nbinsz, zmin, zmax);
        hitspy_orig[i]->hist_hitenergy_mean_z() = h_orig_mean_hitenergy_z[i];

        h_Rz[i] = analyze.InitTH2(prefixall + "hist_Rz", "2D", 1000, 0, 5000,
                                  1000, -10000, 10000);
        h_Rz_outOfRange[i] =
            analyze.InitTH2(prefixall + "hist_Rz_outOfRange", "2D", 1000, 0,
                            5000, 1000, -10000, 10000);
      ...
      if (doZVertexStudies) {
        h_deltaEtaAveragedPerEvent[i] =
            new TH1F((prefixall + "deltaEtaAveragedPerEvent").c_str(), "", 4000,
                     -400, 400);
        h_deltaEta[i] =
            new TH1F((prefixall + "deltaEtaHit").c_str(), "", 4000, -400, 400);
        h_deltaPhi[i] =
            new TH1F((prefixall + "deltaPhiHit").c_str(), "", 4000, -400, 400);
        h_deltaRt[i] =
            new TH1F((prefixall + "deltaRtHit").c_str(), "", 4000, -400, 400);
        h_deltaZ[i] =
            new TH1F((prefixall + "deltaZHit").c_str(), "", 4000, -400, 400);
        h_energyPerLayer[i] = new TH1F((prefixall + "energyPerLayer").c_str(),
                                       "", 4000, 0, 65000);
        h_MeanEnergy[i] =
            new TH1F((prefixall + "meanEnergy").c_str(), "", 1, 0, 1);

        h_deltaEtaAveragedPerEvent[i]->Sumw2();
        h_energyPerLayer[i]->Sumw2();
        input_EnergyAndHits[i]->hist_deltaEtaAveragedPerEvent() =
            h_deltaEtaAveragedPerEvent[i];
    ...
    for (int ipca = 1; ipca <= npca; ipca++) {
      ...
      std::pair<double, double> energyMeanSigma =
          input_EnergyAndHits[i]->getMeanEnergyWithError();
      ...
      if (do2DParam) {
        h_alpha_r = (TH2F*)Create2DHistogram(h_orig_hitEnergy_alpha_r[i],
                                             energy_cutoff);
      ...
      if (doMeanRz) {
        ...
        (*tm_mean_weight)[analyze_layer][ipca] =
            h_orig_mean_hitenergy_weight[i]->GetMean();
      }
      ...
      if (doZVertexStudies) {
        std::cout << "\nDelta eta averaged in pca bin " << ipca
                  << " over all events: "
                  << input_EnergyAndHits[i]->getDeltaEtaAveraged() << std::endl;
        std::cout << "\n Energy in layer " << analyze_layer << " and pca "
                  << ipca << " : " << energyMeanSigma.first << " +- "
...
static const char* USAGE = R"(Program to run shower shape parametrization
...
int main(int argc, char** argv) {
  ...
  try {
    ...
    try {
      ...
      throw std::invalid_argument(
          "--dsid option is mandatory and it is expected to be an integer. "
    ...
    try {
      ...
      throw std::invalid_argument(
          "--dsid_zv0 option is expected to be an integer. Check input "
          "parameters.");
    }
    ...
    try {
      ...
      throw std::invalid_argument(
          "--sampleData option is expected to be an integer. Check input "
          "parameters.");
    }
    ...
    try {
      ...
      throw std::invalid_argument(
          "--topDir option is expected to be an integer. Check input "
          "parameters.");
    }
    ...
    try {
      ...
      throw std::invalid_argument(
          "--version option is expected to be an integer. Check input "
          "parameters.");
    }
    ...
    try {
      ...
      throw std::invalid_argument(
          "--energy_cutoff option is expected to be a float. Check input "
          "parameters.");
    }
    ...
    try {
      ...
      throw std::invalid_argument(
          "--topPlotDir option is expected to be a string. Check input "
          "parameters.");
    }
    ...
    try {
      ...
      throw std::invalid_argument(
          "--do2DParam option is expected to be an integer. Check input "
          "parameters.");
    }
    ...
    try {
      ...
      throw std::invalid_argument(
          "--isPhiSymmetry option is expected to be an integer. Check input "
          "parameters.");
    }
    ...
    try {
      ...
      throw std::invalid_argument(
          "--doMeanRz option is expected to be an integer. Check input "
          "parameters.");
    }
    ...
    try {
      ...
      throw std::invalid_argument(
          "--useMeanRz option is expected to be an integer. Check input "
          "parameters.");
    }
    ...
    try {
      ...
      throw std::invalid_argument(
          "--doZVertexStudies option is expected to be an integer. Check input "
          "parameters.");
    }
    ...
    try {
      ...
      throw std::invalid_argument(
          "--seed option is expected to be an integer. Check input "
          "parameters.");
    }
    ...
    try {
      ...
      throw std::invalid_argument(
          "--nEvents option is expected to be an integer. Check input "
          "parameters.");
    }
    ...
    try {
      ...
      throw std::invalid_argument(
          "--firstEvent option is expected to be an integer. Check input "
          "parameters.");
    }
    ...
    try {
      ...
      throw std::invalid_argument(
          "--debug option is expected to be an integer. Check input "
          "parameters.");
    }
  }
  catch (const std::invalid_argument& e) {
  ...
  if (doMeanRz && useMeanRz) {
    std::cout << "Error: doMeanRz and useMeanRz cannot be set as true at the "
                 "same moment!" << std::endl;
    exit(-3);
  }

  std::cout << std::endl << "Parameters are loaded successfully!" << std::endl
            << "Executing runTFCS2DParametrizationHistogram function."
            << std::endl;

  runTFCS2DParametrizationHistogram(
      dsid, dsid_zv0, sampleData, topDir, version, energy_cutoff, topPlotDir,
      do2DParam, isPhiSymmetry, doMeanRz, useMeanRz, doZVertexStudies, seed,
