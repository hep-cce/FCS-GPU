...
void epara_validation_plots_fromtree( TString firstPCAfilename, TString firstPCA_Appfilename, TString valifilename,
                                      TString valiplotsname, int dsid, bool do_PCAclosure ) {
  double GeV = 1000.0;
  ...
  for ( unsigned int l = 0; l < layerNr.size(); l++ ) {
    TH1D* hist =
        new TH1D( Form( "h_efrac_layer%i_g4", layerNr[l] ), Form( "h_efrac_layer%i_g4", layerNr[l] ), 200, -0.1, 1.1 );
    hist->GetXaxis()->SetTitle( Form( "E fraction in layer %i (%s)", layerNr[l], lname[layerNr[l]].c_str() ) );
    h_efrac_g4.push_back( hist );
    TH1D* hist2 = new TH1D( Form( "h_efrac_layer%i_sim", layerNr[l] ), Form( "h_efrac_layer%i_sim", layerNr[l] ), 200,
                            -0.1, 1.1 );
  ...
  for ( int event = 0; event < read_tree_sim->GetEntries(); event++ ) {
    ...
    if ( read_tree_sim->GetVariable( "randombin" ) == read_tree_sim->GetVariable( "simulated_PCAbin" ) ) count_match++;
  ...
  h_PC_g4[0]->GetXaxis()->SetTitle( Form(
      "Matched bins: %.1f%%   %s", (double)count_match / (double)read_tree_sim->GetEntries() * 100.0, title.c_str() ) );
...
void epara_validation_plots( TString firstPCAfilename, TString valifilename, TString valiplotsname, int dsid,
                             bool do_PCAclosure ) {
  ...
  vector<int> layerNr;
  for ( int i = 1; i <= h_layer->GetNbinsY(); i++ ) {
    if ( h_layer->GetBinContent( 1, i ) == 1 ) layerNr.push_back( h_layer->GetYaxis()->GetBinCenter( i ) );
  }
  vector<string> layer;
  ...
  for ( unsigned int l = 0; l < layer.size(); l++ ) {
    ...
    TH1D*  h_input_lin;
    if ( use_autozoom ) {
      h_output_lin = (TH1D*)file->Get( Form( "h_output_zoom_%s", layer[l].c_str() ) );
...
void run_epara( int dsid = 431004, std::string sampleData = "../python/inputSampleList.txt",
                std::string topDir = "./output_epara/", int npca1 = 5, int npca2 = 1, bool run_validation = true,
                std::string version = "ver01", std::string topPlotDir = "output_plot_epara/", long seed = 42 ) {
  ...
  if ( run_validation ) {
    ...
    for ( unsigned int l = 0; l < layerNr.size(); l++ ) layer.push_back( Form( "layer%i", layerNr[l] ) );
    layer.push_back( "totalE" );
    for ( unsigned int l = 0; l < layer.size(); l++ ) cout << "l " << l << " " << layer[l] << endl;

    TH1D* h_output_fine[layer.size() + 2];
    for ( unsigned int l = 0; l < layerNr.size(); l++ ) {
      h_output_fine[l] =
          new TH1D( Form( "h_output_%s", layer[l].c_str() ), Form( "h_output_%s", layer[l].c_str() ), 5000, 0., 1. );
    }
    ...
    cout << "number of pca bins after load " << etest.n_pcabins() << " , number from firstPCA file: " << pcabins
         << endl;
    ...
    for ( unsigned int l = 0; l < layerNr.size(); l++ )
      tree_sim->Branch( Form( "simulated_efrac_layer%i", layerNr[l] ), &simulated_efrac[l],
    ...
    for ( int i = 0; i < ntoys; i++ ) {
      ...
      for ( unsigned int l = 0; l < layerNr.size(); l++ ) h_output_fine[l]->Fill( simulstate.Efrac( layerNr[l] ) );
      ...
      if ( !do_PCAclosure ) {
        for ( unsigned int l = 0; l < layerNr.size(); l++ ) simulated_efrac[l] = simulstate.Efrac( layerNr[l] );
    ...
    if ( do_PCAclosure ) {
      ...
      for ( unsigned int i = 0; i < simulstates.size(); i++ ) {
        ...
        for ( unsigned int l = 0; l < layer.size(); l++ ) simulated_PCA[l] = PCAdata[l];

        for ( unsigned int l = 0; l < layerNr.size(); l++ ) simulated_efrac[l] = simulstates[i].Efrac( layerNr[l] );
      ...
      for ( unsigned int i = 0; i < cumul_simdata.size(); i++ ) delete cumul_simdata[i];
    ...
    epara_validation_plots_fromtree( pca1Filename, pca1AppFilename, valiFilename, valiPlotsFilename, dsid,
                                     do_PCAclosure );
