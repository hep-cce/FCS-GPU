...
static const char *USAGE = R"(Create parametrization eta slice
...
int runTFCSCreateParamEtaSlice(int pdgid, int int_Mom_min, int int_Mom_max,
                               double etamin, std::string outDir, long seed) {
  ...
  FCS::init_numbers_of_hits(hit_to_cell_mapping,
  ...
  TFCSParametrizationBase *para =
      FCS::NewEnergyChain(randEngine, hit_to_cell_mapping, numbers_of_hits,
                          pdgid, int_Mom_min, int_Mom_max, etamin, etamax);
  if (para) {
    TString filename = Form("%s/%s", outDir.c_str(), para->GetName());
    auto file =
        std::unique_ptr<TFile>(TFile::Open(filename + ".root", "recreate"));
    if (!file) {
