...
void Draw_1Dhist(TH1* hist1, double ymin = 0, double ymax = 0,
                 bool logy = false, TString name = "", TString title = "",
                 TCanvas* c = 0, bool png = false) {
  if (name == "") {
    name = hist1->GetName();
  ...
  t1->SetTextFont(62);
  t1 =
      pt->AddText(Form("%5.3f#pm%5.3f", hist1->GetRMS(), hist1->GetRMSError()));
  t1->SetTextFont(42);
...
void Energy_histograms(TFCSShapeValidation* analyze, int analyze_pcabin,
...
int runTFCSSimulation(int pdgid = 22, int int_E = 65536, double etamin = 0.2,
                      int analyze_layer = 2,
  ...
  diff = t3 - t2;
  std::cout << "Time in funtion loopevent():" << diff.count() << " s"
            << std::endl;
  diff = t4 - t3;
