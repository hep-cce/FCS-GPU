...
std::string find_dsid(std::string pdgid, std::string energy, std::string eta,
  ...
  for (unsigned int i = 0; i < dsid_db_energy.size(); i++) {
    if (energy == dsid_db_energy[i] && pdgid == dsid_db_pdgid[i] &&
        zvertex == dsid_db_z[i] && eta == dsid_db_eta[i]) {
      dsid = dsid_db_dsid[i];
      break;
    }
  }
...
std::string get_dsid_basename(std::string dsid) {
  ...
  basename = std::string("mc16_13TeV.") + dsid + ".ParticleGun_pid" + pdgid +
             "_E" + energy + "_disj_eta_m" + etaend + "_m" + eta + "_" + eta +
             "_" + etaend + "_zv_" + zvertex;
...
int wildcard_add_files_to_chain(TChain* chain, std::string filenames) {
  gSystem->Exec((std::string("ls ") + filenames +
                 " > $TMPDIR/FCS_ls.$PPID.list").c_str());
  TString tmpname = gSystem->Getenv("TMPDIR");
  tmpname += "/FCS_ls.";
  tmpname += gSystem->GetPid();
  tmpname += ".list";
  std::cout << "Temporary file list for selection:" << filenames << " : "
            << tmpname << std::endl;
...
std::string get_dsid_avg_sim_shapename(std::string dsid, std::string basedir,
                                       std::string version) {
  std::string basename = get_dsid_basename(dsid);
...
std::string get_dsid_shapename(std::string dsid, std::string basedir,
                               std::string version) {
  std::string basename = get_dsid_basename(dsid);
...
std::string get_dsid_firstPCAname(std::string dsid, std::string basedir,
                                  std::string version) {
  std::string basename = get_dsid_basename(dsid);
...
std::string get_dsid_firstPCA_Appname(std::string dsid, std::string basedir,
                                      std::string version) {
  std::string basename = get_dsid_basename(dsid);
...
std::string get_dsid_secondPCAname(std::string dsid, std::string basedir,
                                   std::string version) {
  std::string basename = get_dsid_basename(dsid);
...
std::string get_wiggle_name(std::string etarange, int sampling,
                            bool isNewWiggle, std::string basedir,
                            std::string version) {
  ...
  if (isNewWiggle) {
    ...
    version = "ver03";
    filename = basedir + "Wiggle_old/" + etarange +
...
std::string get_param_etaslice_name(int pid, std::string etamin,
                                    std::string etamax, std::string basedir,
                                    std::string version) {
  std::string wildcard = basedir + "TFCSParamEtaSlices/" + "mc16_13TeV." +
                         "pid" + std::to_string(pid) + ".E*eta_" + etamin +
                         "_" + etamax + "_zv0.TFCSParam." + version + ".root";

  gSystem->Exec(
      (std::string("ls ") + wildcard + " > $TMPDIR/FCS_ls.$PPID.list").c_str());
  TString tmpname = gSystem->Getenv("TMPDIR");
  tmpname += "/FCS_ls.";
  tmpname += gSystem->GetPid();
  tmpname += ".list";
