...
class TFCSAnalyzerBase {
 public:
  ...
  TH1F *InitTH1(std::string histname, std::string histtype, int nbins,
                float low, float high, std::string xtitle = "",
                std::string ytitle = "");
  TH2F *InitTH2(std::string histname, std::string histtype, int nbinsx,
                float lowx, float highx, int nbinsy, float lowy, float highy,
                std::string xtitle = "", std::string ytitle = "");
  TProfile *InitTProfile1D(std::string histname, std::string histtype,
                           int nbinsx, float lowx, float highx,
                           std::string xtitle = "", std::string ytitle = "",
                           std::string profiletype = "S");
  TProfile2D *InitTProfile2D(std::string histname, std::string histtype,
  ...
  static void Fill(TProfile2D *h, float valuex, float valuey, float valuez,
                   float weight);

  static void autozoom(TH1 *h1, double &min, double &max, double &rmin,
                       double &rmax);
  static TH1D *refill(TH1 *h_in, double min, double max, double rmin,
                      double rmax);

  static void GetTH1TTreeDraw(TH1F *&histo, TTree *tree, std::string var,
                              std::string *cut, int nbins, double xmin,
                              double xmax);
  void GetTH2TTreeDraw(TH2F *&hist, TTree *tree, std::string var,
                       std::string *cut, int nbinsx, double xmin, double xmax,
                       int nbinsy, double ymin, double ymax);

  static TCanvas *PlotTH1Ratio(TH1F *h1, TH1F *h2, std::string label,
                               std::string xlabel, std::string leg1,
                               std::string leg2, std::string ylabel1,
                               std::string ylabel2);
  static TCanvas *PlotPolar(TH2F *h, std::string label, std::string xlabel,
                            std::string ylabel, std::string zlabel,
                            int zoom_level = 1);

  std::tuple<float, float> GetUnitsmm(float eta_hits, float deta, float dphi,
                                      CaloCell *cell);
  static std::tuple<float, float> GetUnitsmm(float eta_hits, float deta,
                                             float dphi, float cell_r,
                                             float cell_z);
  ...
  const double &total_energy() const {
    return m_total_energy;
  };
  const std::vector<double> &total_layer_cell_energy() const {
    return m_total_layer_cell_energy;
  };
  FCS_matchedcellvector *cellVector() const {
    return m_cellVector;
  };
  FCS_matchedcellvector *avgcellVector() const {
    return m_avgcellVector;
  };
 ...
 protected:
  ...
  // * reading input TTree
  template <typename T>
  void setBranch(TChain *chain, std::vector<TBranch *> &branches,
