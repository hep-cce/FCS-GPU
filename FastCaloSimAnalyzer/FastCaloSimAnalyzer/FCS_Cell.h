...
struct FCS_matchedcell //this is the matched structure for a single cell
{
  ...
  inline float scalingfactor() {float hitsum = 0.; for (unsigned int i = 0; i < hit.size(); i++) {hitsum += hit[i].hit_energy;}; float sf = 1; if (hitsum == 0) { sf = 1; std::cout << "WARNING::Sum of hit energy is 0!, setting SF = 1 and cell energy = "<< cell.energy  << std::endl ;} else {sf = cell.energy / hitsum; } return sf;}; //doesn't check for 0!
