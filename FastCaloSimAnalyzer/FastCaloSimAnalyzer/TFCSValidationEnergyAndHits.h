...
class TFCSValidationEnergyAndHits
    : public TFCSLateralShapeParametrizationHitChain {
 public:
  TFCSValidationEnergyAndHits(const char* name = 0, const char* title = 0,
                              TFCSAnalyzerBase* analysis = 0);
  ...
  void set_analysis(TFCSAnalyzerBase* analysis) {
    m_analysis = analysis;
  };
  TFCSAnalyzerBase* analysis() {
    return m_analysis;
  };
  const TFCSAnalyzerBase* analysis() const {
    return m_analysis;
  };

  int n_bins() {
    return -1;
  };  // TO BE FIXED, SHOULD BE SOMEHOW READ FROM PCA FILE
  ...
  TFCSValidationHitSpy& get_hitspy() {
    return m_hitspy;
  };

  void add_histo(TH1* hist);
  TH1*& hist_hit_time() {
    return m_hist_hit_time;
  };
  TH2*& hist_problematic_hit_eta_phi() {
    return m_hist_problematic_hit_eta_phi;
  };
  TH1*& hist_deltaEtaAveragedPerEvent() {
    return m_hist_deltaEtaAveragedPerEvent;
  };
  TH1*& hist_energyPerLayer() {
    return m_hist_energyPerLayer;
  };
  ...
  inline double getDeltaEtaAveraged() {
    return m_deltaEtaAveraged / m_energy_total;
  }
  inline std::pair<double, double> getMeanEnergyWithError() {
    return std::make_pair<double, double>(
        m_energy_total / m_eventCounter,
        sqrt((m_energy_squared_total / m_eventCounter -
              pow(m_energy_total / m_eventCounter, 2)) /
             m_eventCounter));
  }
 ...
 private:
  ...
  ClassDefOverride(TFCSValidationEnergyAndHits,
                   1)  // TFCSValidationEnergyAndHits
};
