...
class TFCSVertexZPositionStudies {
 public:
  ...
  void loadFiles(string dirname, string filename_nominal,
  ...
  void loadHistogramsForFixedZVAndLayer(int zv_index, int layer,
                                        string histname);
  ...
  void findBinning(bool useMMbinning = false, double factor = 1. / 3,
                   double quantile = 0.003);
  void rebinHistos();
  void plotHistograms(string outputDir, bool ratio_plots = false,
                      bool drawErrorBars = false, bool useLogScale = false);
  void plotHistograms(vector<TH1F*>& histos, string outputDir,
                      bool drawErrorBars = false, bool useLogScale = false);
  ...
  void setParticleInfo(string filename);  // Function will attempt to determine
                                          // particle info from filename
