...
void TFCSEnergyBinParametrization::resize() {
  for (std::set<int>::iterator itid = pdgid().begin(); itid != pdgid().end();
       ++itid) {
...
void TFCSEnergyBinParametrization::set_pdgid_Ekin_bin_probability(
  ...
  if (prob.size() != m_pdgid_Ebin_probability[id].size()) {
    ATH_MSG_ERROR(
        "TFCSEnergyBinParametrization::set_pdgid_Ekin_bin_probability(): size "
        "of vectors does not match! in.size()="
...
void TFCSEnergyBinParametrization::load_pdgid_Ekin_bin_probability_from_file(
  ...
  file->cd();
  TVectorF* pcabinprobvector =
      (TVectorF*)gDirectory->Get(prob_object_name.c_str());
  ...
  if (pcabinprobvector) {
    if ((unsigned int)pcabinprobvector->GetNoElements() !=
        m_pdgid_Ebin_probability[id].size()) {
      ATH_MSG_ERROR(
          "TFCSEnergyBinParametrization::load_pdgid_Ekin_bin_probability_from_"
          "file(): size of prob array does not match! in.size()="
          << pcabinprobvector->GetNoElements()
...
FCSReturnCode TFCSEnergyBinParametrization::simulate(
  ...
  int chosenBin =
      TMath::BinarySearch(n_bins() + 1, m_pdgid_Ebin_probability[pdgid].data(),
                          searchRand) +
      1;
  if (chosenBin < 0 || chosenBin > n_bins()) {
    ATH_MSG_ERROR(
        "TFCSEnergyBinParametrization::simulate(): cannot simulate bin="
        << chosenBin);
    ATH_MSG_ERROR("  This error could probably be retried.");
    if (msgLvl(MSG::ERROR)) {
      ATH_MSG(ERROR) << "in " << GetName() << ": E=" << simulstate.E()
                     << " Ebin=" << chosenBin << " rnd=" << searchRand
                     << " array=";
      for (int iEbin = 0; iEbin <= n_bins(); ++iEbin)
        msg() << m_pdgid_Ebin_probability[pdgid][iEbin] << " ";
      msg() << std::endl;
    }
    return FCSFatal;
  }
