...
FCSReturnCode TFCSPCAEnergyParametrization::simulate(
  ...
  if (pcabin == 0) {
    ...
    for (unsigned int l = 0; l <= layerNr.size(); l++) {
      ...
      double gauszz =
          CLHEP::RandGauss::shoot(simulstate.randomEngine(), mean, rms);
      input_data[l] = gauszz;
    }
    ...
    double total_energy =
        simdata[layerNr.size()] * simulstate.E() / Ekin_nominal();
...
void TFCSPCAEnergyParametrization::P2X(TVectorD* SigmaValues,
                                       ...
                                       int gNVariables, double* p, double* x,
                                       int nTest) {
...
bool TFCSPCAEnergyParametrization::loadInputs(TFile* file, std::string folder) {
  ...
  if (RelevantLayers == NULL) {
    ATH_MSG_ERROR(
        "TFCSPCAEnergyParametrization::m_RelevantLayers in first pcabin is "
        "null!");
    load_ok = false;
  }
