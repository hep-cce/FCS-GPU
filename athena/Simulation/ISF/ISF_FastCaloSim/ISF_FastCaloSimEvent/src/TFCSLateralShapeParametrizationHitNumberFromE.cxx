...
int TFCSLateralShapeParametrizationHitNumberFromE::get_number_of_hits(
  ...
  int hits = CLHEP::RandPoisson::shoot(
      simulstate.randomEngine(),
...
void TFCSLateralShapeParametrizationHitNumberFromE::Print(Option_t* option)
    const {
  TString opt(option);
  ...
  if (longprint)
    ATH_MSG_INFO(optprint << "  stochastic=" << m_stochastic
                          << " constant=" << m_constant);
}
