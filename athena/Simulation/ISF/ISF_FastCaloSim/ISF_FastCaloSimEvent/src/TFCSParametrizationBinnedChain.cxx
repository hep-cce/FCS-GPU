...
FCSReturnCode TFCSParametrizationBinnedChain::simulate(
  ...
  for (auto ichain : chain())
    ATH_MSG_DEBUG("Chain[" << ichain << "] name:" << typeid(*(ichain)).name()
                           << " Title=" << ichain->GetName());
  for (int ii = 0; ii < m_bin_start.size(); ii++)
    ATH_MSG_DEBUG("bin_start[" << ii << "]=" << m_bin_start[ii]);
...
void TFCSParametrizationBinnedChain::unit_test(
  ...
  for (int i = 0; i < 3; ++i) {
    TFCSParametrizationBase* param =
        new TFCSParametrization(Form("A%d", i), Form("A %d", i));
    param->setLevel(MSG::DEBUG);
  ...
  for (int i = 3; i > 0; --i) {
    TFCSParametrizationBase* param =
        new TFCSParametrization(Form("B%d", i), Form("B %d", i));
    param->setLevel(MSG::DEBUG);
    chain.push_back_in_bin(param, i);
  }
