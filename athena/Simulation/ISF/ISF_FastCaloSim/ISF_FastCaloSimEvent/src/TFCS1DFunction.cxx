...
TH1* TFCS1DFunction::generate_histogram_random_slope(int nbinsx, double xmin,
                                                     double xmax,
...
TH1* TFCS1DFunction::generate_histogram_random_gauss(int nbinsx, int ntoy,
...
void TFCS1DFunction::unit_test(TH1* hist, TFCS1DFunction* rtof, int nrnd,
                               TH1* histfine) {
  ...
  for (int ix = 1; ix <= nbinsx; ++ix) {
    ...
    // std::cout<<"x="<<hist->GetBinCenter(ix)<<" : pull val="<<val<<"
    // err="<<err<<std::endl;
  }
