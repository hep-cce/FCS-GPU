...
ISF_HitAnalysis::ISF_HitAnalysis(const std::string& name,
    ...
    : AthAlgorithm(name, pSvcLocator)
      ...
      // Note that m_xxx are pointers to vectors set to 0, not set to empty
      // vector! see note around TBranch
{
  ...
  declareProperty("NTruthParticles", m_NtruthParticles = 1,
                  "Number of truth particles saved from the truth collection, "
                  "-1 to save all");

  declareProperty("FastCaloSimCaloExtrapolation",
  ...
  // declareProperty("CalosurfEntranceInstanceName",
  // m_calosurf_entrance_InstanceName);
...
StatusCode ISF_HitAnalysis::geoInit(IOVSVC_CALLBACK_ARGS) {
  ...
  if (service("ToolSvc", p_toolSvc).isFailure()) {
    ...
    if (p_toolSvc->retrieveTool(CaloCoordinateTool.type(),
                                CaloCoordinateTool.name(), algTool,
                                this).isFailure()) {
...
StatusCode ISF_HitAnalysis::updateMetaData(IOVSVC_CALLBACK_ARGS_P(I, keys)) {
  ...
  ATH_MSG_DEBUG("Update called with " << I << " folder " << keys.size()
                                      << " keys:");
...
StatusCode ISF_HitAnalysis::initialize() {
  ...
  if (m_geoModel->geoInitialized()) {
    ...
    sc = detStore()->regFcn(&IGeoModelSvc::geoInit, m_geoModel,
                            &ISF_HitAnalysis::geoInit, this);
    if (sc.isFailure()) {
  ...
  if (detStore()->contains<AthenaAttributeList>(m_MC_DIGI_PARAM)) {
    ...
    if (detStore()
            ->regFcn(&ISF_HitAnalysis::updateMetaData, this, aptr,
                     m_MC_DIGI_PARAM, true)
            .isFailure()) {
  ...
  std::unique_ptr<TFile> dummyFile = std::unique_ptr<TFile>(TFile::Open(
      "dummyFile.root", "RECREATE"));  // This is added to suppress the error
                                       // messages about memory-resident trees
...
StatusCode ISF_HitAnalysis::finalize() {
  ...
  std::unique_ptr<TFile> dummyGeoFile = std::unique_ptr<TFile>(TFile::Open(
      "dummyGeoFile.root", "RECREATE"));  // This is added to suppress the error
                                          // messages about memory-resident
                                          // trees
  TTree* geo = new TTree(m_geoModel->atlasVersion().c_str(),
                         m_geoModel->atlasVersion().c_str());
  std::string fullNtupleName =
      "/" + m_geoFileName + "/" + m_geoModel->atlasVersion();
  ...
  if (geo) {
    ATH_MSG_INFO("Successfull registered TTree: " << fullNtupleName);
    // this actually creates the vector itself! And only if it succeeds! Note
    // that the result is not checked! And the code is probably leaking memory
    // in the end
    // geo->Branch("cells",
  ...
  if (m_calo_dd_man) {
    int ncells = 0;
    for (CaloDetDescrManager::calo_element_const_iterator calo_iter =
             m_calo_dd_man->element_begin();
...
StatusCode ISF_HitAnalysis::execute() {
  ...
  // now if the branches were created correctly, the pointers point to something
  // and it is possible to clear the vectors
  TVector3 vectest;
  ...
  std::map<Long64_t, FCS_cell> cells;  // read all objects and collect them by
                                       // identifier (Long64_t)
  ...
  FCS_cell one_cell;  // note that this is not extra safe if I don't have a
                      // clear method!
  FCS_g4hit one_g4hit;
  FCS_hit one_hit;
  ...
  if (sc.isFailure()) {
    ...
    for (ISF_FCS_Parametrization::FCS_StepInfoCollection::const_iterator it =
             eventStepsES->begin();
      ...
      Identifier cell_id =
          (*it)->identify();  // to be replaced by cell_id in tile

      if (m_calo_dd_man->get_element(id)) {
        CaloCell_ID::CaloSample layer =
            m_calo_dd_man->get_element(id)->getSampling();
        sampling = layer;  // use CaloCell layer immediately
      } else {
        ATH_MSG_WARNING("Warning no sampling info for " << id.getString());
      }

      if (m_larEmID->is_lar_em(id) || m_larHecID->is_lar_hec(id) ||
          m_larFcalID->is_lar_fcal(id))
      ...
      if (m_larEmID->is_lar_em(id)) {
      ...
      } else if (m_tileID->is_tile_barrel(id) ||
                 m_tileID->is_tile_extbarrel(id) || m_tileID->is_tile_gap(id)) {
        ...
        if (tile_sampling != -1)
          sampling = tile_sampling;  // m_calo_dd_man needs to be called with
                                     // cell_id not pmt_id!!
      } else {
  ...
  // Note that there can be more truth particles, the first one is usually the
  // one we need.
  ...
  if (sc.isFailure()) {
    ...
    if (mcEvent) {
      // std::cout<<"ISF_HitAnalysis: MC event size:
      // "<<mcEvent->size()<<std::endl;
      if (mcEvent->size()) {
        int particleIndex = 0;
        int loopEnd = m_NtruthParticles;
        if (loopEnd == -1) {
          loopEnd = (*mcEvent->begin())
                        ->particles_size();  // is this the correct thing?
        }
        // std::cout <<"ISF_HitAnalysis: MC first truth event size:
        // "<<(*mcEvent->begin())->particles_size()<<std::endl;
        for (HepMC::GenEvent::particle_const_iterator it =
                 (*mcEvent->begin())->particles_begin();
             it != (*mcEvent->begin())->particles_end(); ++it) {
          ATH_MSG_DEBUG("Number truth particles="
                        << (*mcEvent->begin())->particles_size()
                        << " loopEnd=" << loopEnd);
          particleIndex++;

          if (particleIndex > loopEnd) break;  // enough particles

          // UPDATE EXTRAPOLATION WITH
          // ALGTOOL***********************************************

          TFCSTruthState truth((*it)->momentum().px(), (*it)->momentum().py(),
                               (*it)->momentum().pz(), (*it)->momentum().e(),
                               (*it)->pdg_id());

          // calculate the vertex
          TVector3 moment;
          moment.SetXYZ((*it)->momentum().px(), (*it)->momentum().py(),
                        (*it)->momentum().pz());
          TVector3 direction = moment.Unit();

          // does it hit the barrel or the EC?

          if (abs(direction.Z()) / m_CaloBoundaryZ <
              direction.Perp() / m_CaloBoundaryR) {
            // BARREL
          ...
          if ((*it)->production_vertex()) {
            truth.set_vertex((*it)->production_vertex()->point3d().x(),
                             (*it)->production_vertex()->point3d().y(),
                             (*it)->production_vertex()->point3d().z());
          } else {
            truth.set_vertex(direction.X(), direction.Y(), direction.Z());
            ATH_MSG_WARNING(
                "No particle production vetext, use VERTEX from direction: x "
                << direction.X() << " y " << direction.Y() << " z "
                << direction.Z());
          }

          if (fabs(direction.X() - truth.vertex().X()) > 0.1 ||
              fabs(direction.Y() - truth.vertex().Y()) > 0.1 ||
              fabs(direction.Z() - truth.vertex().Z()) > 0.1) {
            ATH_MSG_WARNING("VERTEX from direction: x "
                            << direction.X() << " y " << direction.Y() << " z "
                            << direction.Z());
            ATH_MSG_WARNING("but VERTEX from hepmc: x "
                            << truth.vertex().X() << " y " << truth.vertex().Y()
                            << " z " << truth.vertex().Z());
          }
          ...
          m_newTTC_IDCaloBoundary_eta->push_back(
              float(result.IDCaloBoundary_eta()));
          m_newTTC_IDCaloBoundary_phi->push_back(
              float(result.IDCaloBoundary_phi()));
          m_newTTC_IDCaloBoundary_r->push_back(
              float(result.IDCaloBoundary_r()));
          m_newTTC_IDCaloBoundary_z->push_back(
              float(result.IDCaloBoundary_z()));
          ...
          for (int sample = CaloCell_ID_FCS::FirstSample;
               sample < CaloCell_ID_FCS::MaxSample; ++sample) {
            ATH_MSG_DEBUG("sample " << sample);
            ATH_MSG_DEBUG(" eta ENT " << result.eta(sample, 1) << " eta EXT "
                                      << result.eta(sample, 2));
            ATH_MSG_DEBUG(" phi ENT " << result.phi(sample, 1) << " phi EXT "
                                      << result.phi(sample, 2));
            ATH_MSG_DEBUG(" r   ENT " << result.r(sample, 1) << " r   EXT "
                                      << result.r(sample, 2));
            ATH_MSG_DEBUG(" z   ENT " << result.z(sample, 1) << " z   EXT "
                                      << result.z(sample, 2));
            ATH_MSG_DEBUG(" detaBorder   ENT " << result.detaBorder(sample, 1)
                                               << " detaBorder   EXT "
                                               << result.detaBorder(sample, 2));
            ATH_MSG_DEBUG(" OK  ENT " << result.OK(sample, 1) << " OK  EXT "
                                      << result.OK(sample, 2));
            eta_vec_ENT.push_back(
                float(result.eta(sample, TFCSExtrapolationState::SUBPOS_ENT)));
            eta_vec_EXT.push_back(
                float(result.eta(sample, TFCSExtrapolationState::SUBPOS_EXT)));
            eta_vec_MID.push_back(
                float(result.eta(sample, TFCSExtrapolationState::SUBPOS_MID)));
            phi_vec_ENT.push_back(
                float(result.phi(sample, TFCSExtrapolationState::SUBPOS_ENT)));
            phi_vec_EXT.push_back(
                float(result.phi(sample, TFCSExtrapolationState::SUBPOS_EXT)));
            phi_vec_MID.push_back(
                float(result.phi(sample, TFCSExtrapolationState::SUBPOS_MID)));
            r_vec_ENT.push_back(
                float(result.r(sample, TFCSExtrapolationState::SUBPOS_ENT)));
            r_vec_EXT.push_back(
                float(result.r(sample, TFCSExtrapolationState::SUBPOS_EXT)));
            r_vec_MID.push_back(
                float(result.r(sample, TFCSExtrapolationState::SUBPOS_MID)));
            z_vec_ENT.push_back(
                float(result.z(sample, TFCSExtrapolationState::SUBPOS_ENT)));
            z_vec_EXT.push_back(
                float(result.z(sample, TFCSExtrapolationState::SUBPOS_EXT)));
            z_vec_MID.push_back(
                float(result.z(sample, TFCSExtrapolationState::SUBPOS_MID)));
            detaBorder_vec_ENT.push_back(float(
                result.detaBorder(sample, TFCSExtrapolationState::SUBPOS_ENT)));
            detaBorder_vec_EXT.push_back(float(
                result.detaBorder(sample, TFCSExtrapolationState::SUBPOS_EXT)));
            detaBorder_vec_MID.push_back(float(
                result.detaBorder(sample, TFCSExtrapolationState::SUBPOS_MID)));
            OK_vec_ENT.push_back(
                result.OK(sample, TFCSExtrapolationState::SUBPOS_ENT));
            OK_vec_EXT.push_back(
                result.OK(sample, TFCSExtrapolationState::SUBPOS_EXT));
            OK_vec_MID.push_back(
                result.OK(sample, TFCSExtrapolationState::SUBPOS_MID));
          }
          ...
          for(std::vector<Trk::HitInfo>::iterator it = hitVector->begin();it <
          hitVector->end();++it) {
            if((*it).trackParms) {
          ...
          */
          ...
          // Amg::Vector3D
          // mom((*it)->momentum().x(),(*it)->momentum().y(),(*it)->momentum().z());
  ...
  if (sc.isFailure()) {
    ...
    for (; itrCell != itrLastCell; ++itrCell) {
      ...
      if (m_tileID->is_tile_aux((*itrCell)->ID())) {
        ...
        // all other Tile cells
        CaloCell_ID::CaloSample layer =
            m_calo_dd_man->get_element((*itrCell)->ID())->getSampling();
  ...
  std::string lArKey[4] = {"LArHitEMB",  "LArHitEMEC",
                           "LArHitFCAL", "LArHitHEC"};
    ...
    if (evtStore()->retrieve(iter, lArKey[i]) == StatusCode::SUCCESS) {
      ...
      for (hi = (*iter).begin(); hi != (*iter).end(); hi++) {
        ...
        if (m_calo_dd_man->get_element(larhitid)) {
          CaloCell_ID::CaloSample larlayer =
              m_calo_dd_man->get_element(larhitid)->getSampling();
  ...
  if (evtStore()->retrieve(hitVec, "TileHitVec") == StatusCode::SUCCESS &&
      m_tileMgr && m_tileID) {
    int hitnumber = 0;
    for (TileHitVecConstIterator i_hit = hitVec->begin();
         i_hit != hitVec->end(); ++i_hit) {
      hitnumber++;
      Identifier pmt_id = (*i_hit).identify();
      Identifier cell_id = m_tileID->cell_id(pmt_id);
      // const  CaloDetDescrElement* ddElement =
      // m_tileMgr->get_cell_element(cell_id);

      if (m_calo_dd_man->get_element(cell_id)) {
        CaloCell_ID::CaloSample layer =
            m_calo_dd_man->get_element(cell_id)->getSampling();

        float tilesampfrac = m_tileInfo->HitCalib(cell_id);

        // could there be more subhits??
        for (int tilesubhit_i = 0; tilesubhit_i < (*i_hit).size();
             tilesubhit_i++) {
          //!!
          // std::cout <<"Tile subhit: "<<tilesubhit_i<<"/"<<(*i_hit).size()<< "
          // E: "<<(*i_hit).energy(tilesubhit_i)<<std::endl;
  ...
  for (unsigned int cell_i = 0; cell_i < m_cell_identifier->size(); cell_i++) {
    if (cells.find((*m_cell_identifier)[cell_i]) ==
        cells.end())  // doesn't exist
    {
      ...
      cells.insert(
          std::pair<Long64_t, FCS_cell>(one_cell.cell_identifier, one_cell));
    } else {
  ...
  // g4 hits
  if (m_doG4Hits) {
    for (unsigned int g4hit_i = 0; g4hit_i < m_g4hit_identifier->size();
         g4hit_i++) {
      if ((*m_g4hit_sampling)[g4hit_i] >= 0 &&
          (*m_g4hit_sampling)[g4hit_i] <= 25 &&
          (*m_g4hit_time)[g4hit_i] > m_TimingCut) {
        ATH_MSG_DEBUG("Ignoring G4hit, time too large: "
                      << g4hit_i << " time: " << (*m_g4hit_time)[g4hit_i]);
        continue;
      }
      ...
      if (g4hits.find((*m_g4hit_cellidentifier)[g4hit_i]) == g4hits.end()) {
        ...
        if (one_g4hit.sampling >= 12 && one_g4hit.sampling <= 20) {  // tile
          // std::cout <<"Tile: "<<(*m_g4hit_energy)[g4hit_i]<<"
          // "<<(*m_g4hit_samplingfraction)[g4hit_i]<<std::endl;
          if ((*m_g4hit_samplingfraction)[g4hit_i]) {
            one_g4hit.hit_energy = (*m_g4hit_energy)[g4hit_i] *
                                   (*m_g4hit_samplingfraction)[g4hit_i];
            // std::cout <<"TileE: "<<one_g4hit.hit_energy<<std::endl;
          } else
            one_g4hit.hit_energy = 0.;
        } else {
          // std::cout <<"LAr: "<<(*m_g4hit_energy)[g4hit_i]<<"
          // "<<(*m_g4hit_samplingfraction)[g4hit_i]<<std::endl;
          one_g4hit.hit_energy =
              (*m_g4hit_energy)[g4hit_i] / (*m_g4hit_samplingfraction)[g4hit_i];
        }
        // one_g4hit.hit_sampfrac = (*m_g4hit_samplingfraction)[g4hit_i];
        // new g4hit -> insert vector with 1 element
        g4hits.insert(std::pair<Long64_t, std::vector<FCS_g4hit> >(
            one_g4hit.cell_identifier, std::vector<FCS_g4hit>(1, one_g4hit)));
      } else {
        // G4 hit exists in this identifier -> push_back new to the vector
        // //FCS_g4hit one_g4hit;
        one_g4hit.identifier = (*m_g4hit_identifier)[g4hit_i];
        one_g4hit.cell_identifier = (*m_g4hit_cellidentifier)[g4hit_i];
        one_g4hit.sampling = (*m_g4hit_sampling)[g4hit_i];
        one_g4hit.hit_time = (*m_g4hit_time)[g4hit_i];
        if (one_g4hit.sampling >= 12 && one_g4hit.sampling <= 20) {  // tile
          // std::cout <<"Tile2: "<<(*m_g4hit_energy)[g4hit_i]<<"
          // "<<(*m_g4hit_samplingfraction)[g4hit_i]<<std::endl;
          if ((*m_g4hit_samplingfraction)[g4hit_i]) {
            one_g4hit.hit_energy = (*m_g4hit_energy)[g4hit_i] *
                                   (*m_g4hit_samplingfraction)[g4hit_i];
          ...
          // std::cout <<"LAr2: "<<(*m_g4hit_energy)[g4hit_i]<<"
          // "<<(*m_g4hit_samplingfraction)[g4hit_i]<<std::endl;
          one_g4hit.hit_energy =
              (*m_g4hit_energy)[g4hit_i] / (*m_g4hit_samplingfraction)[g4hit_i];
        }
  ...
  for (unsigned int hit_i = 0; hit_i < m_hit_identifier->size(); hit_i++) {
    if ((*m_hit_sampling)[hit_i] >= 0 && (*m_hit_sampling)[hit_i] <= 25 &&
        (*m_hit_time)[hit_i] > m_TimingCut) {
      // if (m_Debug > 1)
      ATH_MSG_DEBUG("Ignoring FCS hit, time too large: "
                    << hit_i << " time: " << (*m_hit_time)[hit_i]);
      continue;
    }
    if (hits.find((*m_hit_cellidentifier)[hit_i]) == hits.end()) {
      ...
      if (one_hit.sampling >= 12 && one_hit.sampling <= 20) {  // tile
        if ((*m_hit_samplingfraction)[hit_i]) {
          one_hit.hit_energy =
              (*m_hit_energy)[hit_i] * (*m_hit_samplingfraction)[hit_i];
        } else
      ...
      hits.insert(std::pair<Long64_t, std::vector<FCS_hit> >(
          one_hit.cell_identifier, std::vector<FCS_hit>(1, one_hit)));
    } else {
  ...
  // Start matching:
  Int_t iindex = 0;
  for (std::map<Long64_t, FCS_cell>::iterator it = cells.begin();
       it != cells.end();) {
    iindex++;
    // std::cout <<iindex<<std::endl;
    one_matchedcell.clear();  // maybe not completely necessery, as we're not
                              // pushing_back into vectors
    // set the cell part
    one_matchedcell.cell = it->second;
    // now look for FCS detailed hits in this cell
    std::map<Long64_t, std::vector<FCS_hit> >::iterator it2 =
        hits.find(it->first);
    if (it2 != hits.end()) {
    ...
    std::map<Long64_t, std::vector<FCS_g4hit> >::iterator it3 =
        g4hits.find(it->first);
    if (it3 != g4hits.end()) {
  ...
  // There could be G4hits/FCS hits for which we don't have a cell ->create a
  // dummy empty cell with 0 energy, take the cell identifier from the hit
  ATH_MSG_DEBUG("ISF_HitAnalysis Check after cells: "
                << cells.size() << " " << g4hits.size() << " " << hits.size());

  for (std::map<Long64_t, std::vector<FCS_hit> >::iterator it = hits.begin();
       it != hits.end();) {
  ...
  ATH_MSG_DEBUG("ISF_HitAnalysis Check after hits: "
                << cells.size() << " " << g4hits.size() << " " << hits.size());
  for (std::map<Long64_t, std::vector<FCS_g4hit> >::iterator it =
           g4hits.begin();
       it != g4hits.end();) {
  ...
  for (int i = 0; i < MAX_LAYER; i++) {
    ...
    for (unsigned int cellindex = 0; cellindex < m_layercells[i]->size();
         cellindex++) {
      if (i != MAX_LAYER - 1) {
        m_final_cell_energy->at(i) +=
            m_layercells[i]->m_vector.at(cellindex).cell.energy;
        m_total_cell_e += m_layercells[i]->m_vector.at(cellindex).cell.energy;
      } else {
        // don't add the energy in the invalid layer to the total energy (if
        // there is any (shouldn't)
        m_final_cell_energy->at(i) +=
            m_layercells[i]
                ->m_vector.at(cellindex)
                .cell.energy;  // this should be here anyway
      }

      // sum energy of all FCS detailed hits in this layer/cell
      for (unsigned int j = 0;
           j < m_layercells[i]->m_vector.at(cellindex).hit.size(); j++) {
        if (i != MAX_LAYER - 1) {
          m_total_hit_e +=
              m_layercells[i]->m_vector.at(cellindex).hit[j].hit_energy;
          m_final_hit_energy->at(i) +=
              m_layercells[i]->m_vector.at(cellindex).hit[j].hit_energy;
        } else {
      ...
      for (unsigned int j = 0;
           j < m_layercells[i]->m_vector.at(cellindex).g4hit.size(); j++) {
        if (i != MAX_LAYER - 1) {
          m_total_g4hit_e +=
              m_layercells[i]->m_vector.at(cellindex).g4hit[j].hit_energy;
          m_final_g4hit_energy->at(i) +=
              m_layercells[i]->m_vector.at(cellindex).g4hit[j].hit_energy;
        } else {
...
std::vector<Trk::HitInfo>* ISF_HitAnalysis::caloHits(
    const HepMC::GenParticle& part) const {
  // Start calo extrapolation
  ATH_MSG_DEBUG("[ fastCaloSim transport ] processing particle "
                << part.pdg_id());
  ...
  Trk::ParticleHypothesis pHypothesis =
      m_pdgToParticleHypothesis.convert(pdgId, charge);
  ...
  if (vtx) {
    ...
    pos = Amg::Vector3D(vtx->point3d().x(), vtx->point3d().y(),
                        vtx->point3d().z());
  }

  Amg::Vector3D mom(part.momentum().x(), part.momentum().y(),
                    part.momentum().z());
  ATH_MSG_DEBUG(
      "[ fastCaloSim transport ] starting transport from position eta="
      << pos.eta() << " phi=" << pos.phi() << " d=" << pos.mag()
      << " pT=" << mom.perp());
  ...
  // double freepath = ( !m_particleDecayHelper.empty()) ?
  // m_particleDecayHelper->freePath(isp) : - 1.;
  double freepath = -1.;
  // ATH_MSG_VERBOSE( "[ fatras transport ] Particle free path : " << freepath);
  // path limit -> time limit  ( TODO : extract life-time directly from decay
  // helper )
  ...
  */
  ...
  //    ATH_MSG_VERBOSE( "[ fatras transport ] Decay is triggered for input
  // particle.");
  ...
  // if (absPdg!=999 && pHypothesis<99) pathLim =
  // m_samplingTool->sampleProcess(mom,isp.charge(),pHypothesis);
  ...
  if (!m_caloEntrance) {
    m_caloEntrance =
        m_extrapolator->trackingGeometry()->trackingVolume(m_caloEntranceName);
    if (!m_caloEntrance)
  ...
  if (m_caloEntrance && m_caloEntrance->inside(pos, 0.001) &&
      !m_extrapolator->trackingGeometry()->atVolumeBoundary(pos, m_caloEntrance,
                                                            0.001)) {
    std::vector<Trk::HitInfo>* dummyHitVector = 0;
    if (charge == 0) {
      caloEntry = m_extrapolator->transportNeutralsWithPathLimit(
          inputPar, pathLim, timeLim, Trk::alongMomentum, pHypothesis,
          dummyHitVector, nextGeoID, m_caloEntrance);
    } else {
      caloEntry = m_extrapolator->extrapolateWithPathLimit(
          inputPar, pathLim, timeLim, Trk::alongMomentum, pHypothesis,
          dummyHitVector, nextGeoID, m_caloEntrance);
    }
  } else
  ...
  if (caloEntry) {
    ...
    hitVector->push_back(
        Trk::HitInfo(caloEntry->clone(), timeLim.time, nextGeoID, 0.));
    ...
    if (charge == 0) {
      eParameters = m_extrapolator->transportNeutralsWithPathLimit(
          *caloEntry, pathLim, timeLim, Trk::alongMomentum, pHypothesis,
          hitVector, nextGeoID);
    } else {
      eParameters = m_extrapolator->extrapolateWithPathLimit(
          *caloEntry, pathLim, timeLim, Trk::alongMomentum, pHypothesis,
          hitVector, nextGeoID);
    }
    ...
    if (eParameters)
      hitVector->push_back(
          Trk::HitInfo(eParameters, timeLim.time, nextGeoID, 0.));
  ...
  if (msgLvl(MSG::DEBUG)) {
    ...
    while (it < hitVector->end()) {
      ...
      ATH_MSG_DEBUG(" HIT: layer=" << sample << " sample=" << sample - 3000
                                   << " eta=" << hitPos.eta() << " phi="
                                   << hitPos.phi() << " d=" << hitPos.mag());
      it++;
    }
  }
...
void ISF_HitAnalysis::minmaxeta(int sample, double eta, double& mineta,
                                double& maxeta) const {
