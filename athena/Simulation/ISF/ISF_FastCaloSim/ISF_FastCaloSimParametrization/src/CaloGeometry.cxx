...
CaloGeometry::CaloGeometry()
  ...
  for (unsigned int i = CaloSampling::PreSamplerB; i <= CaloSampling::FCAL2;
    ...
    m_isCaloBarrel[i] = (CaloSampling::barrelPattern() &
                         CaloSampling::getSamplingPattern(s)) != 0;
  }
...
void CaloGeometry::post_process(int sampling) {
  ...
  for (unsigned int j = 0; j < m_cells_in_regions[sampling].size(); ++j) {
    ...
    "<<j<<"/"<<m_cells_in_regions[sampling].size();
    ...
    */
    for (unsigned int i = j + 1; i < m_cells_in_regions[sampling].size(); ++i) {
      if (m_cells_in_regions[sampling][i]
              ->has_overlap(m_cells_in_regions[sampling][j])) {
        if (!found_overlap) {
          cout << "Sample " << sampling
               << ", starting maps : " << m_cells_in_regions[sampling].size();
          for (unsigned int k = 0; k < m_cells_in_regions[sampling].size();
               ++k) {
            cout << ", " << k << ":" << m_cells_in_regions[sampling][k]->size();
          }
          cout << endl;
        ...
        */
        ...
        m_cells_in_regions[sampling]
            .resize(m_cells_in_regions[sampling].size() - 1);
...
void CaloGeometry::InitRZmaps() {
  ...
  for (t_cellmap::iterator calo_iter = m_cells.begin();
       calo_iter != m_cells.end(); ++calo_iter) {
    ...
    if (theDDE) {
      ...
      // sampling...
      // Should most likely handle the situation nicely rather than with a
      // crash.
    ...
    for (int sample = 0; sample < MAX_SAMPLING; ++sample) {
      ...
      if (rz_map_n[side][sample].size() > 0) {
        for (FSmap<double, int>::iterator iter = rz_map_n[side][sample].begin();
             iter != rz_map_n[side][sample].end(); ++iter) {
  ...
  if (DoGraphs()) {
    for (int sample = 0; sample < MAX_SAMPLING; ++sample) {
      ...
      m_graph_layers[sample]
          ->SetMarkerColor(TMath::Abs(CaloGeometry_calocol[sample]));
      m_graph_layers[sample]
          ->SetLineColor(TMath::Abs(CaloGeometry_calocol[sample]));
      int np = 0;
      for (int side = 0; side <= 1; ++side) {
        for (FSmap<double, int>::iterator iter = rz_map_n[side][sample].begin();
             iter != rz_map_n[side][sample].end(); ++iter) {
          ...
          */
          m_graph_layers[sample]
              ->SetPointError(np, (zext - zmid) * sign_side, rext - rmid);
          ++np;
        }
      }
    }
...
TGraph* CaloGeometry::DrawGeoSampleForPhi0(int sample, int calocol,
                                           bool print) {
  TGraph* firstgr = 0;
...
TCanvas* CaloGeometry::DrawGeoForPhi0() {
  ...
  TH2D* hcalolayout =
      new TH2D("hcalolayoutPhi0",
               "Reconstruction geometry: calorimeter layout;z [mm];r [mm]", 50,
               -7000, 7000, 50, 0, 4000);
  hcalolayout->Draw();
  ...
  for (int sample = 0; sample < MAX_SAMPLING; ++sample) {
    TGraph* gr = DrawGeoSampleForPhi0(sample, CaloGeometry_calocol[sample],
                                      (sample == 17));
    if (gr) {
...
const CaloDetDescrElement* CaloGeometry::getDDE(int sampling, float eta,
      ...
      for (unsigned int j = 0; j < m_cells_in_regions[sampling].size(); ++j) {
        ...
        const CaloDetDescrElement* newDDE =
            m_cells_in_regions[sampling][j]
                ->getDDE(eta, phi, &newdist, &intsteps);
        if (m_debug) {
...
const CaloDetDescrElement* CaloGeometry::getFCalDDE(int sampling, float x,
                                                    float y, float z,
                                                    float* distance,
                                                    int* steps) {
...
bool CaloGeometry::getClosestFCalCellIndex(int sampling, float x, float y,
                                           int& ieta, int& iphi, int* steps) {
...
bool CaloGeometry::PostProcessGeometry() {
  ...
  "<<m_cells_in_sampling[sampling].size()<<" cells";
...
void CaloGeometry::minmaxeta(int sample, double eta, double& mineta,
                             double& maxeta) const {
  int side = 0;
  if (eta > 0) side = 1;
...
bool CaloGeometry::checkFCalGeometryConsistency() {
  ...
  for (int imap = 1; imap <= 3; imap++) {
    ...
    if ((int)m_cells_in_sampling[sampling].size() !=
        2 * std::distance(m_FCal_ChannelMap.begin(imap),
