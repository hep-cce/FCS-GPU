...
CaloGeometryLookup::CaloGeometryLookup(int ind)
    : m_xy_grid_adjustment_factor(0.75), m_index(ind) {
  m_mineta = +10000;
  m_maxeta = -10000;
  m_minphi = +10000;
  m_maxphi = -10000;
...
void CaloGeometryLookup::post_process() {
  ...
  if (sampling < 21) {
    ...
    //  cout<<"Grid: Sampling "<<sampling<<"_"<<index()<<": mapping cells into a
    // regular grid, although cells don't fit well"<<endl;
    //}
  ...
  for (ic = m_cells.begin(); ic != m_cells.end(); ++ic) {
    ...
    if (index_range_adjust(ieta, iphi)) {
      if (m_cell_grid[ieta][iphi]) {
        ...
        cout << "    x=" << m_cells[m_cell_grid[ieta][iphi]->identify()]
                                ->x_raw() << " -> " << refcell->x_raw();
  ...
  for (int ieta = 0; ieta < m_cell_grid_eta; ++ieta) {
    for (int iphi = 0; iphi < m_cell_grid_phi; ++iphi) {
      if (!m_cell_grid[ieta][iphi]) {
        ++nempty;
        // cout<<"Sampling "<<sampling<<"_"<<index()<<": No cell at pos
        // ("<<ieta<<","<<iphi<<")"<<endl;
      } else {
        ++ncells;
      }
  ...
  //  cout<<"Grid: Sampling "<<sampling<<"_"<<index()<<":
  // "<<ncells<<"/"<<size()<<" cells filled, "<<nempty<<" empty grid positions
  // deta="<<m_deta_double<<" dphi="<<m_dphi_double<<endl;
}

float CaloGeometryLookup::calculate_distance_eta_phi(
    const CaloDetDescrElement* DDE, float eta, float phi, float& dist_eta0,
    float& dist_phi0) {
...
const CaloDetDescrElement* CaloGeometryLookup::getDDE(float eta, float phi,
                                                      float* distance,
                                                      int* steps) {
  float dist;
