...
StatusCode FastCaloSimCaloExtrapolation::initialize() {
  ...
  ATH_MSG_INFO("m_CaloBoundaryR=" << m_CaloBoundaryR << " m_CaloBoundaryZ="
                                  << m_CaloBoundaryZ << " m_caloEntranceName "
                                  << m_caloEntranceName);
...
void FastCaloSimCaloExtrapolation::extrapolate(TFCSExtrapolationState& result,
  ...
  //////////////////////////////////////
  ATH_MSG_DEBUG(
      "FastCaloSimCaloExtrapolation::extrapolate:*** Get calo surface ***");
  get_calo_surface(result, hitVector);

  ATH_MSG_DEBUG(
      "FastCaloSimCaloExtrapolation::extrapolate:*** Do extrapolation to "
      "ID-calo boundary ***");
  extrapolate_to_ID(result, truth, hitVector);

  ATH_MSG_DEBUG(
      "FastCaloSimCaloExtrapolation::extrapolate:*** Do extrapolation ***");
  extrapolate(result, truth, hitVector);

  ATH_MSG_DEBUG(
      "FastCaloSimCaloExtrapolation::extrapolate: Truth extrapolation done");

  for (std::vector<Trk::HitInfo>::iterator it = hitVector->begin();
       it < hitVector->end(); ++it) {
    if ((*it).trackParms) {
...
std::vector<Trk::HitInfo>* FastCaloSimCaloExtrapolation::caloHits(
  ...
  Amg::Vector3D pos = Amg::Vector3D(truth->vertex().X(), truth->vertex().Y(),
                                    truth->vertex().Z());

  Amg::Vector3D mom(truth->X(), truth->Y(), truth->Z());

  ATH_MSG_DEBUG("[ fastCaloSim transport ] x from position eta="
...
void FastCaloSimCaloExtrapolation::extrapolate(
  ...
  if (fabs(result.IDCaloBoundary_eta()) < 6) {
    ...
    ATH_MSG_DEBUG("Calo position for particle id "
                  << pdgid << ", trutheta= " << ptruth_eta
      ...
      for (int subpos = SUBPOS_MID; subpos <= SUBPOS_EXT; ++subpos) {
        if (get_calo_etaphi(result, hitVector, sample, subpos))
          ATH_MSG_DEBUG("Result in sample "
                        ...
                        << " (ok=" << result.OK(sample, subpos) << ")");
        else
          ATH_MSG_DEBUG("Extrapolation to sample "
                        << sample << " failed (ok=" << result.OK(sample, subpos)
                        << ")");
      }  // for pos
    }  // for sample
  }  // inside calo
  else
    ATH_MSG_WARNING("Ups. Not inside calo. result.IDCaloBoundary_eta()="
                    << result.IDCaloBoundary_eta());

  ATH_MSG_DEBUG("End extrapolate()");
}

void FastCaloSimCaloExtrapolation::extrapolate_to_ID(
    TFCSExtrapolationState& result, const TFCSTruthState* /*truth*/,
    std::vector<Trk::HitInfo>* hitVector) {
  ...
  for (unsigned int i = 0; i < m_CaloBoundaryR.size(); ++i) {
    Amg::Vector3D hitpos;
    Amg::Vector3D hitmom;
    if (rz_cylinder_get_calo_etaphi(hitVector, m_CaloBoundaryR[i],
                                    m_CaloBoundaryZ[i], hitpos, hitmom)) {
      // test if z within previous cylinder
      ATH_MSG_DEBUG("BOUNDARY ID-CALO r="
                    << m_CaloBoundaryR[i] << " z=" << m_CaloBoundaryZ[i]
                    << ": eta=" << hitpos.eta() << " phi=" << hitpos.phi()
                    << " r=" << hitpos.perp() << " z=" << hitpos[Amg::z]
                    << " theta=" << hitpos.theta()
                    << " ; momentum eta=" << hitmom.eta()
      ...
      // test if previous found cylinder crossing is closer to (0,0,0) than this
      // crossing
      if (result_dist >= 0) {
      ...
      ATH_MSG_DEBUG("BOUNDARY ID-CALO r=" << m_CaloBoundaryR[i] << " z="
                                          << m_CaloBoundaryZ[i] << " accepted");
    }
  }
  ...
  if (result_dist < 0) {
    ...
    ATH_MSG_DEBUG("FINAL BOUNDARY ID-CALO  eta="
                  << result_hitpos.eta() << " phi=" << result_hitpos.phi()
                  << " r=" << result_hitpos.perp() << " z="
                  << result_hitpos[Amg::z] << " theta=" << result_hitpos.theta()
                  << " ; momentum eta=" << result_hitmom.eta() << " phi="
                  << result_hitmom.phi() << " theta=" << result_hitmom.theta());
  }

  TVector3 vec(result_hitpos[Amg::x], result_hitpos[Amg::y],
               result_hitpos[Amg::z]);

  // get the tangentvector on this interaction point:
  // GlobalMomentum*
  // mom=params_on_surface_ID->TrackParameters::momentum().unit() ;
  // Trk::GlobalMomentum*
  // trackmom=params_on_surface_ID->Trk::TrackParameters::momentum();
  if (result_hitmom.mag() > 0) {
    // angle between vec and trackmom:
    TVector3 Trackmom(result_hitmom[Amg::x], result_hitmom[Amg::y],
                      result_hitmom[Amg::z]);
    double angle3D =
        Trackmom.Angle(vec);  // isn't this the same as TVector3 vec?
...
bool FastCaloSimCaloExtrapolation::get_calo_surface(
    TFCSExtrapolationState& result, std::vector<Trk::HitInfo>* hitVector) {
  ...
  if (result.CaloSurface_sample() == CaloCell_ID_FCS::noSample) {
    ...
    if (TMath::Abs(result.CaloSurface_eta()) > 4.9 || pT < 500 ||
      ...
      ATH_MSG_DEBUG(
          "only entrance to calo entrance layer found, no surface : eta="
          << result.CaloSurface_eta() << " phi=" << result.CaloSurface_phi()
          << " r=" << result.CaloSurface_r() << " z=" << result.CaloSurface_z()
          << " pT=" << pT);
    else
      ATH_MSG_WARNING(
          "only entrance to calo entrance layer found, no surface : eta="
    ...
    ATH_MSG_DEBUG("entrance to calo surface : sample="
                  << result.CaloSurface_sample()
                  << " eta=" << result.CaloSurface_eta() << " phi="
                  << result.CaloSurface_phi() << " r=" << result.CaloSurface_r()
                  << " z=" << result.CaloSurface_z()
                  << " deta=" << min_calo_surf_dist);
  }
...
// UPDATED
bool FastCaloSimCaloExtrapolation::get_calo_etaphi(
    TFCSExtrapolationState& result, std::vector<Trk::HitInfo>* hitVector,
    int sample, int subpos) {
  ...
  if (it != hitVector->end()) {
    ...
    ATH_MSG_DEBUG("extrapol with layer hit for sample="
                  << sample << " : id=" << sid1 << " -> " << sid2
                  << " target r/z=" << best_target << " r1=" << hitPos1.perp()
                  << " z1=" << hitPos1[Amg::z] << " r2=" << hitPos2.perp()
                  << " z2=" << hitPos2[Amg::z]
                  << " result.r=" << result.r(sample, subpos)
  ...
  if (!best_found) {
    ...
    while (it < hitVector->end() - 1) {
      ...
      ATH_MSG_DEBUG("extrapol without layer hit for sample="
                    << sample << " : id=" << sid1 << " -> " << sid2
                    << " dist=" << dist << " mindist=" << best_dist
                    << " t=" << t << " best_inside=" << best_inside
                    << " target r/z=" << tmp_target << " r1=" << hitPos1.perp()
                    << " z1=" << hitPos1[Amg::z] << " r2=" << hitPos2.perp()
                    << " z2=" << hitPos2[Amg::z] << " re=" << hitPos.perp()
                    << " ze=" << hitPos[Amg::z] << " rb=" << best_hitPos.perp()
    ...
    if (best_found) {
      ATH_MSG_DEBUG(
          "extrapol without layer hit: id="
          << best_id1 << " -> " << best_id2 << " mindist=" << best_dist
  ...
  ATH_MSG_DEBUG("Final TTC result for sample "
                << sample << " subpos=" << subpos << " OK() "
                << result.OK(sample, subpos)
                << " eta=" << result.eta(sample, subpos)
                << " phi=" << result.phi(sample, subpos) << " dCalo="
                << result.d(sample, subpos) << " dist(hit)=" << hitdist);

  return result.OK(sample, subpos);
}

// UPDATED
bool FastCaloSimCaloExtrapolation::rz_cylinder_get_calo_etaphi(
    std::vector<Trk::HitInfo>* hitVector, double cylR, double cylZ,
    Amg::Vector3D& pos, Amg::Vector3D& mom) {
  ...
  for (int rz = 0; rz <= 1; ++rz) {
    ...
    while (it < hitVector->end() - 1) {
      ...
      if (t >= down_frac && t <= up_frac) {
        if (hitPos.perp() <= cylR * up_frac &&
            fabs(hitPos[Amg::z]) <= cylZ * up_frac)
          inside = true;
      }
      ...
      ATH_MSG_DEBUG(" extrapol without layer hit to r="
                    << cylR << " z=+-" << cylZ << " : id=" << sid1 << " -> "
                    << sid2 << " dist=" << dist << " bestdist=" << best_dist
                    << " t=" << t << " inside=" << inside << " best_inside="
                    << best_inside << " r1=" << hitPos1.perp()
                    << " z1=" << hitPos1[Amg::z] << " r2=" << hitPos2.perp()
                    << " z2=" << hitPos2[Amg::z] << " re=" << hitPos.perp()
                    << " ze=" << hitPos[Amg::z] << " rb=" << best_hitPos.perp()
                    << " zb=" << best_hitPos[Amg::z]);
  ...
  if (best_found) {
    ATH_MSG_DEBUG(" extrapol to r=" << cylR << " z=" << cylZ
                                    << ": id=" << best_id1 << " -> " << best_id2
                                    << " dist=" << best_dist
...
void FastCaloSimCaloExtrapolation::minmaxeta(int sample, double eta,
                                             double& mineta,
                                             double& maxeta) const {
...
double FastCaloSimCaloExtrapolation::rpos(int sample, double eta,
                                          int subpos) const {
  return GetCaloGeometry()->rpos(sample, eta, subpos);
}

double FastCaloSimCaloExtrapolation::zpos(int sample, double eta,
                                          int subpos) const {
  return GetCaloGeometry()->zpos(sample, eta, subpos);
}

double FastCaloSimCaloExtrapolation::rzpos(int sample, double eta,
                                           int subpos) const {
