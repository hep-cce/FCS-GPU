...
FastCaloSimParamAlg::FastCaloSimParamAlg(const std::string& name,
  ...
  declareProperty("Truncate", m_truncate = 2,
                  "truncate hits with t>1000ns (if >=2)");
  declareProperty(
      "MaxDistance", m_maxDistance = 50000.,
  ...
  declareProperty(
      "MaxRadiusLAr", m_maxRadiusLAr = 25.,
      "maximal radius in LAr squared until two hits will be combined");
  declareProperty(
      "MaxRadiusHEC", m_maxRadiusHEC = 100.,
      "maximal radius in HEC squared until two hits will be combined");
  declareProperty(
      "MaxRadiusFCAL", m_maxRadiusFCAL = 100.,
      "maximal radius in FCAL squared until two hits will be combined");
  declareProperty(
      "MaxRadiusTile", m_maxRadiusTile = 100.,
      "maximal radius in Tile squared until two hits will be combined");

  declareProperty("MaxTime", m_maxTime = 25.,
                  "max time difference to merge two hits (ns) ");
  declareProperty("MaxTimeLAr", m_maxTimeLAr = 25.,
                  "max time difference to merge two hits (ns) ");
  declareProperty("MaxTimeHEC", m_maxTimeHEC = 25.,
                  "max time difference to merge two hits (ns) ");
  declareProperty("MaxTimeFCAL", m_maxTimeFCAL = 25.,
                  "max time difference to merge two hits (ns) ");
  declareProperty("MaxTimeTile", m_maxTimeTile = 25.,
                  "max time difference to merge two hits (ns) ");
  ...
  declareProperty("EnergyFraction", m_energyFraction = .02,
                  "the allowed amount of energy that can be deposited outside "
                  "calorimeter region ");
}

StatusCode FastCaloSimParamAlg::initialize() {
  ATH_MSG_DEBUG("Initializing " << this->name() << " - package version "
                                << PACKAGE_VERSION);
...
StatusCode FastCaloSimParamAlg::execute() {
  SG::ReadHandle<ISF_FCS_Parametrization::FCS_StepInfoCollection> inputHandle{
      m_inputCollectionKey};
  SG::WriteHandle<ISF_FCS_Parametrization::FCS_StepInfoCollection> outputHandle{
      m_outputCollectionKey};
  ATH_CHECK(outputHandle.record(
      std::make_unique<ISF_FCS_Parametrization::FCS_StepInfoCollection>()));
  ...
  for (const auto& step : *inputHandle) {
    auto&& stepCopy =
        std::make_unique<ISF_FCS_Parametrization::FCS_StepInfo>(*step);
...
StatusCode FastCaloSimParamAlg::clusterize(
    ISF_FCS_Parametrization::FCS_StepInfoCollection* stepinfo) const {
  ATH_MSG_DEBUG("Initial clusterize size: " << stepinfo->size()
                                            << " - will merge steps in the "
                                               "same cell which are less than "
                                               "dR and dT to each other");
  ...
  std::map<Identifier, ISF_FCS_Parametrization::FCS_StepInfoCollection*>
      FCS_SIC_cells;
  for (const auto& step : *stepinfo) {
    if (FCS_SIC_cells.find(step->identify()) !=
        FCS_SIC_cells.end()) {  // Already have a step for this cell
      auto&& stepCopy =
          std::make_unique<ISF_FCS_Parametrization::FCS_StepInfo>(*step);
      ...
      FCS_SIC_cells.insert(std::pair<
          Identifier, ISF_FCS_Parametrization::FCS_StepInfoCollection*>(
  ...
  for (std::map<Identifier,
                ISF_FCS_Parametrization::FCS_StepInfoCollection*>::iterator it =
           FCS_SIC_cells.begin();
       it != FCS_SIC_cells.end(); ++it) {
    std::stable_sort(FCS_SIC_cells[it->first]->begin(),
                     FCS_SIC_cells[it->first]->end(), SortByE());
    ...
    const CaloCell_ID::CaloSample layer =
        m_calo_dd_man->get_element(it->first)->getSampling();
    double dsame(0.);
    double tsame(0.);
    if (layer >= CaloCell_ID::PreSamplerB && layer <= CaloCell_ID::EME3) {
      ...
      tsame = m_maxTimeHEC;
    } else if (layer >= CaloCell_ID::TileBar0 &&
               layer <= CaloCell_ID::TileExt2) {
    ...
    ISF_FCS_Parametrization::FCS_StepInfoCollection::iterator it1 =
        (it->second)->begin();
    while (it1 != (it->second)->end()) {
      ...
      while (it2 != (it->second)->end()) {
        if (((*it1)->diff2(**it2) < dsame) &&
            std::fabs((*it1)->time() - (*it2)->time()) < tsame) {
          **it1 += **it2;
          it2 = (it->second)->erase(it2);  // also calls delete on the pointer
                                           // (because the DataVector owns its
                                           // elements)
          continue;
        }
        ++it2;
  ...
  stepinfo->clear();  // also calls delete on all the removed elements (because
                      // the DataVector owns its elements)
  for (std::map<Identifier,
                ISF_FCS_Parametrization::FCS_StepInfoCollection*>::iterator it =
           FCS_SIC_cells.begin();
       it != FCS_SIC_cells.end(); ++it) {
    ...
    it->second->clear();  // also calls delete on all the removed elements
                          // (because the DataVector owns its elements)
    delete (it->second);
  }
  ...
  ISF_FCS_Parametrization::FCS_StepInfoCollection::iterator stepIter =
      stepinfo->begin();
  while (stepIter != stepinfo->end()) {
    ...
    ++nInvalid;
    stepIter = stepinfo->erase(stepIter);  // also calls delete on the pointer
                                           // (because the DataVector owns its
                                           // elements)
  }
  ATH_MSG_DEBUG("Removed " << nInvalid
                           << " StepInfo objects. New collection size: "
                           << stepinfo->size());
  return StatusCode::SUCCESS;
}

StatusCode FastCaloSimParamAlg::truncate(
    ISF_FCS_Parametrization::FCS_StepInfoCollection* stepinfo) const {
  ATH_MSG_DEBUG("Initial truncate size: " << stepinfo->size()
                                          << " settings: " << m_truncate);
  if (m_truncate > 0) {
    ISF_FCS_Parametrization::FCS_StepInfoCollection::iterator stepIter =
        stepinfo->begin();
    while (stepIter != stepinfo->end()) {
      if ((m_truncate >= 2) && ((*stepIter)->time() > 1000)) {
        stepIter = stepinfo->erase(stepIter);  // also calls delete on the
                                               // pointer (because the
                                               // DataVector owns its elements)
        continue;
      }
      ++stepIter;
    }
