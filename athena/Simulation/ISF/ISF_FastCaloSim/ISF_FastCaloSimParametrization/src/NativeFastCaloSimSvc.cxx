...
/** Constructor **/
ISF::NativeFastCaloSimSvc::NativeFastCaloSimSvc(const std::string& name,
                                                ISvcLocator* svc)
  ...
  declareProperty(
      "SimulateUndefinedBarcodeParticles", m_simulateUndefinedBCs,
  ...
  declareProperty(
      "BatchProcessMcTruth", m_batchProcessMcTruth = false,
...
StatusCode ISF::NativeFastCaloSimSvc::initialize() {
  ...
  if (retrieveTools<ICaloCellMakerTool>(m_caloCellMakerTools_simulate)
          .isFailure())
    return StatusCode::FAILURE;
  if (retrieveTools<ICaloCellMakerTool>(m_caloCellMakerTools_release)
          .isFailure())
    return StatusCode::FAILURE;

  if (m_doPunchThrough && m_punchThroughTool.retrieve().isFailure()) {
    ATH_MSG_ERROR(m_punchThroughTool.propertyName()
                  << ": Failed to retrieve tool " << m_punchThroughTool.type());
    return StatusCode::FAILURE;
  }

  ATH_MSG_DEBUG(m_screenOutputPrefix << " Output CaloCellContainer Name "
  ...
  if (m_caloCellHack) {
    ATH_MSG_WARNING(
        ...
        << " CaloCellContainer: " << m_caloCellsOutputName
        << "will be read in and modified !. To be used with care. ");
  }

  if (m_particleBroker.retrieve().isFailure()) {
    ATH_MSG_FATAL("Could not retrieve ISF ParticleBroker service "
                  << m_particleBroker);
    return StatusCode::FAILURE;
  }
...
StatusCode ISF::NativeFastCaloSimSvc::setupEvent() {
  ...
  if (!m_caloCellHack) {
    ...
    m_theContainer =
        new CaloCellContainer(static_cast<SG::OwnershipPolicy>(m_ownPolicy));

    StatusCode sc = StatusCode::SUCCESS;
    sc = evtStore()->record(m_theContainer, m_caloCellsOutputName);

    if (sc.isFailure()) {
      ATH_MSG_FATAL(m_screenOutputPrefix << "cannot record CaloCellContainer "
                                         << m_caloCellsOutputName);
      return StatusCode::FAILURE;
    }
    ...
    if (sc.isFailure()) {
      ATH_MSG_WARNING(m_screenOutputPrefix << "Error symlinking "
                                              "CaloCellContainer to "
    ...
    if (sc.isFailure() || theConstContainer == 0) {
      ATH_MSG_FATAL(
          m_screenOutputPrefix << "Could not retrieve CaloCellContainer "
                               << m_caloCellsOutputName);
      return StatusCode::FAILURE;
    }
  ...
  // loop on setup tools
  ToolHandleArray<ICaloCellMakerTool>::iterator itrTool =
      m_caloCellMakerTools_setup.begin();
  ToolHandleArray<ICaloCellMakerTool>::iterator endTool =
      m_caloCellMakerTools_setup.end();
    ...
    if (m_chrono) {
      ...
      ATH_MSG_DEBUG(m_screenOutputPrefix
                    ...
                    << m_chrono->chronoDelta(chronoName, IChronoStatSvc::USER) *
                           CLHEP::microsecond / CLHEP::second << " second ");
    }

    if (sc.isFailure()) {
      ATH_MSG_ERROR(m_screenOutputPrefix << "Error executing tool "
                                         << itrTool->name());
    }
  }
  ...
  for (;itrTool!=endTool;++itrTool) {
    FastShowerCellBuilderTool* fcs=dynamic_cast< FastShowerCellBuilderTool*
  >(&(*(*itrTool)));
    if(fcs) {
      if(fcs->setupEvent().isFailure()) {
        ATH_MSG_ERROR( m_screenOutputPrefix << "Error executing tool " <<
  itrTool->name() << " in setupEvent");
        return StatusCode::FAILURE;
      }
    }
  }
...
StatusCode ISF::NativeFastCaloSimSvc::releaseEvent() {
  ...
  if (m_batchProcessMcTruth) {
    ...
    //        (in Python they should be configured to pick up the modified truth
    // collection)

    // ZH commented out these to avoid warnings at the moment
    // ToolHandleArray<ICaloCellMakerTool>::iterator
    // itrTool=m_caloCellMakerTools_simulate.begin();
    // ToolHandleArray<ICaloCellMakerTool>::iterator
    // endTool=m_caloCellMakerTools_simulate.end();
    /*
    ...
    >(&(*(*itrTool)));
      if(!fcs) {
        ATH_MSG_WARNING( m_screenOutputPrefix << "tool " << itrTool->name()<<
    "is not a FastShowerCellBuilderTool" );
        continue;
      }

      ATH_MSG_VERBOSE( m_screenOutputPrefix << "Calling tool " <<
    itrTool->name() );

      if( fcs->process(m_theContainer).isFailure()) {
        ATH_MSG_WARNING( m_screenOutputPrefix << "batch simulation of
    FastCaloSim particles failed" );
  ...
  //
  ToolHandleArray<ICaloCellMakerTool>::iterator itrTool =
      m_caloCellMakerTools_simulate.begin();
  ToolHandleArray<ICaloCellMakerTool>::iterator endTool =
      m_caloCellMakerTools_simulate.end();
  /*
  for (;itrTool!=endTool;++itrTool) {
    FastShowerCellBuilderTool* fcs=dynamic_cast< FastShowerCellBuilderTool*
  >(&(*(*itrTool)));
...
StatusCode ISF::NativeFastCaloSimSvc::simulate(const ISF::ISFParticle& isfp) {
  ...
  if (m_doPunchThrough) {
    ...
    const ISF::ISFParticleContainer* isfpVec =
        m_punchThroughTool->computePunchThroughParticles(isfp);
  ...
  if (m_batchProcessMcTruth) {
    ATH_MSG_DEBUG(
        "particle is ignored now, will run Calo simulation using ID McTruth at "
        "the end of the event");
    return StatusCode::SUCCESS;
  }
  // (b.) throw away particles with undefined Barcode if
  // m_simulateUndefinedBCs==False
...
StatusCode ISF::NativeFastCaloSimSvc::processOneParticle(
    const ISF::ISFParticle& isfp) {
  ATH_MSG_VERBOSE(
      m_screenOutputPrefix << "Simulating pdgid = " << isfp.pdgCode());
  ...
  >(&(*(*itrTool)));
    if(!fcs) {
      ATH_MSG_WARNING( m_screenOutputPrefix << "tool " << itrTool->name()<< "is
  not a FastShowerCellBuilderTool" );
      continue;
    }
    ...
    HepMC::FourVector
  momentum(isfp.momentum().x(),isfp.momentum().y(),isfp.momentum().z(), sqrt(
  isfp.mass()*isfp.mass() + isfp.momentum().mag2()) );
    HepMC::GenParticle* part=new HepMC::GenParticle(momentum,isfp.pdgCode());
    HepMC::FourVector
  position(isfp.position().x(),isfp.position().y(),isfp.position().z(),isfp.timeStamp());
    HepMC::GenVertex vertex(position);
    vertex.add_particle_out( part ); // HepMC::GenVertex destructor will delete
  the particle

    //sc = (*itrTool)->process(m_theContainer);
    if(fcs->process_particle(m_theContainer,part,0).isFailure()) {
      ATH_MSG_WARNING( m_screenOutputPrefix << "simulation of particle pdgid="
  << isfp.pdgCode()<< " failed" );
      sc = StatusCode::FAILURE;
    }

    if (m_chrono) m_chrono->chronoStop( chronoName );
    //ATH_MSG_DEBUG( m_screenOutputPrefix << "Chrono stop : delta " <<
  m_chrono->chronoDelta (chronoName,IChronoStatSvc::USER ) * CLHEP::microsecond
  / CLHEP::second << " second " );
  } //end of for-loop
  */
