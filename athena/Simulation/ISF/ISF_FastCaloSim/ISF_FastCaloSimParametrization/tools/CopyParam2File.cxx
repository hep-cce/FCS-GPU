...
/**
  ...
  * -)
  *
  * $ root -l -b -q 'CopyParam2File.cxx("shape.pdgid_211.en_50000.eta_020.root",
  *"EnergyParam", 211, 50000, 0.20)'
  *
 */
#include "TROOT.h"
#include "TRint.h"
...
std::string doSParam(std::string pstr, std::string kstr) {
  ...
  size_t l_endpos =
      kstr.find_last_of(delim2) - (kstr.find(delim1) + delim1.length());
...
std::string doEParam(std::string pstr, std::string kstr, std::string dirstr) {
  ...
  size_t p_endpos =
      dirstr.find_last_of(delim2) - (dirstr.find(delim1) + delim1.length());
  ...
  // Nope -- we don't know where we are!
    ...
    std::cout
        << "ERROR: where are we? (Not a `layer` or `pca`, that's for sure!)"
        << std::endl;
    exit(1);
  }
  ...
  while ((key = (TKey*)nextkey())) {
    ...
    if (cl->InheritsFrom("TDirectory")) {
      ...
      if (param_str.substr(0, 1) == "s")
        ...
        complete_param_str = doEParam(param_str, std::string(key->GetName()),
...
void CopyParam2File(std::string src_str, std::string param_type,
                    unsigned int pdgid, unsigned int energy, float eta,
                    std::string dest_str = "FCSParams.root") {
  // ROOT-aware
  TLorentzVector* t;
  gROOT->LoadMacro(
      "../20.20.X-VAL/Simulation/ISF/ISF_FastCaloSim/ISF_FastCaloSimEvent/src/"
      "IntArray.cxx+");
  gROOT->LoadMacro(
      "../20.20.X-VAL/Simulation/ISF/ISF_FastCaloSim/ISF_FastCaloSimEvent/src/"
      "TFCS1DFunction.cxx+");
  ...
  if (!fdest || fdest->IsZombie()) {
    printf("Cannot open destination file: %s (it doesn't exist?)",
           dest_str.c_str());
    printf("Creating file...");
    fdest = TFile::Open(dest_str.c_str(), "CREATE");
    // Check if the new file creation was a success; if not, exit with an error.
    if (!fdest || fdest->IsZombie()) {
      printf("Cannot create destination file: %s (something went wrong)",
             dest_str.c_str());
      return;
    }
  }
  ...
  if (param_type == "EnergyParam") {
    ...
    std::cout << "Invalid param_type. Valid param_type arguments are "
                 "EnergyParam and ShapeParam." << std::endl;
    exit(1);
  }
  ...
  s_param_prefix.append(".pdgid_" + std::to_string(pdgid) + ".en_" +
                        std::to_string(energy) + ".eta_" + s_eta);
