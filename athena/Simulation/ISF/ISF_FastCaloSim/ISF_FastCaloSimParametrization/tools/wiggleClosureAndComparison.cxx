...
void wiggleClosureAndComparison(TString sampling = "Sampling_0") {
  ...
  geo->LoadGeometryFromFile(
      "/afs/cern.ch/atlas/groups/Simulation/FastCaloSim/"
      "ATLAS-GEO-20-00-01.root",
      "ATLAS-GEO-20-00-01");
  ...
  TFile *originalFile = TFile::Open(
      "/eos/atlas/user/s/schaarsc/FCS/"
      "user.fladias.428137.FastCalo_pid11_E65536_etam35_35_zv_m100.e4001_s2864_"
      "r7736.w0_162706_matched_output.root/"
      "user.fladias.8834798._000001.matched_output.root");
  ...
  */
  ...
  if (sampling == "Sampling_1" || sampling == "Sampling_2" ||
      sampling == "Sampling_3") {
  ...
  for (int i = 0; i < 11; i++) {
    ...
    dEta_dPhi[i] = new TProfile2D("closure_dEtadPhi_Layer_" + layerName,
                                  "closure_dEtadPhi_Layer_" + layerName,
                                  nbins_eta, -0.5, 0.5, nbins_phi, -0.5, 0.5);
    dEta_dPhi_N[i] = new TH2F("closure_dEtadPhi_N_Layer_" + layerName,
                              "closure_dEtadPhi_N_Layer_" + layerName,
                              nbins_eta, -0.5, 0.5, nbins_phi, -0.5, 0.5);

    dEta_dPhi_res[i] = new TH2F("closure_dEtadPhi_res_Layer_" + layerName,
                                "closure_dEtadPhi_res_Layer_" + layerName,
                                nbins_eta, -0.5, 0.5, nbins_phi, -0.5, 0.5);
    dEta_dPhi_N_res[i] = new TH2F("closure_dEtadPhi_N_res_Layer_" + layerName,
                                  "closure_dEtadPhi_N_res_Layer_" + layerName,
                                  nbins_eta, -0.5, 0.5, nbins_phi, -0.5, 0.5);

    dEta_dPhi_avg_reco[i] =
        new TProfile2D("dEta_dPhi_avg_reco_Layer_" + layerName,
                       "dEta_dPhi_avg_reco_Layer_" + layerName, nbins_eta, -0.5,
                       0.5, nbins_phi, -0.5, 0.5);
    dEta_dPhi_avg_g4[i] =
        new TProfile2D("dEta_dPhi_avg_g4_Layer_" + layerName,
                       "dEta_dPhi_avg_g4_Layer_" + layerName, nbins_eta, -0.5,
                       0.5, nbins_phi, -0.5, 0.5);
    dEta_dPhi_FCS_orig[i] =
        new TProfile2D(" dEta_dPhi_FCS_orig_Layer_" + layerName,
                       " dEta_dPhi_FCS_orig_Layer_" + layerName, nbins_eta,
                       -0.5, 0.5, nbins_phi, -0.5, 0.5);
    dEta_dPhi_FCS_matched[i] =
        new TProfile2D("dEta_dPhi_FCS_matched_Layer_" + layerName,
                       "dEta_dPhi_FCS_matched_Layer_" + layerName, nbins_eta,
                       -0.5, 0.5, nbins_phi, -0.5, 0.5);

    dEta_dPhi_FCS_orig_overAvg[i] =
        new TProfile2D("dEta_dPhi_FCS_orig_overAvg_Layer_" + layerName,
                       "dEta_dPhi_FCS_orig_overAvg_reco_Layer_" + layerName,
                       nbins_eta, -0.5, 0.5, nbins_phi, -0.5, 0.5);
    dEta_dPhi_FCS_matched_overAvg[i] =
        new TProfile2D("dEta_dPhi_FCS_matched_overAvg_Layer_" + layerName,
                       "dEta_dPhi_FCS_matched_overAvg_Layer_" + layerName,
                       nbins_eta, -0.5, 0.5, nbins_phi, -0.5, 0.5);

    dEta_dPhi_FCS_orig_overAvg_N[i] =
        new TH2F("dEta_dPhi_FCS_orig_overAvg_N_Layer_" + layerName,
                 "dEta_dPhi_FCS_orig_overAvg_N_reco_Layer_" + layerName,
                 nbins_eta, -0.5, 0.5, nbins_phi, -0.5, 0.5);
    dEta_dPhi_FCS_matched_overAvg_N[i] =
        new TH2F("dEta_dPhi_FCS_matched_overAvg_N_Layer_" + layerName,
                 "dEta_dPhi_FCS_matched_overAvg_N_Layer_" + layerName,
                 nbins_eta, -0.5, 0.5, nbins_phi, -0.5, 0.5);
  }
  ...
  for (Int_t ientry = 0; ientry < nEvt; ientry++) {
    ...
    double trEta = 0.0;
    double trPhi = 0.0;
    // cout << "TTC_entrance_eta " << (*truth)[0].TTC_entrance_eta.size() << " "
    // << (*truth)[0].TTC_entrance_eta[0] << endl;
    // cout << "TTC_entrance_phi " << (*truth)[0].TTC_entrance_phi.size() << " "
    // << (*truth)[0].TTC_entrance_phi[0] << endl;
    // cout << endl;
    ...
    for (UInt_t j = 0; j < (*vec).size(); j++) {
      ...
      // loop over hits in each cell to find the sum of the hit energies in the
      // layer
      for (int ihit = 0; ihit < ((FCS_matchedcell)(*vec)[j]).hit.size();
           ihit++) {
        sum_energy_hit =
            sum_energy_hit + ((FCS_matchedcell)(*vec)[j]).hit[ihit].hit_energy;
      }

      for (int ihit = 0; ihit < ((FCS_matchedcell)(*vec)[j]).g4hit.size();
           ihit++) {
        sum_energy_g4hit = sum_energy_g4hit +
                           ((FCS_matchedcell)(*vec)[j]).g4hit[ihit].hit_energy;
      }
      ...
      Eoriginal.insert(std::pair<Long64_t, double>(
          ((FCS_matchedcell)(*vec)[j]).cell.cell_identifier, sum_energy_hit));
      EoriginalReco.insert(std::pair<Long64_t, double>(
          ((FCS_matchedcell)(*vec)[j]).cell.cell_identifier,
          ((FCS_matchedcell)(*vec)[j]).cell.energy));
      EoriginalG4.insert(std::pair<Long64_t, double>(
          ((FCS_matchedcell)(*vec)[j]).cell.cell_identifier, sum_energy_g4hit));
    }  // end of loop over cells of originalTree
    ///////////////////////

    // NOW make the average shower shapes by looping over all maps
    for (std::map<Long64_t, double>::iterator it = Eoriginal.begin();
         it != Eoriginal.end(); it++) {
    ...
    for (std::map<Long64_t, double>::iterator it = EoriginalReco.begin();
         it != EoriginalReco.end(); it++) {
    ...
    for (std::map<Long64_t, double>::iterator it = EoriginalG4.begin();
         it != EoriginalG4.end(); it++) {
  ...
  for (Int_t ientry = 0; ientry < nEvt; ientry++) {
    ...
    for (UInt_t j = 0; j < (*vec).size(); j++) {
      ...
      for (int ihit = 0; ihit < ((FCS_matchedcell)(*vec)[j]).hit.size();
           ihit++) {
        // sum_energy_hit = sum_energy_hit +
        // ((FCS_matchedcell)(*vec)[j]).hit[ihit].hit_energy;
        ...
        if (Eoriginal.find(someCellID) != Eoriginal.end()) {
          std::map<Long64_t, double>::iterator someit =
              Eoriginal.find(someCellID);
          someit->second += ((FCS_matchedcell)(*vec)[j]).hit[ihit].hit_energy;
        }  // end cells exist
        else {
          Eoriginal.insert(std::pair<Long64_t, double>(
              someCellID, ((FCS_matchedcell)(*vec)[j]).hit[ihit].hit_energy));
        }  // end of else creating new cell
      }  // end hit loop

      // Eoriginal.insert(std::pair<Long64_t,
      // double>(((FCS_matchedcell)(*vec)[j]).cell.cell_identifier,
      // sum_energy_hit));
    }  // loop over cells
    ...
    for (UInt_t j = 0; j < (*vec).size(); j++) {
      ...
      for (int ihit = 0; ihit < ((FCS_matchedcell)(*vec)[j]).hit.size();
        ...
        if (initCell && cell) {
          float efficiencyEta =
              ((2.0 * (hitVec->Eta() - initCell->eta())) / initCell->deta());
          float efficiencyPhi =
              ((2.0 * (hitVec->Phi() - initCell->phi())) / initCell->dphi());

          double searchRand = random1->Rndm(0);
          // cout << searchRand << endl;
          int chosenBin = (Int_t)TMath::BinarySearch(50, eff_int, searchRand);
          // cout << chosenBin << endl;
          // cout << "x axis value:  " << eff_phi->GetBinCenter(chosenBin+2) <<
          // endl;
          // cout << endl;
          ...
          if (hitSampling < 21)
            foundCell =
                geo->getDDE(hitSampling, hitVec->Eta(),
                            (hitVec->Phi()) - (wigglePhi * initCell->dphi()));
          else if (hitSampling < 24)
          ...
          if (Eclosure.find(foundCellID) != Eclosure.end()) {
            std::map<Long64_t, double>::iterator it =
                Eclosure.find(foundCellID);
            // found cell iD: foundCell->identify()
            it->second += ((FCS_matchedcell)(*vec)[j]).hit[ihit].hit_energy;
          }  // end cells exist
          else {
            // create the cell --> OK assigned probabilistically although not
            // really in terms of cell IDs
            Eclosure.insert(std::pair<Long64_t, double>(
                foundCellID,
                ((FCS_matchedcell)(*vec)[j]).hit[ihit].hit_energy));
          }  // end of else creating new cell
        }  // end if cell ok
    ...
    for (std::map<Long64_t, double>::iterator it2 = Eclosure.begin();
         it2 != Eclosure.end(); it2++) {
      ...
      dEta_dPhi_FCS_matched_overAvg[PCAbin]
          ->Fill(dEta, dPhi, it2->second / eRecoPCA);
    ...
    for (std::map<Long64_t, double>::iterator it = Eoriginal.begin();
      ...
      dEta_dPhi_FCS_orig_overAvg[PCAbin]
          ->Fill(dEta, dPhi, it->second / eRecoPCA);
  ...
  for (int i = 0; i < 11; i++) {
    ...
    for (int ki=1; ki<nbins_eta+1; ki++){
      for (int j=1; j<nbins_phi+1; j++){
        ...
        if
    (dEta_dPhi_FCS_matched_overAvg_N[10]->GetBinContent(dEta_dPhi_FCS_matched_overAvg_N[10]->GetBin(ki,j))
    < 10){
          dEta_dPhi_FCS_matched_overAvg[10]->SetBinContent(dEta_dPhi_FCS_matched_overAvg[10]->GetBin(ki,j),
    0.0);
        }

        if
    (dEta_dPhi_FCS_orig_overAvg_N[10]->GetBinContent(dEta_dPhi_FCS_orig_overAvg_N[10]->GetBin(ki,j))
    < 10){
          dEta_dPhi_FCS_orig_overAvg[10]->SetBinContent(dEta_dPhi_FCS_orig_overAvg[10]->GetBin(ki,j),
    0.0);
        }
    ...
    subtraction[i] = (TProfile2D *)dEta_dPhi_FCS_orig_overAvg[i]
                         ->Clone("subtraction_" + sampling);
