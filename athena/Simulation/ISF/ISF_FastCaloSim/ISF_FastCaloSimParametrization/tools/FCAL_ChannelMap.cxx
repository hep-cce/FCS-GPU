...
const double FCAL_ChannelMap::m_tubeSpacing[] = {0.75 * cm, 0.8179 * cm,
                                                 0.90 * cm};
...
// original
void FCAL_ChannelMap::add_tube(const std::string& tileName, int mod, int /*id*/,
...
//   June 2002  HMA
// ***********************************************************************
bool FCAL_ChannelMap::getTileID(int isam, float x_orig, float y_orig, int& eta,
                                int& phi) const {
  ...
  int ix = ktx +
           ((int)((firstId & 0xffff) - it->second.x() / m_tubeDx[isam - 1])) +
           1;
  int iy =
...
float FCAL_ChannelMap::x(int isam, int eta, int phi) const {
  ...
  if (it != m_tileMap[isam - 1].end()) {
    ...
    snprintf(l_str, sizeof(l_str),
             "Error in FCAL_ChannelMap::x, wrong tile,phi= %d ,eta=: %d ", phi,
             eta);
...
void FCAL_ChannelMap::tileSize(int sam, int ntubes, float& dx,
                               float& dy) const {
...
void FCAL_ChannelMap::tileSize(int sam, int eta, int phi, float& dx,
                               float& dy) const {
...
// ********************** tubemap_begin **************************************
FCAL_ChannelMap::tubemap_const_iterator FCAL_ChannelMap::tubemap_begin(int imap)
    const {
  return m_tubeMap[imap - 1].begin();
}

// ********************** tubemap_end ****************************************
FCAL_ChannelMap::tubemap_const_iterator FCAL_ChannelMap::tubemap_end(int imap)
    const {
  return m_tubeMap[imap - 1].end();
}

// ********************** tubemap_size ***************************************
FCAL_ChannelMap::tubemap_sizetype FCAL_ChannelMap::tubemap_size(int imap)
    const {
