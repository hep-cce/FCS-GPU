...
int main(int argc, char **argv) {
  ...
  std::cout << "Settings :: particle : " << doPart
            << "  layer : " << selected_layer << std::endl;
  TH1::SetDefaultSumw2(kTRUE);
  ...
  Float_t xbins[nxbins] = {-0.4,   -0.1,   -0.05,  -0.04, -0.03,  -0.02,
                           -0.016, -0.014, -0.012, -0.01, -0.008, -0.006,
                           -0.004, -0.002, 0,      0.002, 0.004,  0.006,
                           0.008,  0.01,   0.012,  0.014, 0.016,  0.02,
                           0.03,   0.04,   0.05,   0.1,   0.4};
  Float_t ybins[nybins] = {-0.4,   -0.1,   -0.05,  -0.04, -0.03,  -0.02,
                           -0.016, -0.014, -0.012, -0.01, -0.008, -0.006,
                           -0.004, -0.002, 0,      0.002, 0.004,  0.006,
                           0.008,  0.01,   0.012,  0.014, 0.016,  0.02,
                           0.03,   0.04,   0.05,   0.1,   0.4};
  ...
  Float_t ybinsPos[nybinsPos] = {0,    0.002, 0.004, 0.006, 0.008,
                                 0.01, 0.012, 0.014, 0.016, 0.02,
                                 0.03, 0.04,  0.05,  0.1,   0.4};

  // REGULAR BINNIN IN ALPHA
  const Int_t nxbinsReg = 9;
  float rotZach = TMath::Pi() / 8.;
  Float_t xbinsReg[nxbinsReg] = {
      0 + rotZach,                 0.25 * TMath::Pi() + rotZach,
      0.5 * TMath::Pi() + rotZach, 0.75 * TMath::Pi() + rotZach,
      TMath::Pi() + rotZach,       1.25 * TMath::Pi() + rotZach,
      1.5 * TMath::Pi() + rotZach, 1.75 * TMath::Pi() + rotZach,
      2 * TMath::Pi() + rotZach};
  ...
  if (doPart == "e")
    myfile =
        "INPUTS/"
        "ISF_HitAnalysisESD_evgen_calo__11__E50000_50000__eta20_25_Evts0-5500_"
        "zvertex_0.pool.root";
  if (doPart == "pi")
    // myfile =
    ...
    myfile =
        "INPUTS/"
        "ISF_HitAnalysis_evgen_calo__211_E50000_50000_eta20_25_Evts0-5500_vz_0_"
        "origin_calo.pool.root";
  // myfile =
  // "INPUTS/ISF_HitAnalysisESD_evgen_calo__211__E50000_50000__eta20_25_Evts0-5500_zvertex_0.pool.root"
  // ;
  ...
  std::cout << "########################################################## "
            << std::endl;
  std::cout << "1. CHRISTOPHER BINNING ################################### "
            << std::endl;

  TH1F halpha("halpha", "halpha", 7500000, 0 + rotZach,
              2 * TMath::Pi() + rotZach);
  ...
  TH2F halphadrE =
      fBinning(myfile, nxbins, xbins, nybins, ybins, nxbinsReg, xbinsReg,
               nybinsPos, ybinsPos, "Christopher", selected_layer, doPart,
               halpha, hdr, halphaE, hdrE, mEnergy, mxEnergy, myEnergy);
  ...
  while (isEmptyBin) {
    ...
    TH2F halphadrE2 = fBinning(
        myfile, nxbins, xbins, nybins, ybins, nAlphaBinsVal, vAlphaBins,
        nRBinsVal, vRBins, "AlphaBinned", selected_layer, doPart, halpha2, hdr2,
        halphaE2, hdrE2, mEnergy2, mxEnergy2, myEnergy2);
    ...
    for (Int_t i = 1; i < halphadrE2.GetNbinsX() + 1; i++) {
      ...
      for (Int_t j = 1; j < halphadrE2.GetNbinsY() + 1; j++) {
        if (halphadrE2.GetBinContent(i, j) < tolerance) {
          std::cout << "EMPTY BIN (" << i << "," << j
                    << "), checking new bin number" << std::endl;
          isEmptyBin = true;
          nprob++;
        }
        // Check that the energy variation is lower than 0.30 in the radial
        // direction
        if (j > 1) {
          float relvar = fabs((TMath::Log(halphadrE2.GetBinContent(i, j)) -
          ...
          if ((relvar > nrelvar1 && j < 0.5 * halphadrE2.GetNbinsY()) ||
            ...
            std::cout << "TOO LARGE E VARIATION BIN (" << i << "," << j
                      << ") : " << relvar << ", checking new bin number"
                      << std::endl;
            isEmptyBin = true;
            nprob++;
          }
  ...
  // Need to recreate the vector vRBins outside of the loop with the latest
  // number of dr bins
  doR = true;
  std::vector<float> vRBinsSTD = fVecBin(hdr, nRBins, doR);
  for (Int_t i = 0; i < vRBinsSTD.size(); i++) {
  ...
  while (isEmptyBinlast) {
    ...
    vRBinslast[0] =
        vRBins[nRBinsVal - 2];  // start of last bin => start of last sub-bins
    vRBinslast[nRBins2last - 1] =
        vRBins[nRBinsVal - 1];  // end of last bin   => end of last sub-bins
    ...
    for (Int_t i = 1; i < nRBins2last - 1; i++) {
      ...
      if (!doexp)
        vRBinslast[i] =
            vRBinslast[0] +
            i * ((vRBinslast[nRBins2last - 1] - vRBinslast[0]) / nRBinslast);
      if (doexp)
        vRBinslast[i] =
            vRBinslast[0] +
            (i * ((vRBinslast[nRBins2last - 1] - vRBinslast[0]) / nRBinslast)) /
    ...
    std::cout << nRBins2last - 1 << "  " << vRBinslast[nRBins2last - 1]
              << std::endl;
    ...
    TH2F halphadrE3 = fBinning(
        myfile, nxbins, xbins, nybins, ybins, nAlphaBins + 1, vAlphaBins,
        nRBins + nRBinslast, vRBinsAll, "Rlast", selected_layer, doPart,
        halpha3, hdr3, halphaE3, hdrE3, mEnergy3, mxEnergy3, myEnergy3);
    ...
    for (Int_t i = 1; i < halphadrE3.GetNbinsX() + 1; i++) {
      for (Int_t j = 1; j < halphadrE3.GetNbinsY() + 1; j++) {
        if (halphadrE3.GetBinContent(i, j) < tolerance) {
          std::cout << "PROBLEM WITH BIN (" << i << "," << j
                    << ") : " << halphadrE3.GetBinContent(i, j) << std::endl;
          isEmptyBinlast = true;
          nproblast++;
        }
        if (j > 1) {
          float relvar = fabs((TMath::Log(halphadrE3.GetBinContent(i, j)) -
  ...
  std::cout << "THE FINAL NUMBER OF BINS IN LAST R BIN IS : " << nRBinslast
            << std::endl;
  ...
  for (Int_t i = 0; i < nRBinsVal - 1 + nRBinslast; i++) {
    ...
    if (i >= vRBinsSTD.size() - 1) {
      if (!doexp)
        vRBinsSTDlast.push_back(vRBinsSTD.at(vRBinsSTD.size() - 2) +
                                q * ((vRBinsSTD.at(vRBinsSTD.size() - 1) -
  ...
  for (Int_t i = 0; i < nRBins2last; i++) {
    ...
    if (i < nRBins2last - 1)
      mybinR2last[i + nRBins2last] = vRBinsSTDlast.at(i + 1);
  }
  ...
  std::cout << "########################################################## "
            << std::endl;
  std::cout << " 6. CHECK WITH LAST R BINNING ############################ "
            << std::endl;
  ...
  TH2F halphadrE4 =
      fBinning(myfile, nxbins, xbins, nybins, ybins, nAlphaBins + 1, vAlphaBins,
               nRBinslast + 1, vRBinslast, "Rfinal", selected_layer, doPart,
               halpha4, hdr4, halphaE4, hdrE4, mEnergy4, mxEnergy4, myEnergy4);
  ...
  std::cout << "########################################################## "
            << std::endl;
  std::cout << "7. FINAL CHECK :::::::::::: START TESTING EMPTY BINS"
            << std::endl;
  Int_t nprob = 0;
...
vector<float> fVecBin(TH1F myh, int nBins, bool doR) {
  ...
  std::cout << "INTEGRAL : " << myh.GetEntries() << "  " << myh.Integral()
            << "  " << TotalValue << "  " << nBins << "  " << PerBinValue
            << "  " << myh.GetNbinsX() + 1 << std::endl;
  ...
  for (Int_t g = 1; g < myh.GetNbinsX() + 1; g++) {
    ...
    if (mybincumul.at(q) < PerBinValue) {
      ...
      count++;
      if (g == myh.GetNbinsX())
        std::cout << q << "  " << mybincumul.at(q) << std::endl;
    }
    ...
    if (mybincumul.at(q) >= PerBinValue) {
      ...
      std::cout << "BIN FILLED : " << q << "  " << mybincumul.at(q)
                << std::endl;
      q++;
