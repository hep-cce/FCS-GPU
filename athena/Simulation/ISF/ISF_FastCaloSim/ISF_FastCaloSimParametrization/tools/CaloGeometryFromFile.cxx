...
bool CaloGeometryFromFile::LoadGeometryFromFile(TString filename,
                                                TString treename,
                                                TString hashfile) {
  ...
  while (1) {
    ...
    if (i % 10000 == 0)
      cout << "Line: " << i << " line " << line << " id " << hex << id
           << " hash_id " << dec << hash_id << endl;
    i++;
  }
  cout << "Done." << endl;
  ...
  for (Long64_t jentry = 0; jentry < nentries; jentry++) {
    ...
    if (g_cellId_vs_cellHashId_map.find(cell.m_identify) !=
        g_cellId_vs_cellHashId_map.end()) {
      cell.m_hash_id = g_cellId_vs_cellHashId_map[cell.m_identify];
      if (cell.m_hash_id != jentry)
        cout << jentry << " : ERROR hash=" << cell.m_hash_id << endl;
    } else
      cout << endl
...
bool CaloGeometryFromFile::LoadFCalGeometryFromFiles(TString filename1,
                                                     TString filename2,
                                                     TString filename3) {
