...
void wiggle_closure_inputs(TString sampling = "Sampling_0") {
  ...
  TH1F *eff_tot_phi =
      new TH1F("eff_tot_phi_" + sampling, "eff_tot_phi_" + sampling, 50, -1, 1);
  TH1F *eff_corr_phi = new TH1F("eff_corr_phi_" + sampling,
                                "eff_corr_phi_" + sampling, 50, -1, 1);
  TH1F *eff_tot_count_phi =
      new TH1F("eff_tot_count_phi_" + sampling, "eff_tot_count_phi_" + sampling,
               50, -1, 1);
  TH1F *eff_corr_count_phi =
      new TH1F("eff_corr_count_phi_" + sampling,
               "eff_corr_count_phi_" + sampling, 50, -1, 1);
  eff_corr_phi->Sumw2();
  eff_tot_phi->Sumw2();
  ...
  for (Int_t ientry = 0; ientry < nEvt; ientry++) {
    ...
    for (UInt_t j = 0; j < (*vec).size(); j++) {
      ...
      for (unsigned int ihit = 0;
           ihit < ((FCS_matchedcell)(*vec)[j]).hit.size(); ihit++) {
        ...
        if (hitSampling < 21)
          ...
          cout << endl << "Warning: Found hit with sampling > 23 !!!!!!!!!!!!!!"
               << endl << endl;
          foundCell = 0;
        }
        ...
        if (foundCell) {  // && cell){
          if (hitSampling < 21) {
            eff_tot_phi->Fill(
                ((2 * (hitVec->Phi() - foundCell->phi())) / foundCell->dphi()),
                weight);
            eff_tot_count_phi->Fill(
                ((2 * (hitVec->Phi() - foundCell->phi())) / foundCell->dphi()));

            if (foundCell->identify() == cell->identify()) {

              eff_corr_phi->Fill(((2 * (hitVec->Phi() - foundCell->phi())) /
                                  foundCell->dphi()),
                                 weight);
              eff_corr_count_phi->Fill(
                  ((2 * (hitVec->Phi() - foundCell->phi())) /
                   foundCell->dphi()));
            }
          } else if (hitSampling < 24) {
            eff_tot_phi->Fill(((2 * (x - foundCell->x())) / foundCell->dx()),
                              weight);
            eff_tot_count_phi->Fill(
                ((2 * (x - foundCell->x())) / foundCell->dx()));

            if (foundCell->identify() == cell->identify()) {

              eff_corr_phi->Fill(((2 * (x - foundCell->x())) / foundCell->dx()),
                                 weight);
              eff_corr_count_phi->Fill(
                  ((2 * (x - foundCell->x())) / foundCell->dx()));
            }
  ...
  eff_phi->GetXaxis()->SetTitle(
      "2*(#phi_{hit}-#phi_{assigCell})/dphi_{assigCell}");
